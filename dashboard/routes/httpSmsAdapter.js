...
router.get('/messages/outstanding', async (req, res) => {
    try {
        ...
        // QUEUE-02: leased claim. A row is only offered when its previous
        // claim expired (5 minutes) and the retry budget is not exhausted, and
        // it is claimed atomically so overlapping polls cannot double-send.
        const row = await db.get(
            `SELECT id, external_id, from_number, to_number, message, timestamp, status, sim_slot, encrypted
             ...
             WHERE device_id = ?
               ...
               AND (http_claimed_at IS NULL OR datetime(http_claimed_at) < datetime('now', '-5 minutes'))
               AND COALESCE(http_claim_attempts, 0) < 10
               ${filter}
        ...
        const claim = await db.run(
            `UPDATE sms
             SET status = 'sending',
                 http_claimed_at = CURRENT_TIMESTAMP,
                 http_claim_attempts = COALESCE(http_claim_attempts, 0) + 1
             WHERE device_id = ?
               AND id = ?
               AND status IN ('queued', 'pending', 'sending')
               AND (http_claimed_at IS NULL OR datetime(http_claimed_at) < datetime('now', '-5 minutes'))`,
        ...
        if (!claim.changes) {
            // Another poll just claimed this row; report none for this tick.
            return res.status(200).json({ status: 'success', message: 'no outstanding message', data: null });
        }
        // Terminal-event settlement resets the claim state so a retry cycle
        // can never resurrect a settled row (handled by event endpoint status).
