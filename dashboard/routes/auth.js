...
// Logout handler (SEC-09): POST-only + CSRF token. GET /logout no longer
// destroys the session — a forged <img src=/auth/logout> cannot log a user out.
...
router.post('/logout', (req, res) => {
    // CSRF: the global guard exempts /auth/*, so logout validates the
...
router.get('/logout', (req, res) => {
    // Legacy GET bookmarks: redirect without destroying the session.
    if (!req.session?.user) return res.redirect('/auth/login');
    res.redirect('/dashboard');
});
