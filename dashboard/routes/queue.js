...
router.delete('/:id', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const db = req.app.locals.db;
        if (!db) {
            return res.status(503).json({ success: false, message: 'Database unavailable' });
...
router.post('/clear', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const db = req.app.locals.db;
        if (!db) {
            return res.status(503).json({ success: false, message: 'Database unavailable' });
...
router.post('/actions/restart-device', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const deviceId = normalizeDeviceId(resolveDeviceId(req, DEFAULT_DEVICE_ID));
        if (!deviceId) {
            return res.status(400).json({ success: false, message: 'No active device selected' });
...
router.post('/actions/restart-modem', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const deviceId = normalizeDeviceId(resolveDeviceId(req, DEFAULT_DEVICE_ID));
        if (!deviceId) {
            return res.status(400).json({ success: false, message: 'No active device selected' });
