...
router.get('/webcam/settings', requireDeviceAccess(), async (req, res) => {
    try {
        const deviceId = req.query.deviceId || DEFAULT_DEVICE_ID;
        const db = req.app.locals.db;
...
router.post('/webcam/settings', requireDeviceAccess(undefined, true), [
...
router.post('/webcam/capture', requireDeviceAccess(undefined, true), [
    body('deviceId').optional(),
 ...
 */
router.post('/video/settings', requireDeviceAccess(undefined, true), [
    body('enabled').optional().isBoolean(),
    body('resolution').optional().isIn(['1600x1200', '1280x1024', '1024x768', '800x600', '640x480', '352x288', '320x240']),
    body('fps').optional().isInt({ min: 1, max: 60 }),
    body('quality').optional().isInt({ min: 10, max: 100 }),
 ...
 */
router.post('/audio/settings', requireDeviceAccess(undefined, true), [
    body('enabled').optional().isBoolean(),
 ...
 */
router.post('/servers', requireDeviceAccess(undefined, true), [
 ...
 */
router.post('/call/start', requireDeviceAccess(undefined, true), [
 ...
 */
router.post('/call/end', requireDeviceAccess(undefined, true), [
    body('deviceId').optional()
], async (req, res) => {
    try {
 ...
 */
router.post('/signal', requireDeviceAccess(undefined, true), [
 ...
 */
router.get('/ice-servers', requireDeviceAccess(), async (req, res) => {
    try {
        setNoStoreHeaders(res);
        const deviceId = req.query.deviceId || DEFAULT_DEVICE_ID;
        const db = req.app.locals.db;
 ...
 */
router.post('/snapshot', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const deviceId = req.query.deviceId || DEFAULT_DEVICE_ID;
        const db = req.app.locals.db;
 ...
 */
router.get('/history', requireDeviceAccess(), async (req, res) => {
    try {
        setNoStoreHeaders(res);
        const deviceId = req.query.deviceId || DEFAULT_DEVICE_ID;
 ...
 */
router.get('/snapshots', requireDeviceAccess(), async (req, res) => {
    try {
        setNoStoreHeaders(res);
        const deviceId = req.query.deviceId || DEFAULT_DEVICE_ID;
        const limit = Math.min(Math.max(1, parseInt(req.query.limit) || 50), 500);
 ...
 */
router.delete('/snapshots/:filename', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
...
router.get('/captures', requireDeviceAccess(), async (req, res) => {
    try {
        setNoStoreHeaders(res);
        const deviceId = req.query.deviceId || DEFAULT_DEVICE_ID;
        const captures = await listCaptures({
...
router.get('/captures/export', requireDeviceAccess(), async (req, res) => {
    try {
        const deviceId = req.query.deviceId || DEFAULT_DEVICE_ID;
        const captures = await listCaptures({
            db: req.app.locals.db,
...
router.delete('/captures/:id', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
