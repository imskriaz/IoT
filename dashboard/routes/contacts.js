...
// ---------------------------------------------------------------------------
// Every contact belongs to exactly one user (contacts.user_id). Admins (and
// the env superadmin) see every row; non-admins only see rows they own.
// Legacy rows created before the migration have user_id NULL and are
// visible to admins only. Mutations additionally require ownership, so a
// non-admin can never read, update, delete, favorite, or claim a contact
...
function currentUserId(req) {
    return Number.isFinite(Number(req.user?.id))
        ? Number(req.user.id)
        : (Number.isFinite(Number(req.session?.user?.id)) ? Number(req.session.user.id) : null);
}

/**
 * Ownership check for a fetched contact row.
 * Returns true when the requesting user may act on the row.
 */
function canAccessContact(req, contact) {
    if (!contact) return false;
    if (isAdminUser(req.user)) return true;
    return contact.user_id === currentUserId(req);
}

/** SQL fragment + params that scope SELECTs to the requesting user. */
function ownershipClause(req, alias = '') {
    if (isAdminUser(req.user)) return { clause: '', params: [] };
    return { clause: ` AND ${alias}user_id = ?`, params: [currentUserId(req) ?? -1] };
}

function emitUserEvent(req, event, payload) {
    // User-scoped emit: other tenants must not learn that a contact exists.
    if (!global.io) return;
...
router.get('/', async (req, res) => {
    try {
        ...
        const owner = ownershipClause(req);
...
router.get('/search/:query', async (req, res) => {
    try {
        ...
        const contacts = await db.all(`
            ...
            FROM contacts
            WHERE 1=1 AND (name LIKE ? OR phone_number LIKE ?)${owner.clause}
            ORDER BY favorite DESC, name ASC
            LIMIT ?
        `, [`%${query}%`, `%${query}%`, ...owner.params, limit]);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (!contact || !canAccessContact(req, contact)) {
            // 404 for foreign contacts: existence must not leak cross-tenant.
...
router.post('/', [
    ...
    try {
        ...
        const result = await db.run(`
            INSERT INTO contacts (user_id, name, phone_number, email, company, favorite, notes, created_at, updated_at)
        ...
        emitUserEvent(req, 'contact:created', newContact);
        res.json({
            success: true,
...
router.put('/:id', [
    ...
    try {
        ...
        // Check if contact exists AND is owned by (or visible to) the caller
        const existing = await db.get('SELECT * FROM contacts WHERE id = ?', [id]);
        if (!existing || !canAccessContact(req, existing)) {
            return res.status(404).json({
                success: false,
                message: 'Contact not found'
            });
        ...
        emitUserEvent(req, 'contact:updated', updatedContact);
        res.json({
            success: true,
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        // Ownership-scoped delete: non-admins can only delete their own rows.
        const owner = ownershipClause(req);
        const result = await db.run(
            `DELETE FROM contacts WHERE id = ?${owner.clause}`,
            [id, ...owner.params]
        );
        ...
        emitUserEvent(req, 'contact:deleted', { id: Number(id) });
        res.json({
            success: true,
...
router.patch('/:id/favorite', async (req, res) => {
    try {
        ...
        emitUserEvent(req, 'contact:favorite', { id: Number(id), is_favorite: updatedContact.favorite });
        res.json({
            success: true,
...
router.get('/export/csv', async (req, res) => {
    try {
        ...
        const owner = ownershipClause(req);
        const contacts = await db.all(
            `SELECT name, phone_number, email, company, notes, favorite FROM contacts WHERE 1=1${owner.clause} ORDER BY name`,
            owner.params
        );
 ...
 *
 * Rows whose phone number is already owned by another tenant are skipped:
 * an import can claim or update only the caller's own contacts (admins may
 * update any row).
 */
...
/**
 * Resolves import-row ownership.
 * Returns 'insert' | 'update' | 'skip' for the requesting user + phone.
 */
async function resolveImportAction(db, req, formattedPhone) {
    const existing = await db.get(
        'SELECT id, user_id FROM contacts WHERE phone_number = ?',
        [formattedPhone]
    );
    if (!existing) return { action: 'insert' };
    if (canAccessContact(req, existing)) return { action: 'update', id: existing.id };
    return { action: 'skip' };
}
...
router.post('/import/csv', csvUpload.single('file'), async (req, res) => {
    try {
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            const email   = emailIdx >= 0 ? (cells[emailIdx]?.replace(/^"|"$/g, '') || null) : null;
            const company = companyIdx >= 0 ? (cells[companyIdx]?.replace(/^"|"$/g, '') || null) : null;
            const notes   = notesIdx >= 0 ? (cells[notesIdx]?.replace(/^"|"$/g, '') || null) : null;
            const favorite = favIdx >= 0 ? (cells[favIdx] === '1' ? 1 : 0) : 0;

            try {
                const resolution = await resolveImportAction(db, req, formatted);
                ...
                if (resolution.action === 'update') {
                    await db.run(
                        `UPDATE contacts SET
                         ...
                         WHERE id = ?`,
                        [name, email, company, notes, resolution.id]
                    );
                } else {
                    await db.run(
                        `INSERT INTO contacts (user_id, name, phone_number, email, company, notes, favorite)
                         VALUES (?, ?, ?, ?, ?, ?, ?)`,
                        [userId, name, formatted, email, company, notes, favorite]
                    );
                }
                imported++;
            } catch (e) {
...
router.get('/export/vcf', async (req, res) => {
    try {
        ...
        const owner = ownershipClause(req);
        const contacts = await db.all(
            `SELECT name, phone_number, email, company, notes FROM contacts WHERE 1=1${owner.clause} ORDER BY name`,
            owner.params
        );
 ...
 *
 * Same ownership rule as CSV import: foreign rows are skipped.
 */
                ...
                if (resolution.action === 'update') {
                    await db.run(
                        `UPDATE contacts SET
                            ...
                            updated_at = CURRENT_TIMESTAMP
                         WHERE id = ?`,
                        [name, email || null, company || null, notes || null, resolution.id]
                    );
                } else {
                    await db.run(
                        `INSERT INTO contacts (user_id, name, phone_number, email, company, notes)
                         VALUES (?, ?, ?, ?, ?, ?)`,
                        [userId, name, formatted, email || null, company || null, notes || null]
                    );
                }
                imported++;
            } catch (_) { skipped++; }
