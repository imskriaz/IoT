...
const { admin: adminMiddleware, requireDeviceAccess } = require('../middleware/auth');
...
router.post('/request-status/:deviceId', requireDeviceAccess('deviceId', true), async (req, res) => {
    try {
...
// Capture image
router.post('/capture/:deviceId', requireDeviceAccess('deviceId', true), async (req, res) => {
    try {
        const { deviceId } = req.params;
