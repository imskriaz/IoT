...
router.get('/session', requireDeviceAccess(), async (req, res) => {
    try {
        const db = req.app.locals.db;
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
        const simScope = resolveRequestSimScope(req);
...
router.post('/session/end', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const db = req.app.locals.db;
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
        const simScope = resolveRequestSimScope(req);
...
router.delete('/history', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const db = req.app.locals.db;
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
        const simScope = resolveRequestSimScope(req);
