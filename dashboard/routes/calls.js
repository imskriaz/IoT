...
function publishInteractiveTelephonyCommand(deviceId, command, payload, timeout, waitForResponse = false) {
...
async function callDeviceFamily(db, deviceId) {
    const { row, caps } = await getDeviceCapabilities(db, deviceId);
    const identity = [row?.board, row?.type, caps?.board, caps?.chip, caps?.bridge]
        .map((value) => String(value || '').toLowerCase()).join(' ');
    if (/esp32|a7670/.test(identity)) return 'esp32';
    if (/android/.test(identity)) return 'android';
    return 'unknown';
}

function completedCallResult(result) {
    return String(result?.result || '').toLowerCase() === 'completed';
}

function rejectedCallResult(result) {
    return ['failed', 'rejected', 'timeout', 'error'].includes(String(result?.result || '').toLowerCase());
}
 ...
 */
router.get('/logs', requireDeviceAccess(), async (req, res) => {
    try {
...
// Get call stats
router.get('/stats', requireDeviceAccess(), async (req, res) => {
    try {
        const db = req.app.locals.db;
        if (!db) {
            throw new Error('Database not available');
...
router.get('/recent', requireDeviceAccess(), async (req, res) => {
    try {
        const db = req.app.locals.db;
        if (!db) {
            throw new Error('Database not available');
...
router.post('/sync', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        ...
        if (await callDeviceFamily(req.app.locals.db, deviceId) !== 'android') {
            return res.status(501).json({ success: false, message: 'Call history sync is not supported by this device' });
        }
...
// Get single call
router.get('/logs/:id', requireDeviceAccess(), async (req, res) => {
    try {
        const { id } = req.params;
...
router.post('/dial', dialRateLimit, [
    ...
    try {
        ...
        if (global.mqttService.isDeviceOnline?.(deviceId) === false) {
            return res.status(503).json({ success: false, message: 'Device offline' });
        }
        const commandResult = await publishInteractiveTelephonyCommand(deviceId, 'make-call', {
            number: formattedNumber,
            ...buildCallSimMeta(simScope)
        }, 45000, true);
        if (!completedCallResult(commandResult)) {
            const failed = rejectedCallResult(commandResult);
            return res.status(failed ? 502 : 202).json({
                success: false,
                pending: !failed,
                message: failed ? (commandResult?.message || 'Device failed to dial') : 'Dial result pending; call not confirmed',
                messageId: commandResult?.messageId || null
            });
        }
        const result = await db.run(`
        ...
        res.json({
            success: true,
            message: 'Device accepted dial command',
        ...
        res.status(/timeout/i.test(String(error?.message || '')) ? 504 : 500).json({
            success: false,
            message: /timeout/i.test(String(error?.message || ''))
                ? 'Dial result timed out; execution is unknown'
...
// End current call
router.post('/end', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const db = req.app.locals.db;
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
        const simScope = resolveRequestSimScope(req);
        ...
        const commandResult = await publishInteractiveTelephonyCommand(
            deviceId, 'end-call', buildCallSimMeta(simScope), 45000, true
        );
        if (!completedCallResult(commandResult)) {
            const failed = rejectedCallResult(commandResult);
            return res.status(failed ? 502 : 202).json({
                success: false,
                pending: !failed,
                message: failed ? (commandResult?.message || 'Device failed to end call') : 'Hangup result pending; call state unchanged',
                messageId: commandResult?.messageId || null
            });
        }
        ...
        if (result.changes > 0) logger.info('Device accepted hangup command');

        res.json({
            success: true,
            message: callTransportSuspended
                ? 'Device accepted hangup while voice transport is busy; waiting for call state'
                : 'Device accepted hangup; waiting for call state',
            queued: Boolean(commandResult?.queued),
            queueId: commandResult?.queueId || null,
            messageId: commandResult?.messageId || null,
        ...
        res.status(/timeout/i.test(String(error?.message || '')) ? 504 : 500).json({
            success: false,
            message: /timeout/i.test(String(error?.message || ''))
                ? 'Hangup result timed out; execution is unknown'
                : 'Failed to end call'
        });
    }
});
...
router.post('/answer', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const db = req.app.locals.db;
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
        const simScope = resolveRequestSimScope(req);
        if (!db) throw new Error('Database not available');
        if (await callDeviceFamily(db, deviceId) !== 'android') {
            return res.status(501).json({ success: false, message: 'Answer call is not supported by this device' });
        }

        let commandResult = null;
        try {
            commandResult = await publishInteractiveTelephonyCommand(
                deviceId, 'answer-call', buildCallSimMeta(simScope), 45000, true
            );
        } catch (mqttError) {
            logger.error('MQTT error answering call:', mqttError);
            return res.status(/timeout/i.test(String(mqttError?.message || '')) ? 504 : 503).json({
                success: false,
                pending: /timeout/i.test(String(mqttError?.message || '')),
                message: /timeout/i.test(String(mqttError?.message || ''))
                    ? 'Answer result timed out; execution is unknown'
                    : 'Device answer command could not be delivered'
            });
        }
        if (!completedCallResult(commandResult)) {
            const failed = rejectedCallResult(commandResult);
            return res.status(failed ? 502 : 202).json({
                ...
                pending: !failed,
                message: failed
                    ? (commandResult?.message || 'Device failed to answer call')
                    : 'Answer result pending; call state unchanged',
                messageId: commandResult?.messageId || null
            });
        }
...
router.post('/reject', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const db = req.app.locals.db;
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
        const simScope = resolveRequestSimScope(req);
        if (!db) throw new Error('Database not available');
        if (await callDeviceFamily(db, deviceId) !== 'android') {
            return res.status(501).json({ success: false, message: 'Reject call is not supported by this device' });
        }

        let commandResult = null;
        try {
            commandResult = await publishInteractiveTelephonyCommand(
                deviceId, 'reject-call', buildCallSimMeta(simScope), 45000, true
            );
        } catch (mqttError) {
            ...
            return res.status(/timeout/i.test(String(mqttError?.message || '')) ? 504 : 503).json({
                ...
                pending: /timeout/i.test(String(mqttError?.message || '')),
                message: /timeout/i.test(String(mqttError?.message || ''))
                    ? 'Reject result timed out; execution is unknown'
                    : 'Device reject command could not be delivered'
            });
        }
        if (!completedCallResult(commandResult)) {
            const failed = rejectedCallResult(commandResult);
            return res.status(failed ? 502 : 202).json({
                ...
                pending: !failed,
                message: failed
                    ? (commandResult?.message || 'Device failed to reject call')
                    : 'Reject result pending; call state unchanged',
                messageId: commandResult?.messageId || null
            });
        }
...
router.get('/status', requireDeviceAccess(), async (req, res) => {
    try {
        ...
        const simScope = resolveRequestSimScope(req);
        const modemStatus = global.modemService?.getStatus?.(deviceId) || {};
        const callExplicitlyInactive = modemStatus.online === true && modemStatus?.call?.active === false;
        if (callExplicitlyInactive) {
            const reconciled = await reconcileInactiveCallRows(db, deviceId, simScope);
            if (reconciled > 0) {
                emitDeviceEvent(deviceId, 'call:ended', { deviceId, status: 'ended', reconciled });
            }
        }
        const activeConditions = ['device_id = ?', "status IN ('dialing', 'ringing', 'connected', 'answered')"];
        const activeParams = [deviceId];
...
// Delete call log
router.delete('/logs/:id', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const { id } = req.params;
        if (!isValidNumericId(id)) {
            return res.status(400).json({ success: false, message: 'Invalid call id' });
...
router.post('/logs/bulk-delete', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const { ids } = req.body;
...
router.post('/hold', [
    ...
    res.status(501).json({ success: false, message: 'Call hold and resume are not supported' });
});
...
router.post('/mute', [
    ...
    res.status(501).json({ success: false, message: 'Call mute and unmute are not supported' });
});
