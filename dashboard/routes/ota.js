...
const { prepare: prepareOta } = require('../services/otaLifecycle');
...
router.post('/flash', adminMiddleware, async (req, res) => {
    try {
        ...
        const deviceSnapshot = global.mqttService.deviceStatus?.get(deviceId);
        let otaMetadata;
        try {
            otaMetadata = prepareOta(filePath, filename, deviceSnapshot,
        ...
        const commandResponse = await global.mqttService.otaLifecycle.enqueue(
            deviceId,
            { url: firmwareUrl, timeout: OTA_COMMAND_TIMEOUT_MS, _ota: otaMetadata },
        ...
        logger.info(`OTA flash queued on ${deviceId} — awaiting boot verification`, { filename });
        res.status(202).json({ success: true, message: 'OTA update accepted; waiting for verified reboot', data: {
            deviceId, filename, firmwareUrl, response: commandResponse, ota: otaMetadata
        } });
    } catch (error) {
        logger.error('OTA flash error:', error);
        if (error?.code === 'OTA_ALREADY_PENDING') {
...
router.post('/rollback', adminMiddleware, async (req, res) => {
    try {
        ...
        logger.info(`OTA rollback queued on ${deviceId} — awaiting boot verification`, { filename });
        res.status(202).json({ success: true, message: 'Rollback accepted; waiting for verified reboot', data: {
            deviceId, filename, firmwareUrl, response: commandResponse, ota: otaMetadata
        } });
    } catch (error) {
