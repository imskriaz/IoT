...
const { assertDeliverableUrl } = WebhookService;
const { withEffectiveRole } = require('../middleware/auth');

/** Validate destination policy (https-only public target) at write time. */
...
/** All device_ids in a filter must be assigned to the user (admin bypass). */
async function validateWebhookDeviceScope(req, db, deviceIds) {
    if (!Array.isArray(deviceIds) || !deviceIds.length) return null;
    const user = withEffectiveRole(req.user || req.session?.user);
    if (user?.role === 'admin' || user?.role === 'superadmin') return null;
    const assigned = new Set((await db.all(
        'SELECT device_id FROM device_users WHERE user_id = ?', [user?.id ?? -1]
    )).map(row => String(row.device_id)));
    const foreign = deviceIds.map(String).filter(id => !assigned.has(id));
    return foreign.length ? `No access to device(s): ${foreign.join(', ')}` : null;
}

function emitUserEvent(req, event, payload) {
    // User-scoped emit: other tenants no longer learn that a webhook exists.
    if (!global.io) return;
    global.io.to(`user:${req.session.user.id}`).emit(event, payload);
}
...
router.post('/', webhookValidation, async (req, res) => {
    try {
        ...
        const urlError = await validateWebhookDestination(url);
        if (urlError) return res.status(400).json({ success: false, message: urlError });
        const scopeError = await validateWebhookDeviceScope(req, db, device_ids);
        if (scopeError) return res.status(403).json({ success: false, message: scopeError });
        ...
        emitUserEvent(req, 'webhook:created', newWebhook);
...
router.put('/:id', [
    ...
    try {
        ...
        emitUserEvent(req, 'webhook:updated', updatedWebhook);
...
router.delete('/:id', [param('id').isInt({ min: 1 })], async (req, res) => {
    try {
        ...
        emitUserEvent(req, 'webhook:deleted', { id: parseInt(req.params.id) });
        res.json({ success: true });
    } catch (error) {
...
router.post('/:id/test', [param('id').isInt({ min: 1 })], async (req, res) => {
    try {
        ...
        const outcome = await webhookService.deliverOnce(hook, 'test', 'test-device', {
            message: 'This is a test delivery from ESP32 Dashboard',
            timestamp: new Date().toISOString()
        });
        if (!outcome.ok) {
            return res.status(502).json({ success: false, message: `Test delivery failed: ${outcome.error}` });
        }
        res.json({ success: true, message: `Test payload delivered (HTTP ${outcome.status})` });
    } catch (error) {
