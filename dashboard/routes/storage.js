...
// The active ESP32 registry implements SD list/read/delete, but no write,
// create, move, copy, compress, or format operation. Reject these before
// multer stages uploads or a dashboard publish can be mistaken for execution.
const unsupportedStorageMutations = new Set([
    '/upload', '/save', '/touch', '/compress', '/mkdir', '/format',
    '/rename', '/move', '/copy'
]);

function getStorageMutationSupport(deviceId) {
    const status = global.modemService?.getDeviceStatus?.(deviceId) || {};
    const storage = status.storage || status.status?.storage || {};
    const mediaType = String(
        storage.mediaType || storage.type || storage.label || status.storage_media_type || status.storage_media_label || ''
    ).trim().toLowerCase();
    const physicalSd = status.sdCard || {};
    const online = status.online === true;
    const mounted = storage.mounted === true && storage.mediaAvailable === true;
    const sdIdentity = /(^|\b)(micro\s*sd|sd\s*card|sdhc|sdxc|sd)(\b|$)/i.test(mediaType)
        && !/flash|internal|littlefs|spiffs/i.test(mediaType);
    const physicalIdentity = physicalSd.mounted === true && physicalSd.detected === true;
    const sdOnlyDelete = online && mounted && sdIdentity && physicalIdentity;
    ...
    return {
        ...
        reason: sdOnlyDelete
            ? null
            : 'Delete requires a fresh, mounted physical SD identity from corrected firmware'
    };
}
router.use((req, res, next) => {
    if (req.method === 'POST' && unsupportedStorageMutations.has(req.path)) {
        return res.status(501).json({
            success: false,
            message: req.path === '/format'
                ? 'Storage format is not supported by the active device contract'
                : 'Storage mutation is not supported by the active device contract'
        });
    }
    next();
});
...
function buildStorageInfoFromCachedStatus(deviceId) {
    const status = global.modemService?.getDeviceStatus?.(deviceId);
    const card = status?.sdCard || null;
    if (!card) {
        return null;
    }

    const mounted = card.mounted === true;
    const detected = card.detected === true;
    const total = Number(card.totalBytes || 0);
    const used = Number(card.usedBytes || 0);
    const free = Number(card.freeBytes || 0);

    return {
        success: true,
        data: {
            mutations: typeof getStorageMutationSupport === 'function'
                ? getStorageMutationSupport(deviceId)
                : { delete: false, write: false, format: false },
            sdCard: card,
            ...
            sd: {
                ...
                free: mounted ? free : 0,
                type: 'SD card',
                filesystem: card.filesystem || null,
                cardType: card.cardType || null,
                bus: card.bus || null,
                pins: null,
                lastError: card.error || null,
                error: mounted ? null : (card.error || 'SD card is not mounted')
            }
        }
    };
}
...
function normalizeStorageInfoResponse(response) {
    const total = Number(response?.total ?? response?.total_bytes ?? response?.stats?.total ?? 0);
    const used = Number(response?.used ?? response?.used_bytes ?? response?.stats?.used ?? 0);
    const free = Number(response?.free ?? response?.free_bytes ?? response?.stats?.free ?? 0);
    return {
        mounted: response?.mounted === true || response?.media_available === true || total > 0 || used > 0 || free > 0,
        cardDetected: response?.cardDetected === true || response?.sd_detected === true || response?.mounted === true,
        total,
        used,
        free,
        type: response?.type || 'SD card',
        filesystem: response?.filesystem || null,
        cardType: response?.cardType || null,
        bus: response?.bus || null,
        pins: response?.pins || null,
        lastError: response?.lastError || response?.sd_error || response?.error || null
    };
}
...
router.get('/list', requireDeviceAccess(), listStorageHandler);

// Main storage listing endpoint
router.get('/', requireDeviceAccess(), listStorageHandler);

// Get device storage info via MQTT
router.get('/info', requireDeviceAccess(), async (req, res) => {
    try {
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
        ...
        if (info.mounted) {
            res.json({
                success: true,
                data: {
                    mutations: getStorageMutationSupport(deviceId),
                    sdCard: global.modemService?.getDeviceStatus?.(deviceId)?.sdCard || null,
                    online: global.modemService?.getDeviceStatus?.(deviceId)?.online === true,
                    internal: {
                        available: false,
                        mounted: false,
                        total: 0,
...
// Read file content
router.get('/read', requireDeviceAccess(), async (req, res) => {
    try {
...
// Download file
router.get('/download', requireDeviceAccess(), async (req, res) => {
    try {
        const filePathInput = req.query.path || '';
        const filePathValidation = validateStoragePath(filePathInput, 'path');
        if (!filePathValidation.ok) {
...
// Upload file
router.post('/upload', requireDeviceAccess(undefined, true), upload.array('files'), checkUploadQuota, async (req, res) => {
    try {
...
router.post('/save', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
...
router.post('/touch', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
...
router.post('/download-multiple', requireDeviceAccess(undefined, true), async (req, res) => {
    res.status(501).json({
        success: false,
...
router.post('/compress', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        return res.status(501).json({
            success: false,
...
// Create directory
router.post('/mkdir', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
...
router.post('/delete', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const { items } = req.body;
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
        const mutationSupport = getStorageMutationSupport(deviceId);
        if (!mutationSupport.delete) {
            return res.status(409).json({
                success: false,
                message: mutationSupport.reason
            });
        }
...
router.post('/format', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
...
router.post('/rename', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
...
// Move files
router.post('/move', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
...
// Copy files
router.post('/copy', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const { items, destination } = req.body;
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
