 ...
 */
router.get('/current', requireDeviceAccess(), async (req, res) => {
...
router.get('/devices', async (req, res) => {
    try {
        // SEC-01: aggregate device listing is filtered to devices assigned to
        // the requesting user (admins see all).
        const { filterAuthorizedDevices } = require('../middleware/auth');
        const visible = await filterAuthorizedDevices(req, Array.from(lastLocations.keys()).map(id => ({ id })));
        const allowed = new Set(visible.map(device => String(device.id)));
        const devices = [];
        for (const [deviceId, location] of lastLocations) {
            if (!allowed.has(String(deviceId))) continue;
            devices.push({
                deviceId,
                ...location
            });
 ...
 */
router.post('/toggle', requireDeviceAccess(undefined, true), [
 ...
 */
router.post('/config', requireDeviceAccess(undefined, true), [
 ...
 */
router.delete('/history/:id', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const { id } = req.params;
        if (!/^[0-9]+$/.test(id)) {
 ...
 */
router.delete('/history/device/:deviceId', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
 ...
 */
router.get('/geofences', requireDeviceAccess(), async (req, res) => {
    try {
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
        const db = req.app.locals.db;
        const fences = await db.all(
 ...
 */
router.post('/geofences', requireDeviceAccess(undefined, true), [
 ...
 */
router.put('/geofences/:id', requireDeviceAccess(undefined, true), [
 ...
 */
router.delete('/geofences/:id', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const { id } = req.params;
        if (!/^[0-9]+$/.test(id)) {
            return res.status(400).json({ success: false, message: 'Invalid id' });
