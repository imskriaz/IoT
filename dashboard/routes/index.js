...
async function persistResolvedDeviceId(req, deviceId) {
    ...
    if (!normalizedDeviceId || !req?.session || req.session.deviceId === normalizedDeviceId) {
...
async function renderDashboardPage(req, res) {
    try {
        ...
        let recentSms = await db.all(`
            SELECT id, from_number, to_number, message, timestamp, read, type, source
            FROM sms
        ...
        if (hasSimScope(simScope)) {
            ...
            const conversationRows = await db.all(`
                SELECT id, device_id, from_number, to_number, message, timestamp, read, type, status, source,
        ...
        // Get contact count (scoped to the requesting user; admins see all)
        const contactOwnerClause = (req.user?.role === 'admin' || req.user?.role === 'superadmin')
            ? { clause: '', params: [] }
            : { clause: ' WHERE user_id = ?', params: [req.user?.id ?? -1] };
        const contactCount = await db.get(
            `SELECT COUNT(*) as count FROM contacts${contactOwnerClause.clause}`,
            contactOwnerClause.params
        );
        ...
        if (isDeviceConnected && global.modemService && typeof global.modemService.getDeviceStatus === 'function') {
            try {
                ...
                if (deviceStatus.storage) {
                    const deviceStorage = deviceStatus.storage;
                    storageInfo.sd = {
                        total: Number(deviceStorage.totalBytes || 0),
                        used: Number(deviceStorage.usedBytes || 0),
                        free: Number(deviceStorage.freeBytes || 0),
                        available: !!(deviceStorage.mediaAvailable || deviceStorage.mounted),
                        mounted: !!deviceStorage.mounted,
                        deviceBacked: true,
                        type: deviceStorage.type || deviceStorage.label || 'Device storage',
                        queueDepth: Number(deviceStorage.queueDepth || 0),
                        pendingUploads: Number(deviceStorage.pendingUploads || 0),
                        bufferedOnly: !!deviceStorage.bufferedOnly
                    };
                }
            } catch (statusError) {
        ...
        if (!deviceStatus.simNumber) {
            try {
                ...
                if (storedSimNumber) {
                    ...
                    deviceStatus.simNumberSource = 'stored';
                    deviceStatus.subscriberNumberSource = 'stored';
                    deviceStatus.sim = {
                        ...(deviceStatus.sim || {}),
        ...
        try {
            if (db && deviceId) {
                ...
                caps = mergeCapabilities(caps, capabilityData.caps);
            }
        } catch (_) {}
        ...
        try {
            ...
            logger.warn('Could not build dashboard module health:', moduleError.message);
        }
        ...
        res.render('pages/index', {
            ...
            deviceStatus,
            storageInfo,
            sdCardSummary: sdStatus.describe(deviceStatus.sdCard, isDeviceConnected),
            sdCardMeter: sdStatus.meter(deviceStatus.sdCard, isDeviceConnected),
            dataUsage,
            allDevices,
            healthScore,
            caps,
...
router.get('/sms', async (req, res) => {
    try {
        ...
        const messages = await db.all(`
            SELECT id, from_number, to_number, message, timestamp, read, type, status, sim_slot, source
            FROM sms
...
router.get('/devices', async (req, res) => {
    try {
        ...
        const devices = isAdmin
            ? await db.all(`
                SELECT d.*, dp.location, dp.firmware_version, dp.board,
              ...
              `)
            : await db.all(`
                SELECT d.*, dp.location, dp.firmware_version, dp.board, du.can_write
                FROM devices d
