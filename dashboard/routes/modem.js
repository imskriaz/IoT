...
const CONNECTION_POLICIES = new Set(runtimeCapabilities.routing.connectionPolicies);

function normalizeConnectionPolicy(value, fallback = 'dual_active_development') {
    const normalized = String(value || '').trim().toLowerCase();
    return CONNECTION_POLICIES.has(normalized) ? normalized : fallback;
}
...
let connectionState = {
    ...
    routing: {
        ...
        firewall: true,
        connectedDevices: 0,
        connectionPolicy: 'dual_active_development',
        developmentMode: true,
        concurrentLinksReady: false,
...
function resolveModemConnectedContract({ session, bearer, dataIp, activePath, legacyConnected } = {}) {
    ...
    if (normalizeModemActivePath(activePath) === 'modem') {
        return hasDataIp;
    }
...
async function readStoredWifiNetwork(req, deviceId, ssid) {
    ...
    const row = await db.get(
        `SELECT ssid, security, password, password_set, priority, enabled, connection_count, last_selected_at, last_connected_at,
...
async function listStoredWifiNetworks(req, deviceId, profile = null) {
    ...
    const rows = db && typeof db.all === 'function'
        ? await db.all(
            `SELECT ssid, security, password_set, priority, enabled, connection_count, last_selected_at, last_connected_at,
                    last_signal, last_channel, last_bssid, created_at, updated_at
             FROM device_wifi_networks
             WHERE device_id = ?
             ORDER BY
...
/* The dashboard database is the edit surface; this is the single boundary
 * that mirrors the complete, ordered profile set to the device. Passwords
 * never enter status or logs and the device terminal result is authoritative. */
async function applyStoredWifiProfilesToDevice(req, deviceId) {
    const db = req.app?.locals?.db;
    if (!db || typeof db.all !== 'function' || !global.mqttService?.connected) {
        return { applied: false, pending: true, reason: 'device_transport_unavailable' };
    }
    const rows = await db.all(
        `SELECT ssid, security, password, priority, enabled
         FROM device_wifi_networks WHERE device_id = ?
         ORDER BY enabled DESC, priority ASC, ssid COLLATE NOCASE ASC`,
        [deviceId]
    );
    const current = getLatestDeviceStatusSnapshot(deviceId) || {};
    const currentRevision = Number(current.wifi_profiles_revision ?? current.wifi?.profilesRevision ?? current.wifiClient?.profilesRevision ?? 0);
    if (!Number.isInteger(currentRevision) || currentRevision < 1 || currentRevision >= 0xffffffff) {
        return { applied: false, pending: true, reason: 'device_profile_revision_unavailable' };
    }
    if (rows.length > 4) {
        return { applied: false, pending: true, reason: 'device_profile_capacity_exceeded' };
    }
    ...
    const response = await runQueuedDeviceOperation(deviceId, () => publishModemRuntimeCommand(
        deviceId,
        'wifi_profiles_apply',
        { revision, profiles },
        true,
    ...
    const completed = response?.success === true &&
        String(response?.result || response?.status || '').toLowerCase() === 'completed' &&
        response?.payload?.applied === true && response.payload.revision === revision &&
        response.payload.count === profiles.length;
    return {
        applied: completed,
        pending: !completed,
        revision,
        reason: completed ? null : 'device_profile_apply_not_confirmed'
    };
}
...
function buildStateFromModemService(deviceId) {
    ...
    const modemSession = sim.dataSession
        ?? status.mobile?.dataSession
        ?? rawDevice.status?.modem_data_session_open
        ?? rawDevice.status?.sim?.dataSession
        ?? rawDevice.status?.modem?.dataSession;
    const modemBearerReady = sim.ipBearer
        ?? status.mobile?.ipBearerReady
    ...
    const modemDataIp = String(
        sim.dataIp
        || status.mobile?.dataIp
        || rawDevice.status?.modem_data_ip
        || rawDevice.status?.modem_ip_address
        || rawDevice.status?.sim?.dataIp
        || rawDevice.status?.modem?.dataIp
        || rawDevice.mobile?.dataIp
        || (activePath === 'modem' ? status.ip : '')
        || ''
    ).trim();
    const modemPdpIp = String(
        sim.pdpIp
        || rawDevice.status?.modem_pdp_ip_address
        || rawDevice.status?.sim?.pdpIp
        || rawDevice.status?.modem?.pdpIpAddress
        || rawDevice.mobile?.pdpIpAddress
        || ''
    ).trim();
    ...
    const mobileConnected = Boolean(status.online && resolveModemConnectedContract({
        session: modemSession,
        bearer: modemBearerReady,
        dataIp: modemDataIp,
        activePath,
        legacyConnected: status.mobile?.connected
    }));
    ...
    state.mobile.ipAddress = modemDataIp || modemPdpIp;
    state.mobile.dataIp = modemDataIp;
    state.mobile.pdpIpAddress = modemPdpIp;
    ...
    state.routing.connectionPolicy = normalizeConnectionPolicy(
        rawDevice.status?.connection_policy,
        state.routing.connectionPolicy
    );
    state.routing.developmentMode = state.routing.connectionPolicy === 'dual_active_development';
    state.routing.concurrentLinksReady = Boolean(rawDevice.status?.concurrent_links_ready);
...
function updateStateFromDevice(deviceId, data) {
    ...
    const modemDataIp = String(
        ...
        || (!modemPdpIp ? data.mobile?.ipAddress : '')
        || ''
    ).trim();
    ...
    for (const [wire, field] of [
        ...
        if (data[wire] !== undefined) state.wifiClient[field] = data[wire];
    }
    ...
    const hasExplicitBearerState = data.modem_ip_bearer_ready !== undefined
        || data.modem_data_session_open !== undefined
        || data.mobile?.ipBearerReady !== undefined
        || data.mobile?.dataSession !== undefined;
    state.mobile.connected = Boolean(
        state.mobile.enabled
        && resolveModemConnectedContract({
            session: modemSession,
            bearer: hasExplicitBearerState ? modemBearerReady : undefined,
            dataIp: modemDataIp,
            activePath,
            legacyConnected: data.mobile?.connected
        })
    );
    state.mobile.ipAddress = modemDataIp || modemPdpIp;
    state.mobile.dataIp = modemDataIp;
    ...
    state.routing.preferModemData = state.wifiClient.reconnectSuppressed;
    state.routing.connectionPolicy = normalizeConnectionPolicy(
        data.connection_policy,
        state.routing.connectionPolicy
    );
    state.routing.developmentMode = data.connection_development_mode
        ?? data.developmentMode
        ?? (state.routing.connectionPolicy === 'dual_active_development');
    state.routing.concurrentLinksReady = Boolean(
        data.concurrent_links_ready
        ?? data.concurrentLinksReady
        ?? (state.wifiClient.connected && state.mobile.connected)
    );
...
router.get('/status', requireDeviceAccess(), async (req, res) => {
    try {
        ...
        const deviceStatus = global.modemService?.getStatus?.(deviceId);
        const deviceOnline = deviceStatus?.online === true;
        const networkTelemetryFresh = deviceOnline && deviceStatus?.statusFresh === true;
        const activeDevicePath = normalizeModemActivePath(deviceStatus?.activePath);
        const selectedBearerReady = (activeDevicePath === 'wifi' && state.wifiClient.connected)
            || (activeDevicePath === 'modem' && state.mobile.connected)
            || (activeDevicePath === 'usb' && state.usb.connected);
        const commandReady = networkTelemetryFresh
            && selectedBearerReady
            && global.mqttService?.connected === true
            && deviceStatus?.mqtt?.connected === true
            && deviceStatus?.mqtt?.subscribed === true;
        const internetAvailable = commandReady;

        res.json({
            success: true,
            data: {
                deviceOnline,
                networkTelemetryFresh,
                commandReady,
                capabilities: runtimeCapabilities,
                internet: {
                    available: internetAvailable,
                    activeSource: networkTelemetryFresh ? state.routing.primarySource : 'none',
                    sources: {
                        mobile: networkTelemetryFresh && state.mobile.connected,
                        wifi: networkTelemetryFresh && state.wifiClient.connected,
                        usb: networkTelemetryFresh && state.usb.connected
                    }
                },
                sharing: {
 ...
 */
router.get('/mobile/status', requireDeviceAccess(), async (req, res) => {
    try {
 ...
 */
router.post('/mobile/toggle', requireDeviceAccess(undefined, true), [
 ...
 */
router.post('/mobile/apn', requireDeviceAccess(undefined, true), [
 ...
 */
router.get('/wifi/client/scan', requireDeviceAccess(), async (req, res) => {
    const deviceId = resolveRequestDeviceId(req);
 ...
 */
router.post('/wifi/client/connect', requireDeviceAccess(undefined, true), [
 ...
 */
/**
 * Update or forget a saved Wi-Fi profile. Profiles are dashboard-owned records;
 * changing one does not claim the device connected until a fresh status proves it.
 */
router.post('/wifi/client/profiles', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const deviceId = resolveRequestDeviceId(req);
        const { ssid, security, password } = req.body;
        const priority = req.body.priority === undefined ? 50 : req.body.priority;
        if (typeof ssid !== 'string' || !ssid.trim() || Buffer.byteLength(ssid, 'utf8') > 32
            || !['WPA2-PSK', 'WPA3', 'WPA/WPA2-PSK', 'open'].includes(security)
            || (password !== undefined && (typeof password !== 'string' || Buffer.byteLength(password, 'utf8') > 64))
            || (!Number.isInteger(priority) || priority < 1 || priority > 999)) {
            return res.status(400).json({ success: false, message: 'Invalid Wi-Fi profile' });
        }
        const existing = await readStoredWifiNetwork(req, deviceId, ssid);
        const secret = security === 'open' ? '' : (password || existing?.password || '');
        if (security !== 'open' && !(Buffer.byteLength(secret, 'utf8') >= 8 && Buffer.byteLength(secret, 'utf8') <= 63) && !/^[a-fA-F0-9]{64}$/.test(secret)) {
            return res.status(400).json({ success: false, message: 'Enter an 8–63 byte password or a 64 digit hexadecimal key' });
        }
        if (!req.app?.locals?.db?.run) return res.status(503).json({ success: false, message: 'Profile storage unavailable' });
        await persistKnownWifiNetwork(req, deviceId, { ssid, security, password: secret, selected: false, connected: false });
        await req.app.locals.db.run(
            'UPDATE device_wifi_networks SET priority = ?, enabled = 1, updated_at = CURRENT_TIMESTAMP WHERE device_id = ? AND ssid = ?',
            [priority, deviceId, normalizeSsid(ssid)]
        );
        let deviceSync = { applied: false, pending: true, reason: 'not_attempted' };
        try { deviceSync = await applyStoredWifiProfilesToDevice(req, deviceId); } catch (syncError) {
            logger.warn('Wi-Fi profile device sync pending');
            deviceSync = { applied: false, pending: true, reason: 'device_profile_sync_failed' };
        }
        return res.json({
            success: true,
            message: deviceSync.applied ? 'Saved and applied to the device.' : 'Saved in dashboard; device profile sync is pending.',
            data: { networks: await listStoredWifiNetworks(req, deviceId), deviceSync }
        });
    } catch (error) {
        logger.error('Wi-Fi profile save failed');
        return res.status(500).json({ success: false, message: 'Failed to save Wi-Fi profile' });
    }
});

router.patch('/wifi/client/profiles/:ssid', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const deviceId = resolveRequestDeviceId(req);
        const ssid = normalizeSsid(req.params.ssid);
        const db = req.app?.locals?.db;
        if (!ssid || !db?.run) return res.status(400).json({ success: false, message: 'SSID is required' });
        const updates = [];
        const values = [];
        ...
        if (req.body?.priority !== undefined) {
            ...
            if (!Number.isInteger(priority) || priority < 1 || priority > 999) return res.status(400).json({ success: false, message: 'Priority must be an integer from 1 to 999' });
            updates.push('priority = ?'); values.push(priority);
        }
        if (!updates.length) return res.status(400).json({ success: false, message: 'No profile changes supplied' });
        values.push(deviceId, ssid);
        const result = await db.run(`UPDATE device_wifi_networks SET ${updates.join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE device_id = ? AND ssid = ?`, values);
        if (!result?.changes) return res.status(404).json({ success: false, message: 'Saved Wi-Fi profile not found' });
        let deviceSync = { applied: false, pending: true, reason: 'not_attempted' };
        try { deviceSync = await applyStoredWifiProfilesToDevice(req, deviceId); } catch (_error) {
            deviceSync = { applied: false, pending: true, reason: 'device_profile_sync_failed' };
        }
        return res.json({
            success: true,
            message: deviceSync.applied ? 'Profile updated and applied to the device.' : 'Profile updated; device sync is pending.',
            data: { networks: await listStoredWifiNetworks(req, deviceId), deviceSync }
        });
    } catch (error) {
        logger.error('Wi-Fi profile update failed:', error);
        return res.status(500).json({ success: false, message: 'Failed to update Wi-Fi profile' });
    }
});

router.delete('/wifi/client/profiles/:ssid', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const deviceId = resolveRequestDeviceId(req);
        const ssid = normalizeSsid(req.params.ssid);
        const db = req.app?.locals?.db;
        if (!ssid || !db?.run) return res.status(400).json({ success: false, message: 'SSID is required' });
        if (isWifiConnectObserved(getLatestDeviceStatusSnapshot(deviceId), ssid)) {
            return res.status(409).json({ success: false, message: 'Disconnect this network before forgetting it. Device credentials are unchanged.' });
        }
        const result = await db.run('DELETE FROM device_wifi_networks WHERE device_id = ? AND ssid = ?', [deviceId, ssid]);
        if (!result?.changes) return res.status(404).json({ success: false, message: 'Saved Wi-Fi profile not found' });
        let deviceSync = { applied: false, pending: true, reason: 'not_attempted' };
        try { deviceSync = await applyStoredWifiProfilesToDevice(req, deviceId); } catch (_error) {
            deviceSync = { applied: false, pending: true, reason: 'device_profile_sync_failed' };
        ...
        return res.json({
            success: true,
            message: deviceSync.applied ? 'Network forgotten and applied to the device.' : 'Network forgotten; device sync is pending.',
            data: { networks: await listStoredWifiNetworks(req, deviceId), deviceSync }
        });
    } catch (error) {
        logger.error('Wi-Fi profile delete failed:', error);
        return res.status(500).json({ success: false, message: 'Failed to forget Wi-Fi profile' });
    }
});

router.post('/wifi/client/retry', requireDeviceAccess(undefined, true), [
 ...
 */
router.post('/wifi/client/toggle', requireDeviceAccess(undefined, true), [
    body('enabled').isBoolean(),
    body('deviceId').optional()
], async (req, res) => {
    try {
 ...
 */
router.post('/wifi/client/disconnect', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const deviceId = resolveRequestDeviceId(req);
...
router.post('/routing/prefer-modem', requireDeviceAccess(undefined, true), [
    body('enabled').isBoolean(),
    body('deviceId').optional()
], async (req, res) => {
    try {
 ...
 */
router.post('/wifi/hotspot/toggle', requireDeviceAccess(undefined, true), [
    body('enabled').isBoolean()
], async (req, res) => {
    try {
        const errors = validationResult(req);
 ...
 */
router.post('/wifi/hotspot/configure', requireDeviceAccess(undefined, true), [
 ...
 */
router.get('/wifi/hotspot/clients', requireDeviceAccess(), async (req, res) => {
    try {
        const deviceId = resolveRequestDeviceId(req);
        if (
 ...
 */
router.post('/wifi/hotspot/clients/block', requireDeviceAccess(undefined, true), [
    body('mac').notEmpty()
], async (req, res) => {
    try {
        const { mac } = req.body;
 ...
 */
router.post('/wifi/hotspot/clients/limit', requireDeviceAccess(undefined, true), [
    body('mac').notEmpty(),
 ...
 */
router.post('/usb/toggle', requireDeviceAccess(undefined, true), [
    body('enabled').isBoolean()
], async (req, res) => {
    try {
        const errors = validationResult(req);
 ...
 */
router.get('/data-usage', requireDeviceAccess(), async (req, res) => {
    try {
        const deviceId = resolveRequestDeviceId(req);
        buildStateFromModemService(deviceId);
        await hydrateStateFromInternetStatus(deviceId, 5000);
 ...
 */
router.post('/data-usage/reset', requireDeviceAccess(undefined, true), async (req, res) => {
    try {
        const deviceId = resolveRequestDeviceId(req);
 ...
 */
router.post('/routing/configure', requireDeviceAccess(undefined, true), [
    body('connectionPolicy').optional().isIn(Array.from(CONNECTION_POLICIES)),
    ...
    try {
        ...
        const { connectionPolicy, failover, loadBalancing, nat, firewall } = req.body;
        const deviceId = resolveRequestDeviceId(req);
        ...
        const normalizedPolicy = connectionPolicy
            ? normalizeConnectionPolicy(connectionPolicy, '')
            : '';
        if (normalizedPolicy &&
            (normalizedPolicy === 'cellular_only' || normalizedPolicy === 'cellular_wifi_fallback') &&
            !runtimeCapabilities.routing.secureCellularMqtt) {
            return res.status(409).json({
                success: false,
                message: 'Cellular-primary control requires the public TLS MQTT broker and verified modem TLS support',
                data: {
                    connectionPolicy: normalizedPolicy,
                    code: 'SECURE_CELLULAR_MQTT_UNAVAILABLE'
                }
            });
        }
        ...
        const config = {};
        if (normalizedPolicy) {
            config.connection_policy = normalizedPolicy;
        }
        ...
        if (response && response.success) {
            ...
            state.routing = {
                ...state.routing,
                connectionPolicy: normalizeConnectionPolicy(
                    payload.connection_policy,
                    normalizedPolicy || state.routing.connectionPolicy
                ),
                developmentMode: payload.development_mode === true,
                concurrentLinksReady: payload.modem_bearer_ready === true && payload.wifi_connected === true,
                activationPending: payload.activation_pending === true,
            ...
            const policyData = normalizedPolicy ? {
                connectionPolicy: state.routing.connectionPolicy,
                developmentMode: state.routing.developmentMode,
                concurrentLinksReady: state.routing.concurrentLinksReady,
                activationPending: state.routing.activationPending
            } : {};
            res.json({
                success: true,
                message: normalizedPolicy
                    ? (state.routing.activationPending
                    ? 'Connection policy saved; the device is still bringing one or more links online'
                    : 'Connection policy applied')
                    : 'Routing failover configured',
                data: {
                    ...policyData,
 ...
 */
router.get('/signal', requireDeviceAccess(), async (req, res) => {
    try {
        const deviceId = resolveDeviceId(req, DEFAULT_DEVICE_ID);
