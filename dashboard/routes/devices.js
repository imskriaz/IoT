...
async function persistSessionDeviceId(req, deviceId) {
    ...
    if (!normalizedDeviceId || !req?.session) {
...
async function buildStoredSimSnapshot(db, deviceId, live = {}) {
    ...
    return {
        ...
        simNumberSource: normalizedStatus.simNumberSource || null,
        subscriberNumberSource: normalizedStatus.subscriberNumberSource || null,
...
router.post('/unregistered/:deviceId/assign', [
    ...
    try {
        ...
        const device = await registerDevice(db, {
            ...
            type,
            board: req.body.board || identity.board || identity.model || null,
    ...
    try {
        ...
        const row = await db.get(
            `SELECT d.id, d.name, d.type, d.status, d.last_seen, d.created_at, d.description,
                    ...
                    dp.capabilities, dp.firmware_version, dp.board, dp.probed_at, dp.updated_at,
                    dp.has_gps, dp.has_battery, dp.has_sd, dp.has_camera, dp.has_audio,
                    dp.has_display, dp.has_nfc, dp.has_rfid, dp.has_touch, dp.has_keyboard
             FROM devices d
             LEFT JOIN device_profiles dp ON dp.device_id = d.id
...
router.put('/:id', [
    body('name').optional().trim().notEmpty().withMessage('Name cannot be empty').isLength({ max: 100 })
        .custom(value => /^[\u0009\u000A\u000D\u0020-\u007E\u00A0-\uFFFF]*$/.test(value))
        .withMessage('Name contains unsupported control characters'),
    body('location').optional({ nullable: true }).trim().isLength({ max: 200 }),
    body('apn').optional({ nullable: true }).trim().isLength({ max: 100 }),
    ...
    try {
        ...
        // Broadcast only whitelisted, sanitized fields — never the raw request
        // body (SEC-06: req.body previously flowed unfiltered to all sockets).
        if (global.io) global.io.emit('device:updated', {
            id: req.params.id,
            ...(name !== undefined ? { name: String(name).slice(0, 100) } : {}),
            ...(location !== undefined ? { location: String(location || '').slice(0, 200) } : {}),
            ...(apn !== undefined ? { apn: String(apn || '').slice(0, 100) } : {}),
            ...(wifi_ssid !== undefined ? { wifi_ssid: String(wifi_ssid || '').slice(0, 64) } : {}),
            ...(mqtt_host !== undefined ? { mqtt_host: String(mqtt_host || '').slice(0, 200) } : {})
        });
 ...
 *         description: Saved
 */
router.get('/:id/local-ip', requireDeviceAccess('id'), async (req, res) => {
    try {
 ...
 */
router.get('/:id/direct', requireDeviceAccess('id'), async (req, res) => {
    try {
        const row = await req.app.locals.db.get(
            `SELECT local_ip FROM device_profiles WHERE device_id = ?`,
            [req.params.id]
...
router.get('/:id/capabilities', requireDeviceAccess('id'), async (req, res) => {
    try {
        const db = req.app.locals.db;
        const row = await db.get(
...
router.put('/:id/capabilities', requireDeviceAccess('id', true), async (req, res) => {
    try {
        const db = req.app.locals.db;
        const deviceId = req.params.id;
