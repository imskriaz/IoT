...
const { authorizeDeviceAccess, requireDeviceAccess, filterAuthorizedDevices } = require('../middleware/auth');
...
async function requestFreshStatus(deviceId, timeoutMs = 8000, ignoreCooldown = false) {
    ...
    const refreshPromise = (async () => {
        ...
        // An unsolicited snapshot can arrive while this command is pending.
        // Only the snapshot produced by this action settles the live refresh.
        const eventPromise = waitForMqttEvent('status', deviceId, timeoutMs, data => (
            data?.messageId === messageId || data?.action_id === messageId
        ));
...
router.get('/', async (req, res) => {
    try {
        ...
        if (!deviceId) {
            await filterAuthorizedDevices(req, []);
            return res.json({
                success: true,
        ...
        // Check access before reading caches/queues or publishing a refresh.
        ...
        res.status(error.statusCode || 500).json({
            success: false,
            message: error.statusCode ? error.message : 'Failed to fetch device status'
        });
    }
});

async function authorizeResolvedStatusDevice(req, res, next) {
    try {
        const deviceId = resolveRequestDeviceId(req);
        if (!deviceId) return res.status(400).json({ success: false, message: 'No active device selected' });
        req.authorizedDeviceId = await authorizeDeviceAccess(req, deviceId, { write: true });
        return next();
    } catch (error) {
        return res.status(error.statusCode || 500).json({ success: false, message: error.message || 'Device access denied' });
    }
}

router.post('/module-action', authorizeResolvedStatusDevice, async (req, res) => {
    try {
...
router.get('/devices', async (req, res) => {
    try {
        const devices = await filterAuthorizedDevices(req, modemService.getAllDevices());
        res.json({
            success: true,
            data: devices
        });
        ...
        res.status(error.statusCode || 500).json({
            success: false,
            message: error.statusCode ? error.message : 'Failed to fetch devices'
        });
    }
});

// Get device history
router.get('/history/:deviceId', requireDeviceAccess('deviceId'), (req, res) => {
    try {
