...
(function () {
    ...
    function supportsAndroidCallControls() {
        const caps = getCallsActiveCapabilities();
        const identity = [caps.bridge, caps.platform, caps.device_type, caps.deviceType, caps.board]
            .map((value) => String(value || '').toLowerCase()).join(' ');
        return identity.includes('android');
    }

    function syncCallCapabilityUi() {
        const androidControls = supportsAndroidCallControls();
        document.querySelectorAll('[data-call-capability="android"]').forEach((element) => {
            element.classList.toggle('d-none', !androidControls);
            if ('disabled' in element) element.disabled = !androidControls;
            element.setAttribute('aria-disabled', String(!androidControls));
        });
    }
    ...
    window.muteCall = function () {
        showToast('Call mute is not supported by the active device contract', 'warning');
        return;
    ...
    window.holdCall = function () {
        showToast('Call hold is not supported by the active device contract', 'warning');
        return;
