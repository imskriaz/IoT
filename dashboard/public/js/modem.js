...
(function () {
    ...
    const elements = {
        ...
        connectionPolicySelect: document.getElementById('connectionPolicySelect'),
        applyConnectionPolicyBtn: document.getElementById('applyConnectionPolicyBtn'),
        connectionPolicyDescription: document.getElementById('connectionPolicyDescription'),
        connectionPolicyHint: document.getElementById('connectionPolicyHint'),
        connectionPolicyStage: document.getElementById('connectionPolicyStage'),
        policyWifiState: document.getElementById('policyWifiState'),
        policyCellularState: document.getElementById('policyCellularState'),
    ...
    function attachDisclosureListeners() {
        document.querySelectorAll('.win-settings-disclosure').forEach((details) => {
            details.addEventListener('toggle', () => {
                if (!details.open) return;
                window.requestAnimationFrame(() => {
                    signalCharts.mobile?.resize?.();
                    signalCharts.wifi?.resize?.();
                    usageCharts.history?.resize?.();
                });
            });
        });
    }
    ...
    function failClosedOnStatusRefresh(message = 'Status Unavailable') {
        isDeviceOnline = false;
        currentStatus = null;
        lastUsageSnapshot = null;
        showOfflineUI(message);
        return null;
    }
    ...
    function loadStatus(force = true) {
                ...
                return Promise.resolve(window.resolveActiveDeviceId(false))
                    .then((resolvedDeviceId) => {
                        ...
                        if (!normalizedDeviceId) {
                            return failClosedOnStatusRefresh('No Device Selected');
                        }
                        deviceId = normalizedDeviceId;
                        return loadStatus(force);
                    })
                    .catch(() => {
                        return failClosedOnStatusRefresh('Status Unavailable');
                    });
            }

            return Promise.resolve(failClosedOnStatusRefresh('No Device Selected'));
        }
        ...
        let requestPromise;
        requestPromise = fetch(`/api/modem/status?deviceId=${encodeURIComponent(activeDeviceId)}&_ts=${Date.now()}`, {
            cache: 'no-store',
            headers: {
            ...
            .then(data => {
                if (requestGeneration !== statusRequestGeneration || syncActiveDeviceId() !== activeDeviceId) {
                    return null;
                }
                if (data.success) {
                    currentStatus = data.data;
                    runtimeCapabilities = data.data?.capabilities && typeof data.data.capabilities === 'object'
                        ? data.data.capabilities : {};
                    isDeviceOnline = data.data?.commandReady === true;
                    updateUI(data.data);
                    if (data.data?.networkTelemetryFresh !== true) {
                        showOfflineUI(data.data?.deviceOnline === true ? 'Status Stale' : 'Device Offline');
                    } else if (!isDeviceOnline && elements.internetDetails) {
                        elements.internetDetails.textContent += ' · Device commands unavailable';
                    }
            ...
            .finally(() => {
                if (statusRefreshPromise === requestPromise) statusRefreshPromise = null;
            });

        statusRefreshPromise = requestPromise;
        return requestPromise;
    }

    function showOfflineUI(message = 'Device Offline') {
        lastWifiScanResults = null;
        lastWifiScanDeviceId = '';
        resetModemCharts();
        ...
        if (elements.internetStatus) elements.internetStatus.textContent = message;
        if (elements.internetDetails) elements.internetDetails.textContent = 'Live device status is not available';
        if (elements.activeSource) elements.activeSource.textContent = 'Offline';
        if (elements.policyWifiState) elements.policyWifiState.textContent = 'Not reported';
        if (elements.policyCellularState) elements.policyCellularState.textContent = 'Not reported';
        ...
        if (elements.mobileStatus) elements.mobileStatus.textContent = 'Not reported';
        if (elements.mobileNetwork) elements.mobileNetwork.textContent = 'Not reported';
        if (elements.mobileSignal) elements.mobileSignal.textContent = 'Not reported';
        if (elements.mobileSignalBar) elements.mobileSignalBar.style.width = '';
        if (elements.mobileIP) elements.mobileIP.textContent = 'Not reported';
        if (elements.mobileDataUsed) elements.mobileDataUsed.textContent = 'Not reported';
        ...
        if (elements.wifiClientBadge) {
            elements.wifiClientBadge.textContent = 'Not reported';
            elements.wifiClientBadge.className = 'badge bg-secondary';
        }
        if (elements.wifiClientIcon) elements.wifiClientIcon.className = 'bi bi-wifi text-secondary';
        if (elements.wifiClientSSID) elements.wifiClientSSID.textContent = 'Not reported';
        if (elements.wifiClientStatus) {
            elements.wifiClientStatus.textContent = 'Not reported';
            elements.wifiClientStatus.className = 'badge bg-secondary';
        }
        if (elements.wifiClientSignal) elements.wifiClientSignal.textContent = 'Not reported';
        if (elements.wifiClientSignalBar) elements.wifiClientSignalBar.style.width = '';
        if (elements.wifiClientIP) elements.wifiClientIP.textContent = 'Not reported';
        if (elements.wifiClientSecurity) elements.wifiClientSecurity.textContent = 'Not reported';
        if (elements.wifiClientChannel) elements.wifiClientChannel.textContent = 'Not reported';
        if (elements.wifiClientBSSID) elements.wifiClientBSSID.textContent = 'Not reported';
        if (elements.wifiClientDiagnostics) elements.wifiClientDiagnostics.textContent = 'Not reported';
        if (elements.wifiNetworksSummary) elements.wifiNetworksSummary.textContent = 'Scan unavailable';
        if (elements.hotspotIcon) elements.hotspotIcon.className = 'bi bi-broadcast text-secondary';
        if (elements.hotspotSSID) elements.hotspotSSID.textContent = 'Not reported';
        if (elements.hotspotClientsCount) elements.hotspotClientsCount.textContent = 'Not reported';
        if (elements.clientCount) elements.clientCount.textContent = 'Not reported';
        if (elements.clientsList) elements.clientsList.textContent = 'Not reported';
        if (elements.usbStatus) elements.usbStatus.textContent = 'Not reported';
        if (elements.usbIcon) elements.usbIcon.className = 'bi bi-usb-symbol text-secondary';
        if (elements.usbDetails) elements.usbDetails.textContent = 'Live device status is not available';
        if (elements.dataUsageNotice) elements.dataUsageNotice.textContent = 'Traffic counters are not reported while status is unavailable.';
        for (const metric of [
            elements.totalSent, elements.totalReceived, elements.totalUsage,
            elements.mobileSent, elements.mobileReceived, elements.wifiSent, elements.wifiReceived
        ]) {
            if (metric) metric.textContent = 'Not reported';
        }
        updateWifiRuntimeCapability();

        for (const control of [
            elements.connectionPolicySelect, elements.applyConnectionPolicyBtn,
            elements.mobileToggle, elements.detectSimNumberBtn,
            elements.wifiToggle, elements.retrySavedWiFiBtn,
            elements.disconnectWiFiBtn, elements.scanWiFiBtn,
            elements.hotspotToggle, elements.refreshHotspotClientsBtn,
            elements.usbToggle, elements.dataUsageResetBtn,
            elements.apnSaveBtn, elements.hotspotSaveBtn
        ]) {
            if (control) control.disabled = true;
        }
        for (const toggle of [elements.mobileToggle, elements.wifiToggle, elements.hotspotToggle, elements.usbToggle]) {
    ...
    function updateUI(status) {
        ...
        updateConnectionPolicyUI(status);
    ...
    const connectionPolicyPresentation = Object.freeze({
        cellular_only: {
            label: 'Cellular only',
            description: 'Use the cellular MQTT route exclusively. Wi-Fi remains available only for recovery.',
            development: false
        },
        cellular_wifi_fallback: {
            label: 'Cellular · Wi-Fi fallback',
            description: 'Prefer cellular and move to Wi-Fi only when the cellular command route is unavailable.',
            development: false
        },
        wifi_only: {
            label: 'Wi-Fi only',
            description: 'Use Wi-Fi for dashboard commands and do not promote the modem as an MQTT route.',
            development: false
        },
        wifi_cellular_fallback: {
            label: 'Wi-Fi · Cellular fallback',
            description: 'Prefer Wi-Fi and promote cellular only after the Wi-Fi command route fails.',
            development: false
        },
        dual_active_development: {
            label: 'Wi-Fi + Cellular together',
            description: 'Keep Wi-Fi and the cellular bearer ready together while one MQTT lane remains authoritative.',
            development: true
        }
    });

    function updateConnectionPolicyUI(status) {
        const routing = status?.routing || {};
        const policy = connectionPolicyPresentation[routing.connectionPolicy]
            ? routing.connectionPolicy
            : 'dual_active_development';
        const presentation = connectionPolicyPresentation[policy];
        const secureCellularMqtt = runtimeCapabilities?.routing?.secureCellularMqtt === true;

        if (elements.connectionPolicySelect) {
            elements.connectionPolicySelect.value = policy;
            Array.from(elements.connectionPolicySelect.options).forEach((option) => {
                const cellularPrimary = option.value === 'cellular_only'
                    || option.value === 'cellular_wifi_fallback';
                option.disabled = cellularPrimary && !secureCellularMqtt;
                option.title = option.disabled
                    ? 'Requires a verified public TLS MQTT path through the modem'
                    : '';
            });
            elements.connectionPolicySelect.disabled = !isDeviceOnline
                || runtimeCapabilities?.routing?.remoteConfigure === false;
        }
        if (elements.applyConnectionPolicyBtn) {
            elements.applyConnectionPolicyBtn.disabled = !isDeviceOnline
                || runtimeCapabilities?.routing?.remoteConfigure === false;
        }
        if (elements.connectionPolicyDescription) {
            elements.connectionPolicyDescription.textContent = presentation.description;
        }
        if (elements.connectionPolicyStage) {
            elements.connectionPolicyStage.classList.toggle('d-none', !presentation.development);
        }
        if (elements.connectionPolicyHint) {
            elements.connectionPolicyHint.textContent = secureCellularMqtt
                ? 'The device will preserve command settlement while changing the preferred route.'
                : 'Cellular-primary choices unlock after secure modem MQTT is configured and verified.';
        }

        const wifiReady = Boolean(status?.wifiClient?.connected);
        const cellularReady = Boolean(status?.mobile?.connected);
        if (elements.policyWifiState) {
            elements.policyWifiState.classList.toggle('is-ready', wifiReady);
            elements.policyWifiState.innerHTML = `<i class="bi bi-wifi"></i> Wi-Fi ${wifiReady ? 'ready' : 'standby'}`;
        }
        if (elements.policyCellularState) {
            elements.policyCellularState.classList.toggle('is-ready', cellularReady);
            elements.policyCellularState.innerHTML = `<i class="bi bi-broadcast"></i> Cellular ${cellularReady ? 'ready' : 'standby'}`;
        }
    }

    async function applyConnectionPolicy() {
        const policy = String(elements.connectionPolicySelect?.value || '').trim();
        if (!connectionPolicyPresentation[policy]) {
            showToast('Choose a valid connection policy', 'warning');
            return;
        }
        if (!isDeviceOnline) {
        ...
        const button = elements.applyConnectionPolicyBtn;
        const select = elements.connectionPolicySelect;
        if (button) button.disabled = true;
        if (select) select.disabled = true;
        try {
            const response = await fetch('/api/modem/routing/configure', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ deviceId, connectionPolicy: policy })
            });
            const data = await response.json();
            if (!response.ok || !data.success) {
                throw new Error(data.message || 'Failed to apply connection policy');
            }
            showToast(data.message, data.data?.activationPending ? 'warning' : 'success');
            await loadStatus(true);
        } catch (error) {
            showToast(error.message || 'Failed to apply connection policy', 'danger');
            updateConnectionPolicyUI(currentStatus);
        } finally {
            if (button) button.disabled = !isDeviceOnline;
            if (select) select.disabled = !isDeviceOnline;
        }
    }

    function getWifiRuntimeMessage() {
        const wifiCaps = runtimeCapabilities?.wifiClient || {};
        if (wifiCaps.remoteConnect) {
            return 'The device can connect to a selected saved network. Automatic roaming across all saved networks requires device profile sync.';
        }
        return 'This firmware can report Wi-Fi state, but does not expose remote credential switching.';
    }
    ...
    function updateWifiRuntimeCapability() {
        ...
        if (elements.disconnectWiFiBtn) {
            const wifiConnected = currentStatus?.networkTelemetryFresh === true
                && currentStatus?.wifiClient?.connected === true;
            elements.disconnectWiFiBtn.style.display = wifiConnected && canRemoteDisconnect ? 'block' : 'none';
            elements.disconnectWiFiBtn.disabled = !isDeviceOnline;
        }

        if (elements.scanWiFiBtn) {
            elements.scanWiFiBtn.disabled = !isDeviceOnline || wifiCaps.scan === false;
        }
    ...
    function getWifiDisplayEntries(scannedNetworks = null) {
        ...
        const connected = currentStatus?.networkTelemetryFresh === true
            && currentStatus?.wifiClient?.connected === true;
        ...
        entries.sort((left, right) => {
            // Known-network management honors the stored dashboard order,
            // independent of current signal, visibility, or association.
            if (wifiNetworkView === 'saved' && left.saved && right.saved) {
                const priorityDelta = Number(left.knownNetwork?.priority ?? 50)
                    - Number(right.knownNetwork?.priority ?? 50);
    ...
    function updateWifiNetworksSummary(scannedNetworks) {
        ...
        if (!isDeviceOnline) {
            elements.wifiNetworksSummary.textContent = savedCount > 0
                ? 'Device scan unavailable. Saved networks can still be managed in the dashboard.'
    ...
    let wifiNetworkView = 'all';

    function setWifiNetworkView(view) {
        wifiNetworkView = view === 'saved' ? 'saved' : 'all';
        document.querySelectorAll('[data-wifi-view]').forEach(button => {
            const selected = button.dataset.wifiView === wifiNetworkView;
            button.setAttribute('aria-pressed', String(selected));
            button.classList.toggle('active', selected);
        });
        renderAvailableWifiNetworks();
    }
    ...
    function renderAvailableWifiNetworks(scannedNetworks = null) {
        ...
        const entries = getWifiDisplayEntries(lastWifiScanResults)
            .filter(entry => wifiNetworkView !== 'saved' || entry.saved);
        ...
        if (!entries.length) {
            list.innerHTML = `
                <div class="text-center py-5">
                    ...
                    <p class="text-muted">${wifiNetworkView === 'saved' ? 'No saved networks for this device. Use Add network to save one.' : 'Click Scan to see available networks'}</p>
                    <button class="btn btn-primary" onclick="scanWiFiNetworks()" ${isDeviceOnline ? '' : 'disabled'}>
        ...
        list.innerHTML = entries.map((entry) => {
            ...
            const canDirectConnect = isDeviceOnline && canRemoteConnect && (!entry.encrypted || savedPasswordAvailable || openNetwork);
            const needsPassword = isDeviceOnline && canRemoteConnect && entry.visible && entry.encrypted && !savedPasswordAvailable && !openNetwork;
            const statusBadges = [];
            const metaBits = [];
            const footerBits = [];
            ...
            if (entry.connected) {
                ...
                primaryAction = isDeviceOnline
                    ? '<button type="button" class="btn btn-sm btn-outline-secondary" disabled>Password missing</button>'
                    : '<button type="button" class="btn btn-sm btn-outline-secondary" disabled>Device unavailable</button>';
            }

            const retryAction = isDeviceOnline && matchesSavedProfile && canRetrySaved
            ...
            const profileActions = entry.saved
                ? `<button type="button" class="btn btn-sm btn-outline-secondary" data-ssid="${escapeHtml(entry.ssid)}" onclick="showConnectModal(this.dataset.ssid, true)">Properties</button>${entry.connected ? '' : `<button type="button" class="btn btn-sm btn-outline-danger" data-ssid="${escapeHtml(entry.ssid)}" onclick="forgetWifiProfile(this.dataset.ssid)">Forget</button>`}`
                : '';
                ...
                <div class="list-group-item py-3">
                    <div class="d-flex justify-content-between align-items-start gap-3">
                        ...
                        <div class="text-end flex-shrink-0" style="min-width: 140px;">
                            ...
                            `}
                            ${(primaryAction || retryAction || profileActions) ? `<div class="d-flex justify-content-end gap-2 mt-2 flex-wrap">${primaryAction}${retryAction}${profileActions}</div>` : ''}
                        </div>
                    </div>
                </div>
            `;
    ...
    async function updateWifiProfile(ssid, changes) {
        const activeDeviceId = syncActiveDeviceId();
        const response = await fetch(`/api/modem/wifi/client/profiles/${encodeURIComponent(ssid)}?deviceId=${encodeURIComponent(activeDeviceId)}`, {
            method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(changes)
        });
        const data = await response.json().catch(() => ({}));
        if (!response.ok || !data.success) throw new Error(data.message || 'Profile update failed');
        loadStatus(true);
        return data.data?.deviceSync;
    }

    function showWifiProfileSyncResult(message, sync) {
        showToast(sync?.applied === true
            ? `${message}. Saved on device.`
            : `${message} in dashboard. Device update is not confirmed.`,
        sync?.applied === true ? 'success' : 'warning');
    }

    function toggleWifiProfile(ssid, enabled) {
        updateWifiProfile(ssid, { enabled }).then((sync) => showWifiProfileSyncResult(enabled ? 'Wi-Fi profile enabled' : 'Wi-Fi profile disabled', sync)).catch((error) => showToast(error.message, 'danger'));
    }

    function forgetWifiProfile(ssid) {
        if (!window.confirm(`Forget “${ssid}”? The dashboard will request removal from the device's saved networks. Until confirmed, the device may still use it.`)) return;
        const activeDeviceId = syncActiveDeviceId();
        fetch(`/api/modem/wifi/client/profiles/${encodeURIComponent(ssid)}?deviceId=${encodeURIComponent(activeDeviceId)}`, { method: 'DELETE' })
            .then(async response => ({ ok: response.ok, data: await response.json().catch(() => ({})) }))
            .then(({ ok, data }) => { if (!ok || !data.success) throw new Error(data.message || 'Forget failed'); showWifiProfileSyncResult('Wi-Fi profile forgotten', data.data?.deviceSync); loadStatus(true); })
    ...
    function scanWiFiNetworks() {
        ...
        if (!isDeviceOnline) {
            showToast('Device command path is unavailable', 'warning');
            return;
        }
    ...
    function openWiFiConnectModal({ ssid = '', encrypted = true, security = '', manual = false } = {}) {
        ...
        const priorityInput = document.getElementById('wifiProfilePriority');
        if (priorityInput) priorityInput.value = String(knownNetwork?.priority ?? 50);
        if (modalTitle) {
    ...
    async function saveWifiProfile() {
        const activeDeviceId = syncActiveDeviceId();
        if (!activeDeviceId) return showToast('No device selected', 'warning');
        const payload = {
            deviceId: activeDeviceId,
            ssid: document.getElementById('connectSsidDisplay').value,
            security: document.getElementById('connectSecurity').value,
            priority: Number(document.getElementById('wifiProfilePriority')?.value || 50)
        };
        const password = document.getElementById('connectPassword').value;
        if (password) payload.password = password;
        try {
            const response = await fetch('/api/modem/wifi/client/profiles', {
    ...
    function attachEventListeners() {
        if (elements.connectionPolicySelect) {
            elements.connectionPolicySelect.addEventListener('change', () => {
                const policy = elements.connectionPolicySelect.value;
                const presentation = connectionPolicyPresentation[policy];
                if (presentation && elements.connectionPolicyDescription) {
                    elements.connectionPolicyDescription.textContent = presentation.description;
                }
                if (elements.connectionPolicyStage) {
                    elements.connectionPolicyStage.classList.toggle('d-none', !presentation?.development);
        ...
        elements.applyConnectionPolicyBtn?.addEventListener('click', applyConnectionPolicy);
    ...
    function refreshStatus() {
        loadStatus(true)
            .then((data) => {
                if (data?.networkTelemetryFresh === true && data?.commandReady === true) {
                    showToast('Status refreshed', 'success');
                } else if (data?.networkTelemetryFresh === true) {
                    showToast('Status refreshed; device commands are unavailable', 'warning');
                } else if (data) {
                    showToast('Device status is stale', 'warning');
                } else {
    ...
    window.addEventListener('device:changed', (event) => {
        ...
        deviceId = nextDeviceId;
        failClosedOnStatusRefresh('Status Unavailable');
        lastWifiScanResults = null;
        lastWifiScanDeviceId = '';
        resetModemCharts();
        loadStatus(true);
    ...
    window.setWifiNetworkView = setWifiNetworkView;
    ...
    window.toggleWifiProfile = toggleWifiProfile;
    window.forgetWifiProfile = forgetWifiProfile;
