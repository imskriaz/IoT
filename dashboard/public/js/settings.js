...
(function () {
    ...
    window.clearDeviceData = async function () {
        let approved = false;
        if (typeof window.appConfirm === 'function') {
            approved = await window.appConfirm({
        ...
        fetch('/api/settings/clear-device-data', { method: 'POST' })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    showToast(data.message, 'warning');
                    setTimeout(() => window.location.href = '/onboard', 1200);
                } else {
                    showToast(data.message || 'Failed to clear device data', 'danger');
                }
            })
            .catch((error) => {
                console.error('Clear device data error:', error);
                showToast('Failed to clear device data', 'danger');
            });
    };
