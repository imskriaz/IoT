...
(function() {
    ...
    function initFileManager() {
        ...
        loadPreferences();
        applyStorageMutationAvailability();
    }

    function storageMutationAllowed(action) {
        return state.storageInfo?.mutations?.[action] === true;
    }

    function applyStorageMutationAvailability() {
        document.querySelectorAll('[data-storage-mutation]').forEach((element) => {
            const action = element.getAttribute('data-storage-mutation');
            const allowed = storageMutationAllowed(action);
        ...
        const notice = document.getElementById('storageReadOnlyNotice');
        if (notice) {
            const anyMutation = Array.from(document.querySelectorAll('[data-storage-mutation]'))
                .some((element) => storageMutationAllowed(element.getAttribute('data-storage-mutation')));
            notice.classList.toggle('d-none', anyMutation);
        }
    }

    function requireStorageMutation(action) {
        if (storageMutationAllowed(action)) return true;
        showToast(state.storageInfo?.mutations?.reason || 'This storage action is unavailable for the active device', 'warning');
        return false;
    }
    ...
    function initSocket() {
        ...
        state.socket.on('device:status', (status) => {
            const activeDeviceId = window.getActiveDeviceId ? window.getActiveDeviceId() : '';
            const reportedId = status?.deviceId || status?.device_id || status?.id;
            if (activeDeviceId && reportedId !== activeDeviceId) return;
            updatePhysicalSdDisplay(window.SdStatus?.normalize(status), status?.online !== false);
        });
    }
    ...
    window.cutSelected = function() {
        if (!requireStorageMutation('move')) return;
    ...
    window.copySelected = function() {
        if (!requireStorageMutation('copy')) return;
        if (state.selectedItems.size === 0) return;
    ...
    window.paste = async function() {
        if (!requireStorageMutation(state.clipboard.action === 'copy' ? 'copy' : 'move')) return;
    ...
    window.deleteSelected = async function() {
        if (!requireStorageMutation('delete')) return;
        if (state.selectedItems.size === 0) return;
    ...
    window.renameSelected = function() {
        if (!requireStorageMutation('rename')) return;
    ...
    window.editCurrentFile = function() {
        if (!requireStorageMutation('write')) return;
    ...
    window.showNewFolderModal = function() {
        if (!requireStorageMutation('create')) return;
    ...
    async function loadStorageInfo() {
        try {
            ...
            if (data.success) {
                ...
                const element = document.getElementById('physicalSdStatus');
                if (element) element.textContent = 'Storage status unavailable — reconnect and retry';
            }
        } catch (error) {
    ...
    function getExternalStorageLabel(info = state.storageInfo) {
        ...
        const label = String(storage?.type || storage?.cardType || 'storage').trim();
        return label || 'storage';
    }

    function updatePhysicalSdDisplay(card, online) {
        const element = document.getElementById('physicalSdStatus');
        if (element && window.SdStatus) element.textContent = window.SdStatus.describe(card, online);
    }

    function updateStorageDisplay(info) {
        state.storageInfo = info;
        updatePhysicalSdDisplay(info.sdCard, info.online !== false);
        if (info.sd.available) {
            const sdUsed = info.sd.total > 0 ? Math.min(100, Math.max(0, Math.round((info.sd.used / info.sd.total) * 100))) : 0;
    ...
    function updateStorageStats(stats) {
        // Older list replies omit filesystem usage; do not overwrite known info with zero.
        const source = Number(stats?.total) > 0 ? stats : state.storageInfo?.sd;
        const known = Number(source?.total) > 0;
        document.getElementById('freeSpace').textContent = `Free: ${known && Number.isFinite(Number(source.free)) ? formatSize(Number(source.free)) : 'Not reported'}`;
        document.getElementById('totalSpace').textContent = `Total: ${known ? formatSize(Number(source.total)) : 'Not reported'}`;
    }
    ...
    window.switchStorage = function(type) {
        if (type !== 'sd') {
            showToast('Only the SD card is available for device files', 'info');
            return;
        }
    ...
    function updateFormatButtonState() {
        ...
        button.disabled = !(activeIsSd && sdAvailable && storageMutationAllowed('format'));
    ...
    window.formatStorage = async function() {
        if (!requireStorageMutation('format')) return;
        const storageLabel = getExternalStorageLabel();
    ...
    window.compressSelected = async function() {
        if (!requireStorageMutation('compress')) return;
        if (state.selectedItems.size === 0) return;
