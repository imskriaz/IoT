...
function deviceWifiConnected() {
    if (!deviceOnline() || latestDeviceStatus?.wifi?.connected === false || latestDeviceStatus?.wifi_connected === false) {
        return false;
    }
...
function inferStatusOnline(status) {
    ...
    if (status.online === false || status.staleReason === 'no_recent_heartbeat') {
        return false;
    }
    ...
    if (status.statusFresh === false) return false;
...
function normalizeStatusPercent(value) {
    if (value === null || value === undefined || typeof value === 'boolean' || String(value).trim() === '') return null;
    const number = Number(value);
    return Number.isFinite(number) && number >= 0 && number <= 100 ? number : null;
}

function mergeDeviceStatusSnapshot(previous, incoming) {
    const sameDevice = previous && getStatusDeviceId(previous) === getStatusDeviceId(incoming);
    const base = sameDevice ? previous : {};
    const merged = { ...base, ...incoming };
    // Compact updates may omit unchanged members; explicit null clears a module.
    ['wifi', 'mqtt', 'mobile', 'sim', 'transport', 'connectionTransports', 'systemRuntime', 'call'].forEach(key => {
...
function getSimNumberDisplayValue(status, fallback = 'Not reported by SIM') {
    ...
    const deviceId = String(
        status?.deviceId
        || status?.device_id
        || (typeof getActiveDeviceId === 'function' ? getActiveDeviceId() : '')
        || ''
    ).trim();
    const storageKey = deviceId ? `sim-number:last:${deviceId}` : '';
    if (value) {
        if (storageKey && typeof sessionStorage !== 'undefined') {
            try {
                sessionStorage.setItem(storageKey, String(value));
                const source = String(
                    status?.simNumberSource
                    || status?.subscriberNumberSource
                    || status?.sim?.numberSource
                    || 'device'
                ).trim().toLowerCase();
                sessionStorage.setItem(`${storageKey}:source`, source);
            } catch (_) {}
        }
        return value;
    }
    if (storageKey && typeof sessionStorage !== 'undefined') {
        try {
            const cached = String(sessionStorage.getItem(storageKey) || '').trim();
...
function getSimNumberSourceLabel(status) {
    let source = String(
        status?.simNumberSource
        || status?.sim_number_source
        || status?.subscriberNumberSource
        || status?.subscriber_number_source
        || status?.sim?.numberSource
        || status?.sim?.number_source
        || ''
    ).trim().toLowerCase();
    if (!source && typeof sessionStorage !== 'undefined') {
        ...
        try {
            source = deviceId ? String(sessionStorage.getItem(`sim-number:last:${deviceId}:source`) || '').trim().toLowerCase() : '';
        } catch (_) {}
    }
    if (source === 'stored' || source === 'user-confirmed') return 'SIM (stored)';
    if (source === 'ussd') return 'SIM (USSD)';
    return 'SIM Number';
}
...
function getSimNumbersDisplayValue(status, fallback = 'Not reported by SIM') {
...
function buildOfflineDeviceSnapshot(status = latestDeviceStatus, lastSeen = null) {
    ...
    // An offline transition is not evidence that the device was just seen.
    const offlineSeenAt = lastSeen || status?.lastSeen || status?.last_seen || null;
...
function updateHeaderDeviceSummary(status = latestDeviceStatus, queueState = latestQueueState) {
    ...
    if (status?.statusFresh === false) {
        const seenAt = formatHeaderTimestamp(status?.lastSeen);
        headerMeta.textContent = online ? 'Connected — waiting for fresh status' : (seenAt ? `Last seen ${seenAt}` : 'Awaiting live data');
        headerHealth.textContent = 'Health --';
        headerHealth.title = 'No recent status snapshot';
        return;
    }
...
function getIncomingCallPanelStateElements() {
    return {
        ...
        remoteActions: document.getElementById('incomingCallRemoteActions'),
        localNotice: document.getElementById('incomingCallLocalControlNotice')
    };
}

function supportsAndroidIncomingCallControls() {
    const deviceId = window.getActiveDeviceId ? window.getActiveDeviceId() : (window.DEVICE_ID || '');
    let caps = {};
    try {
        caps = JSON.parse(localStorage.getItem('deviceCaps_' + deviceId) || '{}') || {};
    } catch (_) {}
    const identity = [
        caps.bridge, caps.platform, caps.device_type, caps.deviceType, caps.board,
        latestDeviceStatus?.platform, latestDeviceStatus?.device?.platform,
        latestDeviceStatus?.bridge, latestDeviceStatus?.type
    ].map((value) => String(value || '').toLowerCase()).join(' ');
    return identity.includes('android');
}

function syncIncomingCallCapabilityUi() {
    const supported = supportsAndroidIncomingCallControls();
    const { remoteActions, localNotice } = getIncomingCallPanelStateElements();
    remoteActions?.classList.toggle('d-none', !supported);
    localNotice?.classList.toggle('d-none', supported);
    return supported;
}
...
function isActiveSimScopedEvent(data) {
    ...
    // AND-01 (UI side): the device acknowledged the command and owns it now,
    // but the SMS has not left the device yet. Show progress, not success —
    // the terminal sms:sent / sms:send-failed event settles the row.
    socket.on('sms:accepted', function(data) {
        if (!isActiveSimScopedEvent(data)) return;
        console.log('SMS accepted by device (in flight):', data);
        showToast('Device accepted the SMS — sending in progress', 'info');
        scheduleDashboardSmsRefresh();
    });
    ...
    socket.on('action:result', function(data) {
...
function updateSidebarDeviceStatus(status) {
    if (!status) return;
    if (getStatusDeviceId(status) && !matchesActiveDeviceStatus(status)) return;
    const snapshot = mergeDeviceStatusSnapshot(latestDeviceStatus, status);
    const normalizedStatus = {
        ...snapshot,
        online: inferStatusOnline(snapshot)
    };
    latestDeviceStatus = normalizedStatus;
    ...
    if (normalizedStatus.online && normalizedStatus.statusFresh !== false) {
        ...
        const extendedPanelMetricIds = [
            'panelTemperature', 'panelVoltage', 'panelActivePath', 'panelHealth',
            'panelWifiIp', 'panelModemIp', 'panelRamFree', 'panelPsramFree',
            'panelTransport', 'panelTransportPaths', 'panelCharging', 'panelPowerSource', 'panelSignal', 'panelBattery', 'panelNetwork', 'panelUptime'
        ];
        ...
        extendedPanelMetricIds.forEach((id) => {
            const element = document.getElementById(id);
            if (element) element.textContent = '---';
        });
        ['panelSignalBar', 'panelBatteryBar'].forEach((id) => {
            const bar = document.getElementById(id);
            if (bar) setProgressWidth(bar, 0);
        });
        const chargingEl = document.getElementById('sidebarCharging');
        if (chargingEl) chargingEl.style.display = 'none';
        updateDashboardCards(normalizedStatus);
    }
...
function updateDeviceMetrics(status) {
    ...
    // Update signal
    const signal = normalizeStatusPercent(status.signal);
    const signalEl = document.getElementById('sidebarSignal');
    const signalBar = document.getElementById('sidebarSignalBar');
    ...
    // Update battery
    const battery = normalizeStatusPercent(status.battery);
    const batteryEl = document.getElementById('sidebarBattery');
    const batteryBar = document.getElementById('sidebarBatteryBar');
    const chargingEl = document.getElementById('sidebarCharging');
    ...
    if (chargingEl) {
        chargingEl.style.display = status.charging === true || status.charging === 1 ? 'inline-block' : 'none';
    }
    ...
    const pt = document.getElementById('panelTemperature');
    const pv = document.getElementById('panelVoltage');
    const pap = document.getElementById('panelActivePath');
    const ph = document.getElementById('panelHealth');
    const pwi = document.getElementById('panelWifiIp');
    const pmi = document.getElementById('panelModemIp');
    const ptransport = document.getElementById('panelTransport');
    const ptransportPaths = document.getElementById('panelTransportPaths');
    const pram = document.getElementById('panelRamFree');
    const ppsram = document.getElementById('panelPsramFree');
    const pc = document.getElementById('panelCharging');
    const ppower = document.getElementById('panelPowerSource');
    const hasSignal = signal !== null && signal !== undefined;
    const hasBattery = battery !== null && battery !== undefined;
    const voltageMv = Number(status.voltageMv);
    const formatRuntimeBytes = (value) => {
        if (value === null || value === undefined || value === '') return 'Not reported';
        const number = Number(value);
        if (!Number.isFinite(number) || number < 0) return '---';
        return typeof window.formatBytes === 'function' ? window.formatBytes(number) : `${Math.round(number / 1024)} KiB`;
    };
    if (ps) ps.textContent = (hasSignal ? signal + '%' : '-');
    ...
    if (pb) pb.textContent = (hasBattery ? battery + '%' : '-');
    ...
    if (pt) pt.textContent = status.temperature !== null && status.temperature !== undefined ? `${status.temperature} °C` : 'Not reported';
    if (pv) pv.textContent = Number.isFinite(voltageMv) && voltageMv > 0 ? `${(voltageMv / 1000).toFixed(2)} V` : 'Not reported';
    if (pc) pc.textContent = status.charging === true || status.charging === 1 ? 'Yes'
        : status.charging === false || status.charging === 0 ? 'No' : 'Not reported';
    if (ppower) ppower.textContent = status.powerSource || status.power_source || 'Not reported';
    if (pap) pap.textContent = status.activePath === 'wifi'
        ? 'Wi-Fi primary'
        : (status.activePath === 'modem' ? 'Cellular fallback' : (status.activePathLabel || 'Not reported'));
    if (ph) {
        ph.textContent = typeof status.healthScore === 'number' ? `${status.healthScore}%${status.healthDegraded ? ' degraded' : ''}` : 'Not reported';
        ph.classList.remove('text-success', 'text-warning', 'text-danger');
        if (typeof status.healthScore === 'number') {
            ph.classList.add(status.healthScore >= 70 && !status.healthDegraded ? 'text-success' : status.healthScore >= 40 ? 'text-warning' : 'text-danger');
        }
    }
    const transportInfo = status.connectionTransports || {};
    const wifiTransport = transportInfo.wifi || {};
    const modemTransport = transportInfo.modem || {};
    const mqttTransport = transportInfo.mqtt || {};
    const formatIpTransport = (entry, fallbackIp, fallbackLabel) => {
        const ip = entry.ipAddress || fallbackIp;
        const label = entry.transport || fallbackLabel;
        return ip ? `${ip} (${label})` : `Not reported (${label})`;
    };
    if (pwi) pwi.textContent = formatIpTransport(
        wifiTransport,
        status.wifi?.ipAddress || (status.activePath === 'wifi' ? status.ip : null),
        'Wi-Fi'
    );
    if (pmi) pmi.textContent = formatIpTransport(
        modemTransport,
        status.sim?.dataIp || status.sim?.pdpIp || (status.activePath === 'modem' ? (status.mobile?.ipAddress || status.ip) : null),
        'Not available'
    );
    if (ptransport) {
        const websocketLabel = window.socket?.connected === true ? 'WebSocket' : 'WebSocket offline';
        ptransport.textContent = `${mqttTransport.label || 'MQTT offline'} · ${websocketLabel}`;
    }
    if (ptransportPaths) {
        ptransportPaths.textContent = `Wi-Fi ${formatIpTransport(wifiTransport, null, 'Wi-Fi')} · Modem ${formatIpTransport(modemTransport, null, 'Not available')}`;
    }
    if (pram) pram.textContent = formatRuntimeBytes(status.systemRuntime?.runtimeRamFree ?? status.systemRuntime?.heapFree ?? status.systemRuntime?.freeHeap);
    if (ppsram) ppsram.textContent = formatRuntimeBytes(status.systemRuntime?.psramFree ?? status.systemRuntime?.freePsram);
...
function updateDashboardCards(status) {
    const connected = inferStatusOnline(status);
    const staleStatus = connected && status?.statusFresh === false;
    const isOnline = connected && !staleStatus;
    const selectedSim = getSelectedSimSlotSnapshot(status);
    const wifiExplicitlyDisconnected = status?.wifi?.connected === false || status?.wifi_connected === false;
    ...
    if (dashWifiPrimary) {
        dashWifiPrimary.textContent = isOnline && !wifiExplicitlyDisconnected
            ? (status?.wifiRoleLabel || (status?.activePath === 'wifi'
                    ? 'Primary'
                    : (status?.wifi?.connected === true ? 'Connected' : 'Offline')))
            : (staleStatus ? 'Unknown' : 'Offline');
    }
    if (dashModemActive) {
        dashModemActive.textContent = isOnline
            ? (status?.modemRoleLabel || (status?.activePath === 'modem'
                    ? 'Fallback'
                    : (status?.sim?.registered === true ? 'Connected' : 'Offline')))
            : (staleStatus ? 'Unknown' : 'Offline');
    }

    const wifiSignal = status?.wifi?.rssi === 0 ? null : normalizeStatusPercent(status?.wifiSignal);
    ...
    const cellSignal = normalizeStatusPercent(status?.cellularSignal);
    ...
    if (cellMeta) cellMeta.textContent = isOnline ? (status?.cellularStatus || 'Unavailable') : (staleStatus ? 'Status stale' : 'Disconnected');

    const battery = normalizeStatusPercent(status?.battery);
    const voltageMv = Number(status?.voltageMv || 0);
    const batteryHasVoltage = Number.isFinite(voltageMv) && voltageMv > 0;
    const charging = status?.charging === true || status?.charging === 1 ? true
        : status?.charging === false || status?.charging === 0 ? false : null;
    const batteryHasTelemetry = (battery !== null && battery !== undefined) || batteryHasVoltage || (charging !== null && charging !== undefined);
    ...
    const chargingLabel = (charging === null || charging === undefined)
        ? 'Unknown'
        : (charging ? 'Yes' : 'No');
    const batteryPowerSourceLabel = status?.powerSource || status?.power_source || 'Unknown';
    ...
    const dashSimNumberLabel = document.getElementById('dashSimNumberLabel');
    ...
    if (dashSimNumberLabel) dashSimNumberLabel.textContent = isOnline ? getSimNumberSourceLabel(status) : 'SIM Number';
    if (dashCurrentIp) dashCurrentIp.textContent = isOnline ? (status?.mobile?.ipAddress || status?.sim?.dataIp || status?.sim?.pdpIp || (status?.activePath === 'modem' ? status?.ip : null) || 'Not reported') : '-';
    ...
    if (healthEl) {
        ...
        if (isOnline && typeof score === 'number' && Number.isFinite(score)) {
...
function updateQueueStatus(queueState) {
    ...
    if (dashboardFailed > 0) dashboardSummaryText += `, ${dashboardFailed} failed history`;
...
const DASHBOARD_MODULE_ICONS = Object.freeze({
    mqtt: 'bi-broadcast-pin',
    wifi: 'bi-wifi',
    modem: 'bi-reception-4',
    storage: 'bi-sd-card'
});

function renderDashboardModuleHealth(moduleHealth) {
    const target = document.getElementById('dashboardModuleHealth');
    if (!target) return;

    if (!hasActiveDeviceContext() || !deviceOnline()) {
        target.innerHTML = '<div class="dashboard-empty-state text-center py-2"><i class="bi bi-cloud-slash fs-4 text-muted"></i><p class="text-muted mt-2 mb-0">Waiting for device heartbeat</p></div>';
        return;
    }

    const preferred = ['mqtt', 'wifi', 'modem', 'storage'];
    const modules = preferred
        .map((key) => (Array.isArray(moduleHealth) ? moduleHealth.find((entry) => entry.moduleKey === key) : null))
        .filter(Boolean)
        .filter((entry) => entry.state !== 'unsupported');

    if (modules.length === 0) {
        target.innerHTML = '<div class="text-muted small">Waiting for module diagnostics…</div>';
        return;
    }

    const tiles = modules.map((entry) => {
        const href = getStatusPanelModuleHref(entry.moduleKey) || '/devices/about';
        return `<a href="${escapeHtml(href)}" data-device-aware-href="${escapeHtml(href)}" class="dashboard-module-tile text-decoration-none" data-state="${escapeHtml(entry.state || 'unknown')}">
            <span class="dashboard-module-icon"><i class="bi ${escapeHtml(DASHBOARD_MODULE_ICONS[entry.moduleKey] || 'bi-cpu')}"></i></span>
            <span class="min-w-0 flex-grow-1">
                <span class="d-flex justify-content-between gap-2"><strong>${escapeHtml(entry.label || entry.moduleKey || 'Module')}</strong><span class="dashboard-module-state">${escapeHtml(entry.state || 'unknown')}</span></span>
                <small class="text-muted d-block text-truncate" title="${escapeHtml(entry.message || '')}">${escapeHtml(entry.message || 'Waiting for telemetry')}</small>
            </span>
        </a>`;
    }).join('');

    const activePath = latestDeviceStatus?.activePath === 'wifi'
        ? 'Wi-Fi'
        : (latestDeviceStatus?.activePath === 'modem' ? 'Cellular' : 'Not reported');
    const ramFree = Number(latestDeviceStatus?.systemRuntime?.runtimeRamFree
        ?? latestDeviceStatus?.systemRuntime?.heapFree
        ?? latestDeviceStatus?.systemRuntime?.freeHeap);
    const ramText = Number.isFinite(ramFree)
        ? (typeof window.formatBytes === 'function' ? window.formatBytes(ramFree) : `${Math.round(ramFree / 1024)} KiB`)
        : '—';

    target.innerHTML = `<div class="dashboard-module-grid">${tiles}</div>
        <div class="dashboard-runtime-facts mt-3">
            <div><span>Active route</span><strong id="dashboardRuntimePath">${escapeHtml(activePath)}</strong></div>
            <div><span>Internal RAM free</span><strong id="dashboardRuntimeRam">${escapeHtml(ramText)}</strong></div>
        </div>`;
}
...
function renderSidebarModuleHealth(moduleHealth) {
    renderDashboardModuleHealth(moduleHealth);
    ...
    const preferred = ['mqtt', 'modem', 'wifi', 'gps', 'storage', 'display', 'camera', 'audio', 'nfc', 'rfid', 'touch', 'keyboard'];
    ...
    target.innerHTML = sorted.map((entry) => {
        const href = getStatusPanelModuleHref(entry.moduleKey);
...
const PAGE_HARDWARE_MAP = {
    ...
    storage:   { cap: 'sd',       label: 'SD card' },
    intercom:  { cap: 'intercom', label: 'camera / audio module' },
    webcam:    { cap: 'camera',   label: 'camera module' },
...
function applyDeviceCapabilities(caps) {
    ...
    document.querySelectorAll('[data-cap], [data-cap-any]').forEach(el => {
        ...
        const has = keys.some(key => Boolean(caps[key]));
...
window.addEventListener('device:changed', function () {
    ...
    lastLiveDeviceStatusAt = 0;
    updateSidebarDeviceStatus(latestDeviceStatus);
    const restoredSidebarVisibility = loadCachedSidebarVisibility(deviceId);
    loadCachedDeviceCapabilities(deviceId, { preserveVisibilityOnMiss: restoredSidebarVisibility });
    updateHeaderDeviceSummary(latestDeviceStatus, latestQueueState);
    updateSidebarSimSelector(latestDeviceStatus);
    renderUnreadBadgeState(0);
    updateUnreadBadge();
    refreshNotificationSummary();
    refreshHeaderNotificationPreview();
...
window.answerIncomingCall = async function () {
    if (!supportsAndroidIncomingCallControls()) {
        showToast('Answer this call on the connected device', 'warning');
        return;
    }
    hideIncomingCallPanel();
...
window.rejectIncomingCall = async function () {
    if (!supportsAndroidIncomingCallControls()) {
        showToast('Reject this call on the connected device', 'warning');
        return;
    }
    hideIncomingCallPanel();
    setIncomingCallActionState(true);
...
// Theme preference (the existing header control cycles light, dark, system).
const themeModes = ['light', 'dark', 'system'];
const systemThemeQuery = window.matchMedia?.('(prefers-color-scheme: dark)');

function applyTheme(preference) {
    const mode = themeModes.includes(preference) ? preference : 'system';
    const resolved = mode === 'system' ? (systemThemeQuery?.matches ? 'dark' : 'light') : mode;
    document.documentElement.setAttribute('data-theme-preference', mode);
    document.documentElement.setAttribute('data-bs-theme', resolved);
    document.documentElement.style.colorScheme = resolved;
    const icon = document.getElementById('darkModeIcon');
    if (icon) icon.className = mode === 'system' ? 'bi bi-display' : mode === 'dark' ? 'bi bi-moon-stars-fill' : 'bi bi-sun-fill';
...
window.toggleDarkMode = function () {
    ...
    const themeVal = themeModes[(themeModes.indexOf(current) + 1) % themeModes.length];
...
// Prepaint has already resolved the saved preference; keep OS changes live.
(function () {
    applyTheme(document.documentElement.getAttribute('data-theme-preference'));
