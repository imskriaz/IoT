...
async function initializeDatabase() {
    ...
    try {
        ...
        for (const [table, col, def] of [
            ['device_wifi_networks', 'priority', 'INTEGER DEFAULT 50'],
            ['device_wifi_networks', 'enabled', 'INTEGER DEFAULT 1']
        ]) {
            const cols = await db.all(`PRAGMA table_info(${table})`);
            if (cols && !cols.some(c => c.name === col)) {
                await db.exec(`ALTER TABLE ${table} ADD COLUMN ${col} ${def}`);
        ...
        await db.exec(`
            CREATE TRIGGER IF NOT EXISTS forget_wifi_selected_credentials
            AFTER DELETE ON device_wifi_networks
            BEGIN
                UPDATE device_profiles SET wifi_ssid = NULL, wifi_pass = NULL,
                    updated_at = CURRENT_TIMESTAMP
                WHERE device_id = OLD.device_id AND wifi_ssid = OLD.ssid;
            END;
        `);
        ...
        // Scheduled SMS conditional-claim lease (QUEUE-02): claimed_at is the
        // lease timestamp, attempt_count bounds transient retries.
        try { await db.exec(`ALTER TABLE scheduled_sms ADD COLUMN claimed_at DATETIME`); } catch (e) {}
        try { await db.exec(`ALTER TABLE scheduled_sms ADD COLUMN attempt_count INTEGER DEFAULT 0`); } catch (e) {}

        // Automation flow ownership (SEC-03): a flow belongs to exactly one user.
        try { await db.exec(`ALTER TABLE automation_flows ADD COLUMN user_id INTEGER REFERENCES users(id)`); } catch (e) {}

        // Webhook queue lease column (QUEUE-01): tracks the drain that claimed a row.
        try { await db.exec(`ALTER TABLE webhook_queue ADD COLUMN claimed_at DATETIME`); } catch (e) {}

        // HTTP SMS poll lease (QUEUE-02): claim timestamp + retry budget per row.
        try { await db.exec(`ALTER TABLE sms ADD COLUMN http_claimed_at DATETIME`); } catch (e) {}
        try { await db.exec(`ALTER TABLE sms ADD COLUMN http_claim_attempts INTEGER DEFAULT 0`); } catch (e) {}
        ...
        // Contact ownership (cross-tenant isolation): each contact belongs to
        // exactly one user. Rows with NULL user_id are legacy/shared rows that
        // predate this migration and remain visible to admins only.
        try { await db.exec(`ALTER TABLE contacts ADD COLUMN user_id INTEGER REFERENCES users(id)`); } catch (e) {}
        try { await db.exec(`CREATE INDEX IF NOT EXISTS idx_contacts_user ON contacts(user_id)`); } catch (e) {}
        ...
        try {
            ...
            const smsCreateSql = String(rawDb.prepare(
                `SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'sms'`
            ).get()?.sql || '');
            const hasLegacySmsTimestampUnique = /UNIQUE\s*\(\s*device_id\s*,\s*timestamp\s*,\s*from_number\s*\)/i.test(smsCreateSql);
            ...
            if (!hasFk('sms') || tableHasColumn('sms', legacySimScopeColumn) || hasLegacySmsTimestampUnique) {
                const smsColumns = [
                    'id',
                    'device_id',
                    'conversation_id',
                ...
                rebuildTable(
                    ...
                    `CREATE TABLE sms_new (
                        ...
                        multipart_part_count INTEGER,
                        multipart_group_key TEXT,
                        http_claimed_at DATETIME,
                        http_claim_attempts INTEGER DEFAULT 0,
                        FOREIGN KEY (device_id) REFERENCES devices(id),
                        FOREIGN KEY (user_id) REFERENCES users(id),
                        FOREIGN KEY (conversation_id) REFERENCES sms_conversations(id) ON DELETE SET NULL
                    );`,
                    'Migration: sms table rebuilt for current constraints'
                );
            }
        ...
        await db.exec(`
            ...
            CREATE INDEX IF NOT EXISTS idx_sms_device_firmware_storage
                ON sms(device_id, firmware_storage_id);

            CREATE TRIGGER IF NOT EXISTS trg_sms_device_firmware_storage_dedup
            BEFORE INSERT ON sms
            WHEN NEW.type = 'incoming'
              AND COALESCE(NEW.device_id, '') != ''
              AND NEW.firmware_storage_id IS NOT NULL
              AND EXISTS (
                  SELECT 1 FROM sms
                  WHERE device_id = NEW.device_id
                    AND type = 'incoming'
                    AND firmware_storage_id = NEW.firmware_storage_id
              )
            BEGIN
                SELECT RAISE(IGNORE);
            END;
        ...
        `);
        const deliveryColumns = await db.all('PRAGMA table_info(device_command_queue)');
        for (const [column, type] of [
            ['delivery_status', 'TEXT'], ['delivery_report', 'TEXT'],
            ['delivery_at', 'TEXT'], ['expires_at', 'DATETIME']
        ]) {
            if (!deliveryColumns.some((entry) => entry.name === column)) {
                await db.exec(`ALTER TABLE device_command_queue ADD COLUMN ${column} ${type}`);
            }
        }
        ...
        await db.exec(`CREATE INDEX IF NOT EXISTS idx_device_command_queue_expiry ON device_command_queue(status, expires_at)`);
        // Admission is enforced in SQLite so every producer, including the
        // atomic multipart SMS transaction, observes one concurrency-safe cap.
        await db.exec(`
            CREATE TRIGGER IF NOT EXISTS trg_device_command_queue_capacity
            BEFORE INSERT ON device_command_queue
            WHEN (
                SELECT COUNT(*) FROM device_command_queue
                WHERE device_id = NEW.device_id
                  AND status IN ('pending', 'dispatching', 'waiting_response')
            ) >= 256 OR (
                SELECT COUNT(*) FROM device_command_queue
                WHERE status IN ('pending', 'dispatching', 'waiting_response')
            ) >= 2048
            BEGIN
                SELECT RAISE(ABORT, 'device command queue capacity reached');
            END;
        `);

        // Durable execution-tier receipts. Firmware terminal results are stored
        // here before the dashboard sends its database acknowledgement. This
        // table is intentionally independent of the short-lived command queue:
        // queue cleanup must not erase proof needed for replay/deduplication.
        await db.exec(`
            CREATE TABLE IF NOT EXISTS device_action_results (
                ...
                broker_published_at DATETIME,
                ack_attempts INTEGER NOT NULL DEFAULT 0,
                conflict_count INTEGER NOT NULL DEFAULT 0,
                last_ack_error TEXT,
                UNIQUE(device_id, action_id, command)
            );
            CREATE INDEX IF NOT EXISTS idx_device_action_results_ack
                ON device_action_results(broker_published_at, received_at);
            CREATE INDEX IF NOT EXISTS idx_device_action_results_device
                ON device_action_results(device_id, received_at DESC);
        `);

        // Carrier reports can arrive after operators clear completed command
        // rows. Keep only compact modem/SMS correlation, independent of the
        // short-lived execution queue, and expire it after the carrier window.
        await db.exec(`
            CREATE TABLE IF NOT EXISTS sms_delivery_correlations (
                device_id TEXT NOT NULL,
            ...
            CREATE INDEX IF NOT EXISTS idx_sms_delivery_correlations_reference
                ON sms_delivery_correlations(device_id, message_reference, expires_at);
            CREATE INDEX IF NOT EXISTS idx_sms_delivery_correlations_group
                ON sms_delivery_correlations(device_id, base_message_id, part_index);
            CREATE TRIGGER IF NOT EXISTS trg_device_queue_sms_delivery_correlation
            AFTER UPDATE OF status, response_payload ON device_command_queue
            WHEN NEW.status = 'completed'
             AND NEW.command IN ('send-sms', 'send-sms-multipart')
             AND json_type(NEW.response_payload, '$.message_reference') IN ('integer', 'real')
            BEGIN
                INSERT INTO sms_delivery_correlations (
                    device_id, message_id, queue_id, message_reference,
                    base_message_id, sms_id, part_index, part_count, to_number,
                    created_at, expires_at
                ) VALUES (
                    NEW.device_id,
                    NEW.message_id,
                    NEW.id,
                    CAST(json_extract(NEW.response_payload, '$.message_reference') AS INTEGER),
                    COALESCE(NULLIF(json_extract(NEW.payload, '$.sms_base_message_id'), ''),
                        CASE WHEN instr(NEW.message_id, '_p') > 0
                             THEN substr(NEW.message_id, 1, instr(NEW.message_id, '_p') - 1)
                             ELSE NEW.message_id END),
                    CAST(COALESCE(json_extract(NEW.payload, '$.smsId'), json_extract(NEW.payload, '$.sms_id')) AS INTEGER),
                    CAST(COALESCE(json_extract(NEW.payload, '$.sms_part_index'), json_extract(NEW.payload, '$.sms_part')) AS INTEGER),
                    CAST(COALESCE(json_extract(NEW.payload, '$.sms_part_count'), json_extract(NEW.payload, '$.sms_parts_total')) AS INTEGER),
                    COALESCE(json_extract(NEW.payload, '$.to'), json_extract(NEW.payload, '$.number')),
                    CURRENT_TIMESTAMP,
                    datetime('now', '+30 days')
                )
                ON CONFLICT(device_id, message_id) DO UPDATE SET
                    queue_id = excluded.queue_id,
                    message_reference = excluded.message_reference,
                ...
                DELETE FROM sms_delivery_correlations WHERE datetime(expires_at) <= CURRENT_TIMESTAMP;
            END;
        `);
        ...
        // Dashboard-owned plug-and-play hardware configuration. Catalog entries
        // are immutable definitions; desired/applied/observed revisions remain
        // separate so intent is never mistaken for device readiness.
        await db.exec(`
            CREATE TABLE IF NOT EXISTS hardware_catalog_revisions (
            ...
            CREATE TABLE IF NOT EXISTS device_hardware_apply_jobs (
            ...
            CREATE INDEX IF NOT EXISTS idx_hardware_jobs_device_status
                ON device_hardware_apply_jobs(device_id, status, updated_at DESC);
        `);

        // Existing intent rows have no recoverable snapshot. Leave them NULL;
        // never substitute today's desired configuration for a historical job.
        const hardwareJobColumns = await db.all('PRAGMA table_info(device_hardware_apply_jobs)');
        for (const column of ['requested_config_json', 'catalog_version']) {
            if (!hardwareJobColumns.some(info => info.name === column)) {
                await db.exec(`ALTER TABLE device_hardware_apply_jobs ADD COLUMN ${column} TEXT`);
            }
        }
        await db.exec(`
            CREATE TRIGGER IF NOT EXISTS hardware_apply_job_immutable_request
            BEFORE UPDATE ON device_hardware_apply_jobs
            WHEN NEW.action_id IS NOT OLD.action_id
                OR NEW.device_id IS NOT OLD.device_id
                OR NEW.enrollment_epoch IS NOT OLD.enrollment_epoch
                OR NEW.requested_revision IS NOT OLD.requested_revision
                OR NEW.requested_hash IS NOT OLD.requested_hash
                OR NEW.previous_hash IS NOT OLD.previous_hash
                OR NEW.requested_config_json IS NOT OLD.requested_config_json
                OR NEW.catalog_version IS NOT OLD.catalog_version
                OR NEW.created_by IS NOT OLD.created_by
                OR NEW.created_at IS NOT OLD.created_at
            BEGIN
                SELECT RAISE(ABORT, 'Hardware apply request is immutable');
            END;
        `);
