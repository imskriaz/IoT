...
const { isPlaceholder: isPlaceholderSecret } = require('./utils/check_env');
...
const {
    ...
    isLiveMqttCommandAcceptingStatus
} = require('./utils/dashboardStatus');
...
let db;
// Resolves once the database (and storage directories) are ready. The HTTP
// listener and background workers are gated on this promise so `/health` can
// never report healthy while database readiness is false (DASH-03).
const dbReady = (async () => {
    try {
        db = await initializeDatabase();
        app.locals.db = db;
        app.locals.dbReadyAt = Date.now();
        try {
...
// ==================== REVERSE PROXY TRUST (DASH-02) ====================
// Behind nginx/Cloudflare, X-Forwarded-* headers must be trusted for secure
// cookies and IP-keyed rate limits to work. Configure TRUST_PROXY to the
// number of trusted proxy hops (e.g. "1" for a single local nginx), a CIDR
// range (e.g. "127.0.0.1/32"), or "loopback". Unset = direct exposure (dev).
(function configureTrustProxy() {
    ...
    logger.info(`Trust proxy configured: ${configured}`);
})();
...
app.use(helmet({
    // Conservative CSP: locks object-src/base-uri/frame-ancestors/form-action.
    // Inline scripts are still permitted ('unsafe-inline') because the EJS
    // pages embed server-rendered JSON payloads; the stored-XSS vector for
    // those payloads is closed by escaping `<` in every JSON.stringify
    // injection (SEC-06) instead. A nonce-based CSP requires a view refactor.
    ...
    crossOriginEmbedderPolicy: false  // permits the map's external image tiles
}));
...
require('./config/vendorAssets').mountVendorAssets(app);

// Per-user rate limiting — mounted BEFORE the session middleware only for the
// IP-keyed login limiter. The session-aware /api limiter is mounted further
// down (after session) so the per-user bucket actually applies.
...
// Deployment gate (OPS-01): production refuses to boot while any critical
// secret is still a placeholder or the insecure default. Rotate with
// `npm run env:rotate` (previous values are preserved in a safety copy).
const INSECURE_DEFAULT_SECRET = 'secret-key-change-in-production';
const CRITICAL_BOOT_SECRET_KEYS = ['SESSION_SECRET', 'MQTT_PASSWORD', 'ADMIN_PASSWORD', 'SUPER_PASS'];
const insecureSecretFailures = [];
for (const key of CRITICAL_BOOT_SECRET_KEYS) {
    const value = String(process.env[key] || '');
    if (!value) {
        // Empty MQTT/admin credentials are legal (broker or first-boot
        // without auth); an empty SESSION_SECRET is not.
        if (key === 'SESSION_SECRET') insecureSecretFailures.push(`${key} (not set)`);
        continue;
    }
    if (value === INSECURE_DEFAULT_SECRET) {
        insecureSecretFailures.push(`${key} (insecure default)`);
    } else if (isPlaceholderSecret(value)) {
        insecureSecretFailures.push(`${key} (placeholder)`);
    }
}
if (insecureSecretFailures.length && process.env.NODE_ENV === 'production') {
    logger.error(
        `FATAL: deployment secrets must be rotated before a production boot: ` +
        `${insecureSecretFailures.join(', ')}. Run npm run env:rotate, update external ` +
        `systems (broker credentials, admin login) to match, and restart.`
    );
    process.exit(1);
}
if (insecureSecretFailures.length) {
    logger.warn(
        `⚠️  Deployment secrets are placeholders: ${insecureSecretFailures.join(', ')} — ` +
        `rotate with npm run env:rotate before going live.`
    );
...
// Session-aware API rate limiter (per user, IPv6-safe IP fallback) — mounted
// after the session middleware so req.session is populated (previously it sat
// before the session store and always keyed by IP).
...
// Serve firmware binaries — auth guarded (session or API key) and limited to
// .bin only. Mounted after the session middleware so browser sessions can
// authenticate too; previously it sat before the session store existed, which
// silently broke session-based firmware downloads.
...
// CSRF protection for cookie-authenticated state changes. API-key requests are
// exempt because they are bearer-authenticated and do not use browser cookies.
// Enabled by default in ALL environments (SEC-09); set CSRF_PROTECTION=false
// only for trusted local integrations that cannot send the token header.
// /auth/* stays exempt: those endpoints run before/around session
// regeneration (login/register/reset), so a session-bound token cannot be
// required there. Authenticated state changes (including logout) are covered.
const csrfEnabled = process.env.CSRF_PROTECTION !== 'false';
if (csrfEnabled) {
    app.use((req, res, next) => {
        if (['GET', 'HEAD', 'OPTIONS'].includes(req.method)) return next();
        const pathName = String(req.originalUrl || req.url || '').split('?')[0];
        if (pathName.startsWith('/auth/') || pathName === '/auth/login' || pathName === '/health') return next();
        const authHeader = String(req.headers?.authorization || '');
        if (req.headers?.['x-api-key'] || authHeader.startsWith('Bearer edk_') || authHeader.startsWith('Bearer pk_')) {
            return next();
        }
        const expected = String(req.session?.csrfToken || '');
        const supplied = String(req.get?.('x-csrf-token') || req.body?._csrf || '');
        const valid = expected.length >= 16 && supplied.length === expected.length
...
io.use((socket, next) => {
    try {
        ...
        sessionMiddleware(socket.request, {}, (err) => {
            ...
            // Personal room for user-scoped emits (e.g. webhook CRUD events).
            socket.join(`user:${sessionData.user.id}`);
            return next();
        });
    } catch (error) {
...
const statusWatchDisableTimers = new Map();
const STATUS_WATCH_DISCONNECT_GRACE_MS = 5000;
...
function setSocketDeviceWatch(socket, deviceId) {
    ...
    const pendingDisable = statusWatchDisableTimers.get(nextDeviceId);
    if (pendingDisable) {
        clearTimeout(pendingDisable);
        statusWatchDisableTimers.delete(nextDeviceId);
    }
...
function clearSocketDeviceWatch(socket) {
    ...
    if (previousCount > 0) {
        ...
        dashboardDeviceWatchers.delete(previousDeviceId);
        const existingTimer = statusWatchDisableTimers.get(previousDeviceId);
        if (existingTimer) clearTimeout(existingTimer);
        const disableTimer = setTimeout(() => {
            statusWatchDisableTimers.delete(previousDeviceId);
            if (Number(dashboardDeviceWatchers.get(previousDeviceId) || 0) === 0) {
                publishStatusWatch(previousDeviceId, false);
            }
        }, STATUS_WATCH_DISCONNECT_GRACE_MS);
        disableTimer.unref?.();
        statusWatchDisableTimers.set(previousDeviceId, disableTimer);
    }
...
mqttService.on('status', (deviceId, data) => {
    const normalizedDeviceId = String(deviceId || '').trim();
    const watcherCount = Number(dashboardDeviceWatchers.get(normalizedDeviceId) || 0);
    const liveCommandAccepting = isLiveMqttCommandAcceptingStatus(data);

    if (!normalizedDeviceId) {
        return;
    }

    const wasCommandAccepting = statusWatchReadyDevices.has(normalizedDeviceId);
    ...
    if (watcherCount <= 0 || !liveCommandAccepting || wasCommandAccepting) {
        return;
    }
...
async function isSocketDeviceAllowed(socket, deviceId) {
    const normalized = String(deviceId || '').trim();
    if (!normalized) return false;
    const user = withEffectiveRole(socket.user || socket.request?.session?.user);
    if (user && ['admin', 'superadmin'].includes(user.role)) return true;
    const database = app.locals?.db;
    if (!database || !user?.id) return false;
    try {
        const assignment = await database.get(
            'SELECT 1 AS allowed FROM device_users WHERE device_id = ? AND user_id = ?',
            [normalized, user.id]
        );
        return Boolean(assignment?.allowed);
    } catch (error) {
        logger.warn('Socket device authorization lookup failed', { message: error?.message });
        return false;
    }
}
...
io.on('connection', async (socket) => {
    ...
    let initialDeviceId = resolveDeviceId({ session: socket.request?.session }, DEFAULT_DEVICE_ID);
    if (initialDeviceId && !(await isSocketDeviceAllowed(socket, initialDeviceId))) {
        initialDeviceId = '';
    }
    ...
    socket.on('subscribe:device', socketRateLimit('subscribe:device', async ({ deviceId } = {}) => {
        if (!deviceId || typeof deviceId !== 'string') return;
        const normalizedDeviceId = String(deviceId).trim();
        if (!normalizedDeviceId) return;
        if (!(await isSocketDeviceAllowed(socket, normalizedDeviceId))) {
            socket.emit('device:access-denied', { deviceId: normalizedDeviceId });
            return;
        }
    ...
    socket.on('get:device-status', socketRateLimit('get:device-status', async ({ deviceId } = {}) => {
        const requestedDeviceId = resolveDeviceId({
            body: { deviceId },
            session: socket.request?.session
        }, DEFAULT_DEVICE_ID);
        if (!requestedDeviceId) return;
        if (!(await isSocketDeviceAllowed(socket, requestedDeviceId))) {
            socket.emit('device:access-denied', { deviceId: requestedDeviceId });
            return;
        }
    ...
    socket.on('get:devices', socketRateLimit('get:devices', async () => {
        let devices = modemService.getAllDevices();
        const user = withEffectiveRole(socket.user || socket.request?.session?.user);
        if (!['admin', 'superadmin'].includes(user?.role)) {
            ...
            if (!database || !user?.id) {
                ...
                try {
                    const rows = await database.all('SELECT device_id FROM device_users WHERE user_id = ?', [user.id]);
                    const allowed = new Set(rows.map(row => String(row.device_id || '').trim()));
                    devices = devices.filter(device => allowed.has(String(device.id || device.deviceId || '').trim()));
                } catch (error) {
                    logger.warn('Socket device list authorization lookup failed', { message: error?.message });
                    devices = [];
                }
            }
        }
...
// ==================== BACKGROUND WORKERS (DB-GATED, DASH-03) ====================
// All DB-dependent workers start only after dbReady resolves; the HTTP
// listener below is gated on the same promise.
...
app.get('/health', (req, res) => {
    const databaseReady = !!app.locals.db;
    res.status(databaseReady ? 200 : 503).json({
        ...
        mqtt: {
            ...
            subscriptionsReady: mqttService.subscriptionsReady
        },
...
// ==================== ROUTES ====================
try {
    // Keep the public entry point canonical; older links and bookmarks should
    // not fall through to the dashboard 404 shell.
    app.get('/login', (_req, res) => res.redirect(302, '/auth/login'));
...
dbReady.then(() => {
    ...
    // initializeDatabase failures already exit(1) inside dbReady; this guard
    // only covers rejections that escape the internal try/catch.
    process.exit(1);
});
