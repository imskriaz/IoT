...
function parseArgs(argv = process.argv.slice(2)) {
    ...
    for (const arg of argv) {
        ...
        if (arg === '--rotate') {
            // OPS-01: rotation regenerates supported secrets even when
            // already set. Always paired with a safety copy.
            options.rotate = true;
            options.write = true;
            continue;
        }
...
function normalizedHasValue(value) {
...
function buildReplacementPlan(content, selectedKeys = null, { rotate = false } = {}) {
    ...
    lines.forEach((line, index) => {
        ...
        if (!rotate && !shouldReplaceValue(key, value)) {
            skipped.push({ key, reason: 'already set' });
            return;
        }
        if (rotate && !normalizedHasValue(value)) {
            skipped.push({ key, reason: 'empty (set a value first)' });
            return;
        }
...
function seedSecrets({
    ...
    const plan = buildReplacementPlan(content, keys, { rotate });
...
function main() {
    ...
    if (options.rotate) {
        console.log('');
        console.log('Rotation mode: supported secrets were regenerated.');
        if (result.safetyCopyPath) {
            console.log('Previous values are preserved in the safety copy above.');
        }
        console.log('Update any external systems (broker credentials, admin login) to match.');
    }
    if (!options.write) {
        console.log('');
