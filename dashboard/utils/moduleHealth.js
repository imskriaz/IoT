...
function isSupported(moduleKey, caps = {}) {
    ...
    return isCapabilityAvailable(caps, MODULE_DEFINITIONS[moduleKey]?.capability || moduleKey);
}
...
function getBaseEntry(moduleKey, caps = {}, mqttConnected = false, live = {}) {
    ...
    if (moduleKey === 'wifi' && !Object.prototype.hasOwnProperty.call(caps, 'wifi')
        && live.wifi && Object.keys(live.wifi).length > 0) {
        supported = true;
    }

    if (moduleKey === 'storage' && !Object.prototype.hasOwnProperty.call(caps, 'storage')
        && !Object.prototype.hasOwnProperty.call(caps, 'sd') && live.storage) {
        supported = true;
    }
...
async function getDeviceModuleHealth(db, deviceId, caps = {}, options = {}) {
    caps = sanitizeStoredCapabilities(caps);
    ...
    return Object.keys(MODULE_DEFINITIONS).map(moduleKey => {
        ...
        // Historical operation health cannot grant/revoke current support.
        // Drop obsolete state/details when the capability has changed.
        if (!base.supported || (stored && stored.supported !== base.supported)) {
            return offlineAdjustedEntry(base, live);
        }
