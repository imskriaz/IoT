...
function wifiRssiToPercent(rssi) {
    ...
    // Firmware uses 0 as "not sampled". A valid station RSSI is negative.
    if (level === null || level === 0 || level > 0 || level < -127) {
        return null;
    }
...
function resolveStatusFreshness(status, explicitOnline) {
    ...
    const isOnline = hintedOnline === true && status?.online !== false && !staleByAge;
...
function normalizeWifiSnapshot(status) {
    ...
    const hasFlatWifi = status && (
        ...
        || status?.wifi_profiles_revision !== undefined
        || status?.wifi_profiles_count !== undefined
    );
    ...
    return {
        ...
        profilesRevision: firstNumber(wifi?.profilesRevision, status?.wifi_profiles_revision),
        profilesCount: firstNumber(wifi?.profilesCount, status?.wifi_profiles_count),
...
function normalizeSimSnapshot(status) {
    ...
    return {
        ...
        pdpIp: firstText(selectedSlot?.pdpIp, sim?.pdpIp, status?.modem_pdp_ip_address),
        slotCount,
        dualSim: slotCount >= 2,
    ...
    return rawSlots
        .map((slot, index) => {
            ...
            return {
                ...
                number: firstText(slot?.number, slot?.simNumber, slot?.sim_number, slot?.subscriberNumber, slot?.subscriber_number, slot?.msisdn, slot?.phoneNumber, slot?.phone_number),
                simNumber: firstText(slot?.simNumber, slot?.sim_number, slot?.number, slot?.subscriberNumber, slot?.subscriber_number, slot?.msisdn, slot?.phoneNumber, slot?.phone_number),
                subscriberNumber: firstText(slot?.subscriberNumber, slot?.subscriber_number, slot?.number, slot?.simNumber, slot?.sim_number, slot?.msisdn, slot?.phoneNumber, slot?.phone_number),
...
function normalizeDeviceInfo(status) {
    ...
    let model = firstText(status?.model, status?.deviceModel, device?.model, status?.hardware?.boardName, status?.board_name);
...
function getSignalPresentation(status) {
    ...
    if (wifiConnected || activePath === 'wifi') {
        const reportedWifiRssi = toNumberOrNull(wifi?.rssi);
        const wifiRssi = wifiRssiToPercent(reportedWifiRssi) === null ? null : reportedWifiRssi;
...
function resolveModemTransportLabel(source, sim, modemIpAddress, activePath, mqttReady) {
    const explicit = firstText(
        source?.modem_transport,
        source?.modem_data_transport,
        source?.modem_link_transport,
        source?.modem?.transport,
        source?.modem?.dataTransport,
        source?.sim?.dataTransport,
        sim?.dataTransport,
        source?.transport?.modem,
        source?.transport?.modemTransport
    );
    const explicitToken = String(explicit || '').trim().toLowerCase();
    if (explicitToken.includes('ppp')) return activePath === 'modem' && mqttReady ? 'PPP/MQTT' : 'PPP';
    if (explicitToken.includes('at') || explicitToken.includes('uart')) return activePath === 'modem' && mqttReady ? 'AT/MQTT' : 'AT bearer';

    const pppActive = firstBoolean(
        source?.ppp_active,
        source?.ppp_connected,
        source?.modem_ppp_active,
        source?.modem?.pppActive,
        source?.modem?.pppConnected
    );
    if (pppActive === true) return activePath === 'modem' && mqttReady ? 'PPP/MQTT' : 'PPP';

    const dataSession = firstBoolean(
        source?.modem_data_session_open,
        source?.modem_ip_bearer_ready,
        sim?.dataSession,
        sim?.ipBearer
    );
    if (dataSession === true || modemIpAddress) return activePath === 'modem' && mqttReady ? 'AT/MQTT' : 'AT bearer';

    const registered = firstBoolean(sim?.registered, source?.modem_registered);
    return registered === true ? 'AT standby' : 'Not available';
}

function buildConnectionTransportInfo(source, mqtt, sim, activePath, wifiIpAddress) {
    const modemIpAddress = firstText(
        source?.modem_data_ip,
        source?.modem_ip_address,
        source?.modem_pdp_ip_address,
        source?.modem?.dataIp,
        source?.modem?.ipAddress,
        sim?.dataIp,
        sim?.pdpIp,
        activePath === 'modem' ? source?.mobile?.ipAddress : null
    );
    const mqttConnected = mqtt?.connected === true;
    const mqttSubscribed = mqtt?.subscribed === true;
    const mqttReady = mqttConnected && mqttSubscribed;
    return {
        wifi: {
            ipAddress: wifiIpAddress,
            transport: activePath === 'wifi' && mqttReady ? 'Wi-Fi/MQTT' : 'Wi-Fi',
            active: activePath === 'wifi'
        },
        modem: {
            ipAddress: modemIpAddress,
            transport: resolveModemTransportLabel(source, sim, modemIpAddress, activePath, mqttReady),
            active: activePath === 'modem'
        },
        mqtt: {
            connected: mqttConnected,
            subscribed: mqttSubscribed,
            label: mqttConnected && mqttSubscribed ? 'MQTT' : (mqttConnected ? 'MQTT pending' : 'MQTT offline')
        },
        // WebSocket is a dashboard-to-browser channel, not a device bearer.
        websocket: { label: 'WebSocket (dashboard)' }
    };
}
...
function buildDashboardDeviceStatus(status, online) {
    ...
    const simNumberSource = firstText(
        source?.simNumberSource,
        source?.sim_number_source,
        source?.subscriberNumberSource,
        source?.subscriber_number_source,
        sim?.numberSource,
        sim?.number_source,
        source?.sim?.numberSource,
        source?.sim?.number_source
    ) || (simNumber ? 'device' : null);
    const activePath = String(source?.activePath || source?.active_path || source?.status?.active_path || '').trim().toLowerCase() || null;
    const firmwareVersion = firstText(
        source?.firmwareVersion,
        source?.firmware_version,
        source?.status?.firmware_version,
        source?.status?.firmwareVersion
    );
    const wifiIpAddress = firstText(wifi?.ipAddress, source?.wifi?.ipAddress, source?.wifi_ip_address, activePath === 'wifi' ? source?.ip : null);
    const connectionTransports = buildConnectionTransportInfo(source, mqtt, sim, activePath, wifiIpAddress);
    ...
    const wifiSignal = (wifiConnected || activePath === 'wifi')
        ? (wifiRssi === 0 ? null : toNumberOrNull(source?.wifiSignal ?? wifiRssiToPercent(wifiRssi)))
        : null;
    ...
    if (mqtt?.connected === true && mqtt?.subscribed === true) {
    ...
    if (!isOnline) {
        const offlineResult = {
            ...
            app: firstText(source?.app, source?.device?.app),
            type: firstText(source?.type, source?.deviceType, source?.device_type, source?.device?.type),
            bridge_transport: firstText(source?.bridge_transport, source?.transport_mode),
            transport_mode: firstText(source?.transport_mode, source?.bridge_transport),
            ...
            sync,
            storage,
            sdCard: sdStatus.normalize(source),
            systemRuntime,
            hardware,
            call: source?.call
                ? {
            ...
            connectionTransports: {
                ...connectionTransports,
                wifi: { ...connectionTransports.wifi, active: false },
                modem: { ...connectionTransports.modem, active: false },
                mqtt: { ...connectionTransports.mqtt, connected: false, subscribed: false, label: 'MQTT offline' }
            },
            sim,
            simSlots,
    ...
    const result = {
        ...
        powerSource: firstText(
            source?.powerSource,
            source?.power_source,
            source?.system?.powerSource,
            source?.system?.power_source
        ),
        network,
        operator,
        simNumber,
        ...
        statusFresh: source?.statusFresh !== false,
        statusAgeMs: source?.statusAgeMs !== undefined ? source.statusAgeMs : freshness.ageMs,
        lastStatusAt: source?.lastStatusAt || null,
        staleReason: source?.statusFresh === false ? (source.staleReason || 'no_recent_status') : null
    };
...
function isLiveMqttCommandAcceptingStatus(status) {
    if (!status || typeof status !== 'object' || status.online === false) {
        return false;
    }

    const normalized = buildDashboardDeviceStatus(status, true);
    return Boolean(
        normalized?.transport?.mqttCommandAccepting === true
        || (
            normalized?.mqtt?.connected === true
            && normalized?.mqtt?.subscribed === true
        )
    );
}
