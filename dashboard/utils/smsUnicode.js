...
function decodeSmsRecord(record) {
    ...
    const firmwareUtf8 = ['esp32-mqtt', 'esp32-mqtt-sync'].includes(record.source);
    const fromNumber = firmwareUtf8 ? record.from_number : decodeUcs2Hex(record.from_number);
    const toNumber = firmwareUtf8 ? record.to_number : decodeUcs2Hex(record.to_number);
    const message = firmwareUtf8 ? record.message : decodeUcs2Hex(record.message);
