...
const { refreshSmsConversationBySmsId } = require('./smsConversations');
const { OtaLifecycle, isApplyResult, validMetadata: validOtaMetadata } = require('./otaLifecycle');

const FIRMWARE_ACTION_ID_MAX_LENGTH = 31;
// Node timers overflow above this limit even though firmware accepts uint32.
const COMMAND_TIMEOUT_MAX_MS = 0x7FFFFFFF;
const REQUIRED_DASHBOARD_SUBSCRIPTIONS = [
    'device/+/status', 'device/+/heartbeat', 'device/+/action/result'
];
const SUBSCRIPTION_ACK_TIMEOUT_MS = 5000;
const DEFAULT_COMMAND_ADMISSION_TTL_MS = 24 * 60 * 60 * 1000;
const MAX_COMMAND_ADMISSION_TTL_MS = 7 * 24 * 60 * 60 * 1000;
...
function validatedCommandTimeoutMs(value, allowZero = false) {
    const parsed = Number(value);
    if (!Number.isInteger(parsed) || parsed < 0 || parsed > COMMAND_TIMEOUT_MAX_MS ||
        (!allowZero && parsed === 0)) {
        const error = new Error('Command timeout must be a positive whole number of milliseconds within the runtime limit');
        error.code = 'INVALID_COMMAND_TIMEOUT';
        throw error;
    }
    return parsed;
}
...
function isFailedCommandResponse(response) {
    ...
    if (['failed', 'timeout', 'rejected', 'error'].includes(status)) return true;
...
class MQTTService extends EventEmitter {
    constructor() {
        super();
        this.otaLifecycle = new OtaLifecycle(this);
        this.client = null;
        ...
        this._defaultSubscriptionTopics = [];
        this.messageHandlers = new Map();
        this.pendingMessages = new Map();
        this.deviceStatus = new Map(); // Track device last seen
        this.otaHealthRequests = new Map();
        this.seenMessages = new Set(); // Dedup exact broker repeats, not later states for one action ID.
        ...
        this._resultAckRetryAt = 0;
        this._resultAckRetryRunning = false;
    ...
    handleConnect() {
        ...
        this.lastError = null;
        this.lastErrorAt = null;
        const subscriptionEpoch = ++this._subscriptionEpoch;
        clearTimeout(this._subscriptionRetryTimer);
        this._subscriptionRetryTimer = null;
        this.subscribedTopics.clear();
        this.subscriptionsReady = false;
        this._confirmDefaultSubscriptions(subscriptionEpoch).catch((error) => {
            logger.error('Failed to confirm dashboard MQTT subscriptions:', error);
            this._scheduleSubscriptionRetry(subscriptionEpoch);
        });
    ...
    async _confirmDefaultSubscriptions(subscriptionEpoch) {
        // Command/result safety depends only on these three topics. An optional
        // feature topic must not hold the durable command lane at startup.
        await this.subscribeToDefaultTopics(subscriptionEpoch, true);
        if (!this.connected || subscriptionEpoch !== this._subscriptionEpoch) return;
        const wasReady = this.subscriptionsReady;
        this.subscriptionsReady = REQUIRED_DASHBOARD_SUBSCRIPTIONS.every((topic) =>
            this.subscribedTopics.has(topic));
        if (!wasReady && this.subscriptionsReady) {
            this.emit('subscriptions-ready');
            if (global.io) global.io.emit('mqtt:status', this.getStatus());
            this.processPersistentQueue().catch((error) => {
                logger.error('Failed to process persistent command queue after MQTT subscribe:', error);
            });
            this._retryUnpublishedActionResultAcks(true).catch((error) => {
                logger.error('Failed to replay durable action-result acknowledgements:', error);
            });
        }
        await this.subscribe(this._defaultSubscriptionTopics.filter((topic) =>
            !REQUIRED_DASHBOARD_SUBSCRIPTIONS.includes(topic) &&
            !this.subscribedTopics.has(topic)), subscriptionEpoch);
        if (!this.connected || subscriptionEpoch !== this._subscriptionEpoch) return;
        if (this._defaultSubscriptionTopics.some((topic) => !this.subscribedTopics.has(topic))) {
            this._scheduleSubscriptionRetry(subscriptionEpoch);
        } else {
            clearTimeout(this._subscriptionRetryTimer);
            this._subscriptionRetryTimer = null;
    ...
    _scheduleSubscriptionRetry(subscriptionEpoch) {
        if (!this.connected || subscriptionEpoch !== this._subscriptionEpoch ||
            this._subscriptionRetryTimer) return;
        this._subscriptionRetryTimer = setTimeout(() => {
            this._subscriptionRetryTimer = null;
            this._confirmDefaultSubscriptions(subscriptionEpoch).catch((error) => {
                logger.error('Failed to retry dashboard MQTT subscriptions:', error);
                this._scheduleSubscriptionRetry(subscriptionEpoch);
            });
        }, 5000);
        this._subscriptionRetryTimer.unref?.();
    }

    subscribeToDefaultTopics(subscriptionEpoch = this._subscriptionEpoch, requiredOnly = false) {
        const topics = [
            'device/+/status',
            'device/+/heartbeat',
            'device/+/sms/incoming',
        ...
        this._defaultSubscriptionTopics = topics;
        return this.subscribe(topics.filter((topic) =>
            !this.subscribedTopics.has(topic) &&
            (!requiredOnly || REQUIRED_DASHBOARD_SUBSCRIPTIONS.includes(topic))), subscriptionEpoch);
    }

    _firmwareCommandName(command) {
        switch (String(command || '').trim().toLowerCase().replace(/_/g, '-')) {
            case 'sync-sms':
            case 'pull-sms':
            case 'pull-messages':
    ...
    _buildNativeCommandMessage(command, payload = {}, messageId, source, deviceId = '') {
        // Dashboard-only evidence is durable but must not cross the device wire.
        if (payload?._ota) { payload = { ...payload }; delete payload._ota; }
        const normalizedCommand = String(command || '').trim();
        const commandTimeout = payload.timeout === undefined || payload.timeout === null || payload.timeout === ''
            ? null : validatedCommandTimeoutMs(payload.timeout, true);
        ...
        if (normalizedCommand === 'send-sms' || normalizedCommand === 'send-sms-multipart') {
            ...
            // SMS-01 (end-to-end fix): the device-family resolver (smsQueue)
            // requests the plain-text SMS contract explicitly for Android
            // bridges, which read number/text and cannot consume
            // dashboard-built PDUs. The flag lives in the payload so it also
            // survives durable-queue retries (row.payload is stored verbatim).
            // The ESP32 firmware parses the same plain-text shape natively.
            const plainTextSmsRequested = payloadSmsMetadata.sms_plain_text === true ||
                String(payloadSmsMetadata.sms_family || '').trim().toLowerCase() === 'android' ||
                String(payloadSmsMetadata.sms_transport || '').trim().toLowerCase() === 'text';
            const smsMetadata = {
                ...(payloadProvidesPdu || plainTextSmsRequested ? {} : buildSmsTransportMetadataForRecipient(number, text)),
                ...payloadSmsMetadata
            };
            ...
            const hasDashboardPdu = !plainTextSmsRequested &&
                typeof smsMetadata.sms_pdu === 'string' &&
                smsMetadata.sms_pdu.trim() &&
                (payloadProvidesPdu || !metadataPduIsMultipart);
            if (plainTextSmsRequested) {
                const message = {
                    schema: 1,
                ...
                if (smsMetadata.sms_transport_encoding) message.sms_transport_encoding = smsMetadata.sms_transport_encoding;
                ...
                if (commandTimeout > 0) message.timeout = commandTimeout;
                if (payload.sim_slot !== null && payload.sim_slot !== undefined && payload.sim_slot !== '') {
                    message.sim_slot = Number(payload.sim_slot);
                }
                return message;
            }
            // Every native ESP32 command uses the same versioned envelope.
            // Android remains on its separate plain-text adapter contract.
            const message = {
                schema: 1,
                device_id: String(deviceId || '').trim(),
                action_id: messageId,
            ...
            if (hasDashboardPdu) {
                message.payload.sms_pdu = smsMetadata.sms_pdu;
                message.payload.sms_pdu_encoding = smsMetadata.sms_pdu_encoding;
            } else {
                message.payload.number = number;
                message.payload.text = text;
                if (smsMetadata.sms_transport_encoding) {
                    message.payload.sms_transport_encoding = smsMetadata.sms_transport_encoding;
                }
                if (Number.isFinite(Number(smsMetadata.sms_parts)) && Number(smsMetadata.sms_parts) > 0) {
                    message.payload.sms_parts = Number(smsMetadata.sms_parts);
                }
            }
            if (commandTimeout > 0) message.timeout = commandTimeout;
            if (payload.sim_slot !== null && payload.sim_slot !== undefined && payload.sim_slot !== '') {
                message.payload.sim_slot = Number(payload.sim_slot);
            }
            return message;
        }

        const messagePayload = { ...payload };
        delete messagePayload.timeout;
        const message = {
            schema: 1,
            device_id: String(deviceId || '').trim(),
            action_id: messageId,
        ...
        switch (normalizedCommand) {
            case 'send-ussd':
                if (!messagePayload.code && payload.ussd) messagePayload.code = String(payload.ussd).trim();
                break;
            case 'storage-list':
                if (messagePayload.path == null) messagePayload.path = '';
                if (messagePayload.max_entries == null && Number.isFinite(Number(payload.limit))) {
                    messagePayload.max_entries = Number(payload.limit);
                }
                break;
            case 'storage-delete':
                if (!messagePayload.path && Array.isArray(payload.items) && payload.items.length === 1) {
                    messagePayload.path = String(payload.items[0] || '').trim();
                }
                break;
            default:
                break;
    ...
    _normalizeIncomingPayload(topicParts, data) {
        ...
        if (topicParts[2] === 'action' && topicParts[3] === 'result') {
            ...
            const result = String(normalized.result || normalized.status || '').trim().toLowerCase();
            normalized.terminal = ['completed', 'failed', 'rejected', 'timeout', 'error'].includes(result);
            if (normalized.terminal) {
                // Terminal execution result wins over a conflicting envelope flag.
                normalized.success = result === 'completed';
                if (!normalized.success && !normalized.error) {
                    normalized.error = normalized.detail || 'Command failed';
                }
            } else {
                // An accepted/progress envelope is neither success nor failure,
                // even if an older bridge supplied a misleading boolean.
    ...
    _isStalePrimaryStatus(existingStatus, data) {
        const previous = existingStatus?.lastStatus;
        if (!previous || !data || !['device_status', 'status'].includes(String(data.type || '').trim())) {
            return false;
        }
        const previousBootId = String(previous.boot_id || '').trim();
        const incomingBootId = String(data.boot_id || '').trim();
        if (incomingBootId && existingStatus.retiredBootIds?.includes(incomingBootId)) {
            return true;
        }
        const previousSequence = Number(previous.status_sequence);
        const incomingSequence = Number(data.status_sequence);
        if (!previousBootId || !incomingBootId || previousBootId !== incomingBootId ||
            !Number.isSafeInteger(previousSequence) || previousSequence < 1 ||
            !Number.isSafeInteger(incomingSequence) || incomingSequence < 1) {
            return false;
        }
        return incomingSequence <= previousSequence;
    }

    _resolvePendingMessage(deviceId, data) {
        if (isApplyResult(data)) return false;
        if (data?.terminal === false) {
            return false;
        }
        // Only a terminal execution result can complete a waiting command.
        // A status snapshot or an intermediate acknowledgement may share its
        // action ID but is not proof that the requested action finished.
        if (!this._terminalActionResult(data)) {
            return false;
        }
        ...
        if (this._normalizeDeviceId(pending.deviceId) !== this._normalizeDeviceId(deviceId)) {
            return false;
        }
        const responseCommand = String(this._firmwareCommandName(data?.command) || '').trim().toLowerCase().replace(/-/g, '_');
        const expectedCommand = String(this._firmwareCommandName(pending.command) || '').trim().toLowerCase().replace(/-/g, '_');
        if (!responseCommand || !expectedCommand || responseCommand !== expectedCommand) {
            return false;
        }
    ...
    _terminalActionResult(data) {
        const result = String(data?.result || data?.status || '').trim().toLowerCase();
        return ['completed', 'failed', 'rejected', 'timeout', 'error'].includes(result);
    }

    _canonicalActionResult(deviceId, data) {
        return {
            schema_version: Number(data?.schema_version),
            device_id: this._normalizeDeviceId(deviceId),
            action_id: String(data?.action_id || data?.messageId || '').trim(),
            command: String(data?.command || '').trim().toLowerCase().replace(/-/g, '_'),
            result: String(data?.result || data?.status || '').trim().toLowerCase(),
            result_code: Number.isFinite(Number(data?.result_code)) ? Number(data.result_code) : null,
            feature_reason: Number.isFinite(Number(data?.feature_reason)) ? Number(data.feature_reason) : null,
            detail: String(data?.detail || ''),
            created_ms: Number.isFinite(Number(data?.created_ms)) ? Number(data.created_ms) : null,
            timeout_ms: Number.isFinite(Number(data?.timeout_ms)) ? Number(data.timeout_ms) : null,
            payload: data?.payload === undefined ? null : data.payload
        };
    }

    async _publishActionResultAck(record) {
        const payload = {
            schema_version: 1,
        ...
        await this.publish(
            `device/${record.device_id}/command/action-result-ack`,
            payload,
            { qos: 1, retain: false }
        );
        const db = this._db();
        if (this._canDbRun(db)) {
            await this._dbRun(db,
                `UPDATE device_action_results
                 SET broker_published_at = CURRENT_TIMESTAMP,
                     ack_attempts = ack_attempts + 1,
                     last_ack_error = NULL
                 WHERE device_id = ? AND action_id = ? AND command = ?`,
                [record.device_id, record.action_id, record.command]
            );
        }
    }

    async _persistTerminalActionResultAndAck(deviceId, data) {
        if (!this._terminalActionResult(data) || Number(data?.schema_version) !== 1) return false;
        const canonical = this._canonicalActionResult(deviceId, data);
        if (!canonical.device_id || !canonical.action_id || !canonical.command ||
            String(data?.device_id || '').trim() !== canonical.device_id) {
            return false;
        }
        const db = this._db();
        if (!this._hasDbMethods(db, ['get']) || !this._canDbRun(db)) return false;
        const canonicalPayload = JSON.stringify(canonical);
        const insert = await this._dbRun(db,
            `INSERT OR IGNORE INTO device_action_results
                (device_id, action_id, command, schema_version, result, result_code,
                 canonical_payload, received_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`,
            [canonical.device_id, canonical.action_id, canonical.command,
                canonical.schema_version, canonical.result, canonical.result_code,
                canonicalPayload]
        );
        const stored = await db.get(
            `SELECT device_id, action_id, command, canonical_payload
             FROM device_action_results
             WHERE device_id = ? AND action_id = ? AND command = ?`,
            [canonical.device_id, canonical.action_id, canonical.command]
        );
        if (!stored) return false;
        if (stored.canonical_payload !== canonicalPayload) {
            // Firmware may rebuild a replay envelope with different runtime
            // metadata (for example created_ms) while retaining the same
            // action identity and terminal outcome. Acknowledge that replay
            // using the durable first-seen record; only a different outcome
            // is a real conflict that must remain unacknowledged.
            let previous;
            try { previous = JSON.parse(stored.canonical_payload || ''); } catch (_) { previous = null; }
            const equivalentOutcome = previous
                && previous.device_id === canonical.device_id
                && previous.action_id === canonical.action_id
                && previous.command === canonical.command
                && previous.result === canonical.result
                && previous.result_code === canonical.result_code;
            if (!equivalentOutcome) {
                await this._dbRun(db,
                    `UPDATE device_action_results
                     SET conflict_count = conflict_count + 1,
                         last_ack_error = 'conflicting terminal replay'
                     WHERE device_id = ? AND action_id = ? AND command = ?`,
                    [canonical.device_id, canonical.action_id, canonical.command]
                );
                logger.error('Conflicting terminal action result was not acknowledged', {
                    deviceId: canonical.device_id,
                    actionId: canonical.action_id,
                    command: canonical.command,
                    inserted: Number(insert?.changes || 0) > 0
                });
                return false;
            }
        }
        try {
            await this._publishActionResultAck(stored);
            return true;
        } catch (error) {
            await this._dbRun(db,
                `UPDATE device_action_results
                 SET ack_attempts = ack_attempts + 1, last_ack_error = ?
                 WHERE device_id = ? AND action_id = ? AND command = ?`,
                [String(error?.message || 'ack publish failed').slice(0, 240),
                    canonical.device_id, canonical.action_id, canonical.command]
            ).catch(() => {});
            return false;
        }
    ...
    async _retryUnpublishedActionResultAcks(force = false) {
        if (this._resultAckRetryRunning || !this.connected) return;
        const now = Date.now();
        if (!force && now < this._resultAckRetryAt) return;
        this._resultAckRetryAt = now + 10000;
        const db = this._db();
        if (!this._hasDbMethods(db, ['all']) || !this._canDbRun(db)) return;
        this._resultAckRetryRunning = true;
        try {
            const rows = await db.all(
                `SELECT device_id, action_id, command
                 FROM device_action_results
                 WHERE broker_published_at IS NULL
            ...
            for (const row of rows || []) {
                try {
                    await this._publishActionResultAck(row);
                } catch (error) {
                    await this._dbRun(db,
                        `UPDATE device_action_results
                         SET ack_attempts = ack_attempts + 1, last_ack_error = ?
                         WHERE device_id = ? AND action_id = ? AND command = ?`,
                        [String(error?.message || 'ack publish failed').slice(0, 240),
                            row.device_id, row.action_id, row.command]
                    ).catch(() => {});
                }
            }
        } finally {
    ...
    _emitCompatibilityEvents(topicParts, deviceId, data) {
        ...
        if (topicParts[2] === 'action' && topicParts[3] === 'result') {
            // The canonical action/result event was already emitted above.
            // Do not recreate the removed command/response compatibility lane.
            if (!this._terminalActionResult(data)) return;
            ...
            switch (String(data?.command || '').toLowerCase()) {
                ...
                case 'file_delete':
                    ...
                    break;
                case 'storage_info':
                    this.emit('storage:info', deviceId, {
                        ...payload,
                        success: data.success,
                        messageId: data.messageId,
                        detail: data.detail
    ...
    _deviceCommandChannelPendingReason(deviceId) {
        if (this.connected && !this.subscriptionsReady) {
            return 'Dashboard MQTT result subscription is not ready';
        }
    ...
    _requestNativeOtaHealth(deviceId, status) {
        const challenge = status?.ota_validation_action_id;
        if (!this.connected || status?.device_id !== deviceId || status?.type !== 'device_status' ||
            !/^[0-9a-f]{16}$/.test(status?.boot_id || '') ||
            !Number.isInteger(status?.status_sequence) || status.status_sequence < 1 ||
            status.status_sequence > 0xffffffff || status.ota_state !== 'pending_verify' ||
            status.mqtt_connected !== true || status.mqtt_subscribed !== true ||
            !/^ota_[0-9a-f]{24}$/.test(challenge || '')) {
            this.otaHealthRequests.delete(deviceId);
            return;
        }
        const now = Date.now();
        const previous = this.otaHealthRequests.get(deviceId);
        if (previous?.challenge === challenge && previous.boot === status.boot_id &&
            (previous.inflight || previous.sent || now - previous.at < 10000)) return;
        // Bound memory without retry timers that could act on stale status.
        for (const [id, entry] of this.otaHealthRequests) {
            if (now - entry.at > 120000) this.otaHealthRequests.delete(id);
        }
        if (this.otaHealthRequests.size >= 256 && !this.otaHealthRequests.has(deviceId)) return;
        const entry = { challenge, boot: status.boot_id, at: now, inflight: true, sent: false };
        this.otaHealthRequests.set(deviceId, entry);
        // Publish success is not OTA success: the native committed result and
        // exact database ACK must still complete the health exchange.
        Promise.resolve().then(() => {
            if (!this.connected || this.otaHealthRequests.get(deviceId) !== entry) return;
            return this.publishCommand(deviceId, 'get_status', {}, false, 10000, {
                messageId: challenge, skipPersistentQueue: true
            }).then(() => { entry.sent = true; });
        }).catch(() => {
            logger.warn('OTA health request publish failed', { deviceId });
    ...
    handleMessage(topic, message, packet = {}) {
        try {
            ...
            // A broker replay has no trustworthy receive time from the device.
            // Ignore retained runtime events before deduplication so they cannot
            // renew liveness or suppress an identical subsequent live message.
            // Capabilities are static discovery metadata and may be restored
            // without emitting a heartbeat or a current status snapshot.
            if (packet.retain === true) {
                if (topicParts.length === 3 && topicParts[0] === 'device' &&
                    topicParts[2] === 'capabilities' && data && typeof data === 'object' && !Array.isArray(data)) {
                    this.emit('capabilities', topicParts[1], data);
                }
                return;
            }

            logger.debug('MQTT message received', { topic, bytes: Buffer.byteLength(messageStr, 'utf8') });

            // A single action ID legitimately moves from accepted to terminal
            // on the same topic. Hash the wire body so only exact repeats drop.
            const correlationId = data?.messageId || data?.action_id;
            if (correlationId && typeof correlationId === 'string') {
                const bodyHash = crypto.createHash('sha256').update(messageStr).digest('hex');
                const dedupeKey = `${topic}|${correlationId}|${bodyHash}`;
                if (this.seenMessages.has(dedupeKey)) {
                    logger.debug(`Duplicate MQTT message ignored: ${dedupeKey}`);
                    // A durable terminal result is replayed until the device
                    // receives our database acknowledgement. Re-publish that
                    // acknowledgement for an exact replay without re-emitting
                    // the action to application handlers.
                    if (topicParts[2] === 'action' && topicParts[3] === 'result') {
                        const deviceId = topicParts[1];
                        if (!data?.device_id || String(data.device_id).trim() === deviceId) {
                            this._persistTerminalActionResultAndAck(deviceId, data).catch((error) => {
                                logger.error('Failed to acknowledge replayed terminal action result:', error);
                            });
                        }
                    }
                    return;
            ...
            if (topicParts.length >= 2) {
                const deviceId = topicParts[1];
                if (topicParts[2] === 'action' && topicParts[3] === 'result' &&
                    data?.device_id && String(data.device_id).trim() !== deviceId) {
                    logger.warn('Dropped action result with mismatched device identity', {
                        topicDeviceId: deviceId,
                        envelopeDeviceId: String(data.device_id).trim()
                    });
                    return;
                }
                ...
                const staleActionStatus = actionDerivedStatus &&
                    String(data.command || '').trim().toLowerCase() === 'get_status' &&
                    this._isStalePrimaryStatus(existingStatus, actionDerivedStatus);

                if (!actionDerivedStatus && this._isStalePrimaryStatus(existingStatus, data)) {
                    logger.warn('Dropped stale device status snapshot', {
                        deviceId,
                        bootId: String(data.boot_id),
                        statusSequence: Number(data.status_sequence),
                        currentSequence: Number(existingStatus.lastStatus?.status_sequence)
                    });
                    return;
                }
                ...
                const isPrimaryStatus = !staleActionStatus && ((topicParts.length === 3 && topicParts[2] === 'status') ||
                    Boolean(actionDerivedStatus) ||
                    data.type === 'status' ||
                    data.type === 'device_status');
                const previousBootId = String(existingStatus.lastStatus?.boot_id || '').trim();
                const nextBootId = String(primaryStatusData?.boot_id || '').trim();
                const retiredBootIds = isPrimaryStatus && previousBootId && nextBootId && previousBootId !== nextBootId
                    ? [...(existingStatus.retiredBootIds || []), previousBootId].slice(-8)
                    : existingStatus.retiredBootIds;

                // Delayed SMS/call/capability/result events are not heartbeats.
                const establishesLiveness = isPrimaryStatus ||
                    (topicParts.length === 3 && topicParts[2] === 'heartbeat');
                this.deviceStatus.set(deviceId, {
                    ...existingStatus,
                    lastSeen: establishesLiveness ? receivedAt : existingStatus.lastSeen,
                    online: establishesLiveness ? true : this.isDeviceOnline(deviceId),
                    retiredBootIds,
                    lastStatusAt: isPrimaryStatus ? receivedAt : existingStatus.lastStatusAt,
                    lastStatus: isPrimaryStatus ? { ...primaryStatusData } : existingStatus.lastStatus
                });

                if (establishesLiveness) {
                ...
                if (isPrimaryStatus) {
                    if (topicParts.length === 3 && topicParts[0] === 'device' && topicParts[2] === 'status') {
                        this._requestNativeOtaHealth(deviceId, primaryStatusData);
                    }
                    this.emit('status', deviceId, primaryStatusData);
                    const pending = this.pendingMessages.get(String(data.messageId || data.action_id || ''));
                    const fresh = Boolean(actionDerivedStatus && pending &&
                        pending.deviceId === deviceId && this._firmwareCommandName(pending.command) === 'get_status');
                    this.otaLifecycle.observeStatus(deviceId, primaryStatusData, {
                        retained: packet.retain === true, fresh, queryStartedAt: pending?.timestamp
                    }).catch((error) => logger.error('Failed to reconcile OTA boot evidence:', error));
                }

                if (topicParts[2] === 'action' && topicParts[3] === 'result') {
                    this._resolvePendingMessage(deviceId, data);
                    this._persistTerminalActionResultAndAck(deviceId, data).catch((error) => {
                        logger.error('Failed to persist terminal action result:', error);
                    });
                    ...
                    const deliveryData = topicParts[3] === 'delivered'
                        ...
                        : data;
                    // Handlers consume the same ordered promise rather than
                    // independently replaying the report during event emission.
                    Object.defineProperty(data, '_deliveryResult', { enumerable: false,
                        value: this._settlePersistentQueueFromSmsDelivery(deviceId, deliveryData) });
                    data._deliveryResult.catch((error) => {
                ...
                if (topicParts.length >= 4) {
                    ...
                    if (topicParts[3] !== 'heartbeat' && (topicParts[3] !== 'status' || !isPrimaryStatus)) {
                        this.emit(topicParts[3], deviceId, data);
                    }
                } else if (topicParts.length >= 3) {
                    // Format: device/deviceId/type
                    const eventName = topicParts[2];
                    // The primary snapshot above already emitted this event.
                    // Preserve other topic events without doubling status ingestion.
                    if (eventName !== 'heartbeat' && (eventName !== 'status' || !isPrimaryStatus)) {
                        this.emit(eventName, deviceId, data);
                    }
                }
    ...
    async subscribe(topics, subscriptionEpoch = this._subscriptionEpoch) {
        ...
        const results = await Promise.all(topicArray.map((topic) => new Promise((resolve) => {
            let settled = false;
            ...
            const timer = setTimeout(() => {
                logger.warn(`Dashboard MQTT subscription timed out: ${topic}`);
                // The old broker attempt may time out after a new session has
                // already confirmed the same topic. Never revoke that grant.
                if (subscriptionEpoch === this._subscriptionEpoch && client === this.client && this.connected) {
            ...
            try {
                client.subscribe(topic, { qos: 1 }, (error, granted) => {
                    ...
                    const accepted = !error && Array.isArray(granted) &&
                        granted.some((entry) => entry?.topic === topic &&
                            Number.isInteger(entry.qos) && entry.qos >= 0 && entry.qos <= 2);
                    if (accepted) {
                        this.subscribedTopics.add(topic);
                        logger.info(`📡 Subscribed to ${topic}`);
                    } else {
                        this.subscribedTopics.delete(topic);
                        logger.warn(`Dashboard MQTT subscription not acknowledged: ${topic}`);
                    }
                    finish(accepted);
                });
            } catch (error) {
                logger.warn(`Dashboard MQTT subscribe failed: ${topic}`);
                this.subscribedTopics.delete(topic);
                finish(false);
            }
        })));
    ...
    resubscribe() {
        return this.subscribe([...this.subscribedTopics]);
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            this.client.publish(topic, payload, options, (err) => {
                if (err) {
                    ...
                    // Serialized command bodies bypass key-based redaction.
                    // Log transport metadata only (profiles contain credentials).
                    logger.debug('MQTT publish completed', { topic, bytes: Buffer.byteLength(payload, 'utf8') });
    ...
    _restoreBusyAfterPublishFailure(deviceId, previousUntil, markedUntil) {
        const normalized = this._normalizeDeviceId(deviceId);
        if (!normalized || markedUntil <= previousUntil ||
            this.deviceBusyUntil.get(normalized) !== markedUntil) return;
        // Another in-flight command may still require polling suppression.
        if ([...this.pendingMessages.values()].some((pending) =>
            this._normalizeDeviceId(pending.deviceId) === normalized &&
            this._busyDurationMs(pending.command) > 0)) return;
        if (previousUntil > Date.now()) {
            this.deviceBusyUntil.set(normalized, previousUntil);
        } else {
            this.clearDeviceBusy(normalized);
        }
    }
    ...
    _normalizeCommandMessageId(command, value) {
        ...
        if (raw.length <= FIRMWARE_ACTION_ID_MAX_LENGTH && /^[A-Za-z0-9_.:-]+$/.test(raw)) {
            return raw;
        }
    ...
    _commandExpiryTimestamp(timeout, options = {}) {
        const requested = Number(options.admissionTtlMs);
        const floor = Math.max(30000, Number(timeout) || 30000);
        const ttl = Number.isFinite(requested)
            ? Math.min(MAX_COMMAND_ADMISSION_TTL_MS, Math.max(floor, requested))
            : Math.max(floor, DEFAULT_COMMAND_ADMISSION_TTL_MS);
        return this._sqlTimestamp(Date.now() + ttl);
    }
    ...
    _isReplaySafeCommand(command, options = {}) {
        if (String(command || '').trim() === 'ota-update') return false;
    ...
    async _usesAsyncDurableResultForRow(row) {
        if (row?.command === 'ota-update') return true;
    ...
    _publishAwaitingTerminal(deviceId, command, payload, timeout, messageId, topic, message) {
        if (this.pendingMessages.has(messageId)) {
            const error = new Error('Command action ID is already pending');
            error.code = 'DUPLICATE_ACTION_ID';
            return Promise.reject(error);
        }
        if (this.pendingMessages.size >= 100) {
            return Promise.reject(new Error('Command queue full - device may be offline'));
        }

        const previousBusyUntil = this.deviceBusyUntil.get(deviceId) || 0;
        this.markDeviceBusy(deviceId, command);
        const markedBusyUntil = this.deviceBusyUntil.get(deviceId) || 0;
        return new Promise((resolve, reject) => {
            const pending = {
                command, deviceId, payload, timestamp: Date.now(),
                resolve, reject, timeout: null
            };
            const timeoutId = setTimeout(() => {
                if (this.pendingMessages.get(messageId) !== pending) return;
            ...
            this.publish(topic, message).catch((error) => {
                // A late publish callback must not erase a newer request that
                // reused this ID after the first terminal result or timeout.
                if (this.pendingMessages.get(messageId) !== pending) return;
                clearTimeout(timeoutId);
                this.pendingMessages.delete(messageId);
                this._restoreBusyAfterPublishFailure(deviceId, previousBusyUntil, markedBusyUntil);
                reject(error);
            });
        });
    }
    ...
    async _publishCommandNow(deviceId, command, payload = {}, waitForResponse = false, timeout = 30000, options = {}) {
        timeout = validatedCommandTimeoutMs(timeout);
        if (!this.connected) {
            return Promise.reject(new Error('MQTT not connected'));
        }
        if (!this.subscriptionsReady) {
            const error = new Error('Dashboard MQTT result subscription is not ready');
            error.code = 'MQTT_SUBSCRIPTION_NOT_READY';
            throw error;
        }
        ...
        if ((normalizedCommand === 'send-sms' || normalizedCommand === 'send-sms-multipart') &&
            (normalizedPayload.timeout === undefined || normalizedPayload.timeout === null ||
                normalizedPayload.timeout === '' || normalizedPayload.timeout === 0)) {
            normalizedPayload.timeout = timeout;
        }
        const topicCommand = this._firmwareCommandName(command);
        const topic = `device/${deviceId}/command/${topicCommand}`;
        const messageId = this._normalizeCommandMessageId(command, options.messageId);
        const message = this._buildNativeCommandMessage(command, normalizedPayload, messageId, options.source || 'dashboard', deviceId);

        if (waitForResponse) {
            return this._publishAwaitingTerminal(deviceId, command, normalizedPayload,
                timeout, messageId, topic, message);
        }
    ...
    async _recoverPersistentQueue() {
        ...
        // Earlier versions classified OTA as replay-safe. An interrupted OTA
        // must be reconciled against its next boot, never automatically flashed
        // again (which could overwrite the recovery image).
        await this._dbRun(db, `UPDATE device_command_queue SET replay_safe = 0
            WHERE command = 'ota-update' AND replay_safe <> 0`);
    ...
    _buildStatusFromActionResult(deviceId, data) {
        if (String(data?.result || '').toLowerCase() !== 'completed') return null;
        ...
        if (command === 'mobile_toggle') {
            ...
            return {
                ...
                // Opening the standby cellular bearer does not change the
                // active route while primary Wi-Fi still has an address.
    ...
    async _resolveSmsStatusFromQueueTracking(row, tracking, requestedStatus) {
        if (!tracking?.partCount || !tracking.baseMessageId) {
        ...
        const rows = await db.all(
            `SELECT message_id, status, payload, delivery_status
             FROM device_command_queue
             WHERE device_id = ?
        ...
        for (const candidate of rows || []) {
            ...
            const member = this._extractSmsTracking({ ...candidate, command: row.command });
            if (!member || member.baseMessageId !== tracking.baseMessageId || member.smsId !== tracking.smsId ||
                member.partCount !== tracking.partCount || !Number.isInteger(member.partIndex) ||
                member.partIndex < 1 || member.partIndex > tracking.partCount ||
                candidate.message_id !== `${tracking.baseMessageId}_p${member.partIndex}`) continue;
            const candidateStatus = String(candidate?.status || '').trim().toLowerCase();
            if (candidateStatus === 'failed' || candidate.delivery_status === 'failed') return 'failed';
        ...
        if (requestedStatus !== 'sent') return requestedStatus;
    ...
    async _syncSmsStatusFromQueueRow(row, explicitStatus = null, explicitError = null) {
        ...
        let nextStatus = await this._resolveSmsStatusFromQueueTracking(row, tracking, requestedStatus);
        if (typeof db.get === 'function') {
            const delivery = await db.get('SELECT delivery_status FROM device_command_queue WHERE id = ?', [row.id]);
            if (delivery?.delivery_status === 'failed') nextStatus = 'failed';
        }
        ...
        if (tracking.smsId) {
            await this._dbRun(db,
                ...
                [nextStatus, nextError, tracking.baseMessageId || tracking.messageId, tracking.smsId, row.device_id]
            );
            await this._refreshSmsConversationStatus(db, tracking.smsId);
            return;
        }
        ...
        if (tracking.messageId) {
            await this._dbRun(db,
                `UPDATE sms
                 SET status = CASE WHEN status = 'delivered' THEN status ELSE ? END,
                     error = CASE WHEN status = 'delivered' THEN NULL ELSE ? END
                 WHERE external_id = ? AND device_id = ?`,
                [nextStatus, nextError, tracking.messageId, row.device_id]
            );
            if (this._hasDbMethods(db, ['get'])) {
                const sms = await db.get(
                    'SELECT id FROM sms WHERE external_id = ? AND device_id = ? LIMIT 1',
                    [tracking.messageId, row.device_id]
                );
                await this._refreshSmsConversationStatus(db, sms?.id);
            }
        }
    }

    async _refreshSmsConversationStatus(db, smsId) {
        if (!smsId || !this._hasDbMethods(db, ['get', 'all']) || !this._canDbRun(db)) return;
        try {
            await refreshSmsConversationBySmsId(db, smsId);
        } catch (error) {
            // Queue settlement is hardware truth; a denormalized conversation
            // preview must never turn a completed command back into a retry.
            logger.warn(`SMS conversation status refresh failed for row ${smsId}: ${error.message || error}`);
        }
    }
    ...
    async _resolvePersistentQueueWaiter(row) {
        // A concurrent cleanup/reconciliation may have removed the queue row
        // after the status transition. There is no waiter to settle in that
        // case; never let a background timer crash the queue worker.
    ...
    async _settlePersistentQueueFromResponse(deviceId, data) {
        ...
        if (!row) return false;
        if (row.command === 'ota-update') {
            if (this._firmwareCommandName(data.command) !== 'ota_update') return false;
            if (await this.otaLifecycle.handleAction(row, data)) return true;
        }
        ...
        const result = String(data?.result || data?.status || '').trim().toLowerCase();
        const terminal = ['completed', 'failed', 'rejected', 'timeout', 'error'].includes(result);
        if (!terminal) {
            return false;
        }
        // The terminal result is authoritative even when an older producer
        // includes a contradictory `success` envelope flag.
    ...
    async _findPersistentSmsQueueRow(deviceId, data) {
        ...
        if (correlationId) {
            ...
            /* Completed execution rows are operator-cleanable, while carrier
             * delivery reports can arrive days later. An exact message ID is
             * the strongest correlation key, so recover it from the compact
             * retention table before declaring the report unmatched. */
            try {
                const archived = await db.get(
                    `SELECT *
                     FROM sms_delivery_correlations
                     WHERE device_id = ?
                       AND message_id = ?
                ...
                if (archived) {
                    return {
                        id: archived.queue_id,
                        device_id: archived.device_id,
                        command: Number(archived.part_count || 0) > 1
                            ? 'send-sms-multipart'
                            : 'send-sms',
                        message_id: archived.message_id,
                        status: 'completed',
                        archived_correlation: true,
                        payload: JSON.stringify({
                            smsId: archived.sms_id,
                            sms_base_message_id: archived.base_message_id,
                            sms_part_index: archived.part_index,
                            sms_part_count: archived.part_count,
                            to: archived.to_number
                        })
                    };
                }
            } catch (error) {
                if (!/no such table:\s*sms_delivery_correlations/i.test(String(error?.message || error))) {
                    throw error;
                }
            }
            return null;
        ...
        if (deliveryReport.messageReference !== null) {
            ...
            let archived = null;
            try {
                archived = await db.get(
                    `SELECT *, 1 AS archived_correlation,
                            COUNT(*) OVER () AS candidate_count,
                            COUNT(*) OVER (PARTITION BY COALESCE(to_number, '')) AS recipient_candidate_count
                     FROM sms_delivery_correlations
                     WHERE device_id = ?
                       AND message_reference = ?
                       AND datetime(expires_at) > CURRENT_TIMESTAMP
                     ORDER BY CASE WHEN to_number = ? THEN 0 ELSE 1 END,
                              datetime(created_at) DESC
                     LIMIT 1`,
                    [normalizedDeviceId, deliveryReport.messageReference, deliveredTo]
                );
            } catch (error) {
                if (!/no such table:\s*sms_delivery_correlations/i.test(String(error?.message || error))) {
                    throw error;
            ...
            if (archived?.archived_correlation) {
                const archivedTo = String(archived.to_number || '').trim();
                const normalizedArchivedTo = formatPhoneNumber(archivedTo) || archivedTo;
                const normalizedDeliveredTo = formatPhoneNumber(deliveredTo) || deliveredTo;
                const exactRecipient = deliveredTo && normalizedArchivedTo === normalizedDeliveredTo;
                if (Number(archived.candidate_count) === 1 ||
                    (exactRecipient && Number(archived.recipient_candidate_count) === 1)) {
                    return {
                        id: archived.queue_id,
                        device_id: archived.device_id,
                        command: archived.part_count > 1 ? 'send-sms-multipart' : 'send-sms',
                        message_id: archived.message_id,
                        status: 'completed',
                        archived_correlation: true,
                        payload: JSON.stringify({
            ...
            // A report with an explicit modem reference must never settle an
            // unrelated send merely because its destination matches.
            return null;
        }
    ...
    async _settlePersistentQueueFromSmsDelivery(deviceId, data) {
        return this.processSmsDelivery(deviceId, data);
    }

    async processSmsDelivery(deviceId, data) {
        this._smsDeliveryChains ||= new Map();
        const previous = this._smsDeliveryChains.get(deviceId) || Promise.resolve();
        const work = previous.catch(() => {}).then(() => this._persistSmsDelivery(deviceId, data));
        this._smsDeliveryChains.set(deviceId, work);
        try { return await work; } finally {
            if (this._smsDeliveryChains.get(deviceId) === work) this._smsDeliveryChains.delete(deviceId);
        }
    }

    async _persistSmsDelivery(deviceId, data) {
        const normalized = normalizeSmsDeliveryPayload(data);
        const row = await this._findPersistentSmsQueueRow(deviceId, normalized);
        const tracking = this._extractSmsTracking(row);
        if (!row || !tracking) return { ...normalized, correlation: 'unmatched', status: 'pending' };
        // A destination alone cannot identify a particular multipart segment.
        if (tracking.partCount && !normalized.messageId && !normalized.action_id &&
            normalizeSmsDeliveryReport(normalized).messageReference === null) {
            return { ...normalized, correlation: 'unmatched', status: 'pending' };
        }
        const db = this._db();
        try {
            await this._dbRun(db, `UPDATE sms_delivery_correlations SET
                delivery_status = CASE WHEN delivery_status IN ('delivered', 'failed') THEN delivery_status ELSE ? END,
                delivery_report = CASE WHEN delivery_status IN ('delivered', 'failed') THEN delivery_report ELSE ? END,
                delivery_at = COALESCE(delivery_at, CURRENT_TIMESTAMP)
                WHERE device_id = ? AND message_id = ?`,
            [normalized.status, JSON.stringify(normalized), row.device_id, row.message_id]);
        } catch (error) {
            if (!/no such table:\s*sms_delivery_correlations/i.test(String(error?.message || error))) {
                throw error;
            }
        }
        if (!row.archived_correlation) {
            await this._dbRun(db, `UPDATE device_command_queue SET
                delivery_status = CASE WHEN delivery_status IN ('delivered', 'failed') THEN delivery_status ELSE ? END,
                delivery_report = CASE WHEN delivery_status IN ('delivered', 'failed') THEN delivery_report ELSE ? END,
                delivery_at = COALESCE(delivery_at, CURRENT_TIMESTAMP) WHERE id = ?`,
            [normalized.status, JSON.stringify(normalized), row.id]);
        }
        // Keep execution waiters/recovery semantics, but preserve delivery evidence
        // in its own columns so later action results cannot overwrite it.
        if (normalized.status !== 'pending' && !row.archived_correlation) {
            await this._legacySettlePersistentQueueFromSmsDelivery(deviceId, normalized);
        }
        let candidates = [];
        try {
            candidates = await db.all(`SELECT message_id, delivery_status,
                    CASE WHEN part_count > 1 THEN 'send-sms-multipart' ELSE 'send-sms' END AS command,
                    'completed' AS status,
                    sms_id, base_message_id, part_index, part_count,
                    json_object('smsId', sms_id, 'sms_base_message_id', base_message_id,
                        'sms_part_index', part_index, 'sms_part_count', part_count) AS payload
                FROM sms_delivery_correlations
                WHERE device_id = ? AND base_message_id = ?
                  AND datetime(expires_at) > CURRENT_TIMESTAMP`,
            [deviceId, tracking.baseMessageId]);
        } catch (error) {
            if (!/no such table:\s*sms_delivery_correlations/i.test(String(error?.message || error))) {
                throw error;
            }
        ...
        if (!Array.isArray(candidates) || candidates.length === 0) {
            candidates = await db.all(`SELECT * FROM device_command_queue
                WHERE device_id = ? AND command IN ('send-sms', 'send-sms-multipart')
                AND (message_id = ? OR substr(message_id, 1, ?) = ?)`,
            [deviceId, tracking.baseMessageId, tracking.baseMessageId.length + 2, `${tracking.baseMessageId}_p`]);
        }
        const parts = new Map();
        const conflictingIndices = new Set();
        for (const candidate of candidates) {
            const member = this._extractSmsTracking(candidate);
            if (!member || member.baseMessageId !== tracking.baseMessageId || member.smsId !== tracking.smsId ||
                member.partCount !== tracking.partCount) continue;
            const index = member.partIndex || 1;
            if (index < 1 || index > (tracking.partCount || 1) || !Number.isInteger(index)) continue;
            if (candidate.message_id !== (tracking.partCount ? `${tracking.baseMessageId}_p${index}` : tracking.baseMessageId)) continue;
            if (parts.has(index) || conflictingIndices.has(index)) {
                conflictingIndices.add(index);
                parts.delete(index);
            } else parts.set(index, candidate);
        }
        const expected = tracking.partCount || 1;
        let status = 'sent';
        if ([...parts.values()].some(part => part.delivery_status === 'failed' || part.status === 'failed')) status = 'failed';
        else if (parts.size === expected && [...parts.values()].every(part => part.delivery_status === 'delivered')) status = 'delivered';
        else if (parts.size !== expected || [...parts.values()].some(part => part.status !== 'completed' && part.delivery_status !== 'delivered')) status = 'sending';
        const error = status === 'failed' ? 'One or more SMS segments failed' : null;
        await this._dbRun(db, `UPDATE sms SET
            ...
            WHERE device_id = ? AND ${tracking.smsId ? 'id' : 'external_id'} = ?`,
        [status, error, status, deviceId, tracking.smsId || tracking.baseMessageId]);
        const persisted = await db.get(`SELECT id, status, error FROM sms WHERE device_id = ? AND ${tracking.smsId ? 'id' : 'external_id'} = ?`,
            [deviceId, tracking.smsId || tracking.baseMessageId]);
        await this._refreshSmsConversationStatus(db, persisted?.id);
        return { ...normalized, id: tracking.smsId, messageId: tracking.baseMessageId,
            status: persisted?.status || status, error: persisted?.error || null, correlation: 'matched' };
    }

    async _legacySettlePersistentQueueFromSmsDelivery(deviceId, data) {
    ...
    async _processPersistentQueueRow(row) {
        ...
        const run = async () => {
            ...
            try {
                ...
                const payload = row.payload ? JSON.parse(row.payload) : {};
                if (row.command === 'ota-update') {
                    const source = this.deviceStatus.get(row.device_id);
                    if (!validOtaMetadata(payload._ota) || source?.lastStatus?.boot_id !== payload._ota.source_boot_id ||
                        source?.lastStatus?.ota_state !== 'valid') {
                        await this._markPersistentQueueAmbiguous(row,
                            new Error('OTA source boot changed or image proof is missing; no flash was dispatched'));
                        return;
                    }
                }
    ...
    async processPersistentQueue() {
        ...
        this._retryUnpublishedActionResultAcks().catch((error) => {
            logger.error('Failed to retry action-result acknowledgements:', error);
        });
        const db = this._db();
        if (!this._hasDbMethods(db, ['all'])) return;
        ...
        try {
            ...
            await this._expirePersistentQueueRows();
            ...
            for (const row of rows) {
                ...
                if (!this.connected || !this.subscriptionsReady) continue;
    ...
    async _expirePersistentQueueRows(limit = 100) {
        const db = this._db();
        if (!this._hasDbMethods(db, ['all']) || !this._canDbRun(db)) return;
        const rows = await db.all(
            `SELECT * FROM device_command_queue
             WHERE status = 'pending'
        ...
        for (const row of rows || []) {
            const detail = 'Command expired before device execution';
            await this._updatePersistentQueueRow(row.id, {
                status: 'failed', completed_at: this._sqlTimestamp(),
                next_attempt_at: null, last_error: detail
            });
            await this._syncSmsStatusFromQueueRow({ ...row, status: 'failed', last_error: detail }, 'failed', detail);
            const failed = this._hasDbMethods(db, ['get'])
                ? await db.get('SELECT * FROM device_command_queue WHERE id = ?', [row.id])
                : { ...row, status: 'failed', last_error: detail };
            await this._resolvePersistentQueueWaiter(failed);
            await this._emitDeviceQueueState(row.device_id);
        }
    }
    ...
    async enqueuePersistentDeviceCommand(deviceId, command, payload = {}, waitForResponse = false, timeout = 30000, options = {}) {
        ...
        const db = this._db();
        if (!this._canDbRun(db)) {
            const error = new Error('Durable device command queue is unavailable');
            error.code = 'DURABLE_QUEUE_UNAVAILABLE';
            throw error;
        }

        const normalizedPayload = normalizeMqttContractPayload(payload);
        if (normalizedPayload.timeout !== undefined && normalizedPayload.timeout !== null && normalizedPayload.timeout !== '') {
            validatedCommandTimeoutMs(normalizedPayload.timeout, true);
        }
        const queueId = this._generatePersistentQueueId();
        if (command === 'ota-update' && !validOtaMetadata(normalizedPayload._ota)) {
            throw new Error('OTA requires a staged image identity and fresh source boot evidence');
        }
        const messageId = this._normalizeCommandMessageId(command, options.messageId);
        const requiresResponse = this._requiresDurableAck(command, waitForResponse, options);
        const nowSql = this._sqlTimestamp();
        const expiresAt = this._commandExpiryTimestamp(timeout, options);
    ...
    async getDeviceQueueState(deviceId) {
        ...
        if (!normalized || !this._hasDbMethods(db, ['all'])) {
            return {
                summary: { pending: 0, active: 0, failed: 0, failedHistory: 0, ambiguous: 0, totalOpen: 0 },
                domains: {},
        ...
        // `failed` is retained as an audit/history count. Failed rows are
        // terminal and are not selected by processPersistentQueue(), so they
        // must not make the live queue appear open or busy.
        const summary = { pending: 0, active: 0, failed: 0, failedHistory: 0, ambiguous: 0, totalOpen: 0 };
        const domains = {};
        for (const row of counts) {
            ...
            if (row.status === 'pending') summary.pending += count;
                ...
                summary.failedHistory += count;
            }
            else if (row.status === 'ambiguous') summary.ambiguous += count;

            const domain = this._commandDomain(row.command);
            if (!domains[domain]) {
                domains[domain] = { pending: 0, active: 0, failed: 0, failedHistory: 0, ambiguous: 0, totalOpen: 0 };
            }
            if (row.status === 'pending') domains[domain].pending += count;
                ...
                domains[domain].failed += count;
                domains[domain].failedHistory += count;
            }
            else if (row.status === 'ambiguous') domains[domain].ambiguous += count;
        }
        summary.totalOpen = summary.pending + summary.active + summary.ambiguous;
        for (const domain of Object.keys(domains)) {
            const entry = domains[domain];
            entry.totalOpen = entry.pending + entry.active + entry.ambiguous;
        }
    ...
    publishCommand(deviceId, command, payload = {}, waitForResponse = false, timeout = 30000, options = {}) {
        ...
        const topicCommand = this._firmwareCommandName(command);
        const topic = `device/${normalizedDeviceId}/command/${topicCommand}`;
        const messageId = this._normalizeCommandMessageId(command, options?.messageId);
        const executePublish = async () => {
            const message = this._buildNativeCommandMessage(
                command,
            ...
            if (waitForResponse) {
                return this._publishAwaitingTerminal(normalizedDeviceId, command, normalizedPayload,
                    timeout, messageId, topic, message);
            }

            // Just publish without waiting for response
            const previousBusyUntil = this.deviceBusyUntil.get(normalizedDeviceId) || 0;
            this.markDeviceBusy(normalizedDeviceId, command);
            const markedBusyUntil = this.deviceBusyUntil.get(normalizedDeviceId) || 0;
            return this.publish(topic, message)
                .then(result => ({ ...result, messageId }))
                .catch((error) => {
                    this._restoreBusyAfterPublishFailure(normalizedDeviceId, previousBusyUntil, markedBusyUntil);
                    throw error;
                });
        };
    ...
    hasFreshStatusSnapshot(deviceId, maxAgeMs = 2 * 60 * 1000) {
        const device = this.deviceStatus.get(deviceId);
        if (!device?.lastStatus || typeof device.lastStatus !== 'object') {
            return false;
        }

        const receivedAt = Date.parse(String(device.lastStatusAt || ''));
        const allowedAgeMs = Number(maxAgeMs);
        if (!Number.isFinite(receivedAt) || !Number.isFinite(allowedAgeMs) || allowedAgeMs < 0) {
            return false;
        }

        return (Date.now() - receivedAt) <= allowedAgeMs;
    }
