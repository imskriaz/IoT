...
// ─── Egress policy (SEC-03) ───────────────────────────────────────────────
const ALLOWED_HTTP_METHODS = new Set(['GET', 'HEAD', 'POST', 'PUT', 'PATCH', 'DELETE']);
const MAX_HTTP_RESPONSE_BYTES = 1024 * 1024; // 1MB

function automationAllowHttp() {
    return String(process.env.AUTOMATION_ALLOW_HTTP || '').trim().toLowerCase() === 'true';
}
...
async function assertPublicHttpsTarget(rawUrl) {
    let parsed;
    try { parsed = new URL(rawUrl); } catch { throw new Error('Invalid integration URL'); }
    const isHttps = parsed.protocol === 'https:';
    if (!isHttps && !(parsed.protocol === 'http:' && automationAllowHttp())) {
        throw new Error('Automation HTTP requests must target HTTPS (set AUTOMATION_ALLOW_HTTP=true for local integrations)');
    }
    if (parsed.username || parsed.password) throw new Error('Credentials-in-URL integrations are not allowed');
    const port = parsed.port ? Number(parsed.port) : (isHttps ? 443 : 80);
    if (port !== 80 && port !== 443 && port !== 8080 && port !== 8443) {
        throw new Error(`Integration target port ${port} is not allowed`);
    }
    const hostname = parsed.hostname.replace(/^\[|\]$/g, '');
    if (net.isIP(hostname)) {
        if (isPrivateAddress(hostname)) throw new Error('Integration targets private addresses are blocked');
        return;
    }
    const records = await dns.promises.lookup(hostname, { all: true }).catch(() => []);
    if (!records.length) throw new Error('Integration host does not resolve');
    if (records.some(record => isPrivateAddress(record.address))) {
        throw new Error('Integration host resolves to a private address');
    }
}

/** Device-plane topics are reserved for the dashboard's command/event
 * pipeline; integrations must not spoof device commands, results or events. */
function assertIntegrationTopicAllowed(topic) {
    const normalized = String(topic || '').trim();
    if (!normalized) throw new Error('Integration topic is required');
    if (/^device\/[^/]+\//.test(normalized) || normalized.startsWith('$SYS')) {
        throw new Error('Publishing to device-plane or system topics is not allowed; use device action nodes');
    }
    return normalized;
}
...
class AutomationEngine {
    ...
    async _publish(topic, payload) {
        if (!this.mqttService?.connected) throw new Error('MQTT not connected');
        const safeTopic = assertIntegrationTopicAllowed(topic);
        const message = typeof payload === 'string' ? payload : JSON.stringify(payload || {});
        await this.mqttService.publish(safeTopic, message);
    }

    async _httpRequest({ url, method = 'POST', headers = {}, body = undefined, timeoutMs = 10000 }) {
        ...
        if (!ALLOWED_HTTP_METHODS.has(normalizedMethod)) {
            throw new Error(`HTTP method ${normalizedMethod} is not allowed`);
        }
        // Egress policy: validated public HTTPS destination (SSRF guard).
        await assertPublicHttpsTarget(url);

        // Drop hop-by-hop / sensitive request headers set by flow configs.
        const safeHeaders = {};
        for (const [key, value] of Object.entries(headers || {})) {
            const lower = String(key).toLowerCase();
            if (['host', 'connection', 'content-length', 'transfer-encoding', 'authorization', 'cookie'].includes(lower)) continue;
        ...
        const options = {
            ...
            headers: { ...safeHeaders },
            signal: AbortSignal.timeout(Math.min(Math.max(Number(timeoutMs) || 10000, 1000), 30000))
        };

        if (!['GET', 'HEAD'].includes(normalizedMethod) && body !== undefined) {
            const serialized = typeof body === 'string' ? body : JSON.stringify(body);
            if (Buffer.byteLength(serialized) > MAX_HTTP_RESPONSE_BYTES) {
                throw new Error('Request body exceeds the 1MB automation limit');
            }
        ...
        if (Buffer.byteLength(text) > MAX_HTTP_RESPONSE_BYTES) {
            throw new Error('Response exceeds the 1MB automation limit');
        }
        let parsed = text;
    ...
    async _callDeviceApi(deviceId, cfg, context) {
        ...
        const localIp = String(profile.local_ip).trim();
        if (!net.isIP(localIp)) {
            throw new Error('Device local IP is invalid');
        }
        if (!net.isIPv4(localIp) || !isPrivateAddress(localIp)) {
            throw new Error('Device local API must target a LAN address');
        }
        if (/(?:\.\.|[:@]|:\/\/|[\\])/i.test(rawPath)) {
            throw new Error('Device API path must be a simple relative path');
        }
        const url = `http://${localIp}/${rawPath}`;
