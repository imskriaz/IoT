    ...
    return (Array.isArray(rows) ? rows : [])
        .map((row) => {
            ...
            return {
                ...
                simNumber,
                numberSource: simNumber ? 'stored' : null,
...
function applyStoredSimFallback(deviceStatus = {}, storedRows = []) {
    ...
    existingSlots.forEach((slot) => {
        ...
        if (Number.isFinite(slotIndex) && slotIndex >= 0) {
            const slotNumber = String(slot?.number || slot?.simNumber || slot?.subscriberNumber || '').trim();
            slotMap.set(slotIndex, {
                ...slot,
                numberSource: slot?.numberSource || slot?.number_source || (slotNumber ? 'device' : null)
            });
        }
    });
    ...
    storedSlots.forEach((storedSlot) => {
        ...
        const currentNumber = String(current.number || current.simNumber || current.subscriberNumber || '').trim();
        slotMap.set(storedSlot.slotIndex, {
            ...
            numberSource: current.numberSource || (currentNumber ? 'device' : storedSlot.numberSource)
        });
    });
    ...
    const selectedSlot = mergedSlots.find((slot) => slot.slotIndex === activeSlotIndex)
        || mergedSlots[0]
        || null;
    const liveTopLevelNumber = String(deviceStatus.simNumber || deviceStatus.subscriberNumber || '').trim();
    const resolvedNumberSource = deviceStatus.simNumberSource
        || deviceStatus.subscriberNumberSource
        || deviceStatus?.sim?.numberSource
        || (liveTopLevelNumber ? 'device' : selectedSlot?.numberSource)
        || null;
    ...
    return {
        ...
        simNumberSource: resolvedNumberSource,
        subscriberNumberSource: resolvedNumberSource,
        ...
        sim: {
            ...
            numberSource: resolvedNumberSource,
