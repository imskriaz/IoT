...
async function refreshSmsConversation(db, conversationId) {
    ...
    const rows = await db.all(
        `SELECT id, device_id, from_number, to_number, message, timestamp, type, status,
                read, source, sim_slot, multipart_ref, multipart_part_index,
                multipart_part_count, multipart_group_key
         FROM sms
         WHERE conversation_id = ?
         ORDER BY datetime(timestamp) ASC, id ASC`,
        [conversationId]
    );
    const messages = conversationDisplayMessages(rows);
    ...
    await db.run(
        ...
        [
            messages.length,
            messages.filter((row) => String(row.type || '').toLowerCase() !== 'outgoing' && !row.read).length,
            latest.id,
