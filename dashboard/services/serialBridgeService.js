 ...
 *
 * Reads the ESP32 provisioning console for explicit diagnostics and recovery.
 * Serial observations must never renew MQTT liveness or overwrite runtime status.
 */
...
class SerialBridgeService extends EventEmitter {
    ...
    async _withCommandPort(callback, options = {}) {
        ...
        const pollingWasActive = alreadyOpen && !!this._pollTimer;
    ...
    async _captureSnapshot(deviceId) {
        ...
        if (fastStatusJsonLine) {
            ...
            logger.info('[SerialBridge] Snapshot updated', {
                ...
                ip: snapshot.modem_data_ip || snapshot.modem_ip_address || snapshot.modem_pdp_ip_address || snapshot.mobile?.ipAddress || snapshot.wifi_ip_address || snapshot.wifi?.ipAddress || null
            });
            this._emitSerialObservation('serial_console_status');
            return snapshot;
        }
    ...
    _emitSerialObservation(reason) {
        const deviceId = this._getTargetDeviceId();
        if (!deviceId) return;
        const timestamp = new Date().toISOString();
        try {
            const room = global.io?.to?.('device:' + deviceId);
            room?.emit?.('device:serial-observation', {
                deviceId,
                source: 'serial_debug',
                reason,
                timestamp
