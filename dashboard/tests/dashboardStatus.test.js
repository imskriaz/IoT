...
describe('dashboardStatus', () => {
    test('a timestamp or bearer hint without explicit online state cannot create an online device', () => {
        const status = buildDashboardDeviceStatus({
            lastSeen: new Date().toISOString(),
            activePath: 'wifi',
            wifi: { connected: true, ipAddress: '192.168.1.20' },
            mqtt: { connected: true, subscribed: true }
        });
        expect(status.online).toBe(false);
        expect(status.mqtt).toEqual(expect.objectContaining({ connected: false, subscribed: false }));
        expect(status.wifi).toEqual(expect.objectContaining({ connected: false }));
        expect(buildDashboardDeviceStatus({ online: false, lastSeen: new Date().toISOString() }, true).online).toBe(false);
    });

    test('exposes the explicit native firmware version without capability inference', () => {
        expect(buildDashboardDeviceStatus({
            online: true,
            firmware_version: '0.2.0',
            capabilities: { firmware: 'legacy' }
        }, true).firmwareVersion).toBe('0.2.0');
        expect(buildDashboardDeviceStatus({
            online: true,
            capabilities: { firmware: 'legacy' }
        }, true).firmwareVersion).toBeNull();
    });

    test('recognizes flat live firmware MQTT fields as command accepting', () => {
        expect(isLiveMqttCommandAcceptingStatus({
            mqtt_connected: true,
    ...
    test('treats firmware Wi-Fi RSSI zero as not sampled', () => {
        expect(wifiRssiToPercent(0)).toBeNull();
        expect(wifiRssiToPercent(-66)).toBe(68);

        const status = buildDashboardDeviceStatus({
            online: true,
            activePath: 'wifi',
            wifiSignal: 100,
            wifi: { connected: true, ssid: 'BenchNet', rssi: 0, ipAddress: '192.168.1.20' }
        }, true);

        expect(status.wifiSignal).toBeNull();
        expect(status.signal).toBeNull();
        expect(status.signalLabel).toBe('N/A');
        expect(status.signalMeta).toBe('BenchNet');
        expect(status.wifiStatus).not.toContain('100%');
        expect(status.wifiStatus).not.toContain('0 dBm');
    });
    ...
    test('keeps transport and memory telemetry truthful across Wi-Fi and modem paths', () => {
        const status = buildDashboardDeviceStatus({
            online: true,
            active_path: 'wifi',
            wifi_connected: true,
            ...
            modem_data_session_open: true,
            modem_ip_bearer_ready: true,
            modem_pdp_ip_address: '10.177.244.78',
            charging: undefined,
        ...
        expect(status.connectionTransports.wifi).toEqual(expect.objectContaining({
            ipAddress: '192.168.0.102',
            transport: 'Wi-Fi/MQTT',
            active: true
        }));
        expect(status.connectionTransports.modem).toEqual(expect.objectContaining({
            ipAddress: '10.177.244.78',
            transport: 'AT bearer',
            active: false
        }));
        expect(status.connectionTransports.mqtt).toEqual(expect.objectContaining({
            connected: true,
            subscribed: true,
            label: 'MQTT'
        }));
        expect(status.charging).toBeNull();
        expect(status.powerSource).toBeNull();
    });

    test('labels an explicitly reported PPP modem transport without inferring PPP', () => {
        const status = buildDashboardDeviceStatus({
            online: true,
            active_path: 'modem',
            modem_registered: true,
        ...
        expect(status.connectionTransports.modem.transport).toBe('PPP/MQTT');
        expect(status.connectionTransports.modem.ipAddress).toBe('100.64.1.2');
    });

    test('uses AT/MQTT only when modem is the active subscribed MQTT route', () => {
        const status = buildDashboardDeviceStatus({
            online: true,
            active_path: 'modem',
            modem_registered: true,
            modem_data_ip: '10.1.2.3',
            mqtt_connected: true,
            mqtt_subscribed: true
        }, true);
        expect(status.connectionTransports.modem.transport).toBe('AT/MQTT');
        expect(status.connectionTransports.wifi.transport).toBe('Wi-Fi');
    });
    ...
    test('hydrates nested module snapshots from flat firmware status payloads', () => {
        const status = buildDashboardDeviceStatus({
            ...
            modem_pdp_ip_address: '10.189.112.220',
    ...
    test('does not clear mqtt_not_connected when subscription telemetry is missing', () => {
        const status = buildDashboardDeviceStatus({
            online: true,
            mqtt: { connected: true },
            systemRuntime: { degradedReason: 'mqtt_not_connected' },
            transport: { reason: 'mqtt_not_connected' }
        }, true);
