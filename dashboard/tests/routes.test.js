...
describe('POST /api/settings/clear-device-data', () => {
    ...
    test('clears device-linked tables and resets in-memory device state', async () => {
        global.modemService = {
        ...
        app.locals.mqttHandlers = {
            ...
            _registeredDeviceCache: new Map([['device-1', { registered: true, checkedAt: Date.now() }]]),
...
describe('GET /api/devices/:id/hotspot-diagnostics', () => {
    const originalHotspotPlatform = process.env.HOST_HOTSPOT_PLATFORM;

    beforeAll(() => {
        // Exercise the Windows hosted-network adapter regardless of the host OS.
        process.env.HOST_HOTSPOT_PLATFORM = 'win32';
    });

    afterAll(() => {
        if (originalHotspotPlatform === undefined) delete process.env.HOST_HOTSPOT_PLATFORM;
        else process.env.HOST_HOTSPOT_PLATFORM = originalHotspotPlatform;
    });
...
describe('rendered dashboard pages', () => {
    ...
    test('home SIM-scoped conversation summary uses complete multipart body and logical count', async () => {
        const segments = [2, 1].map((index) => ({
            id: index, device_id: 'device-11', from_number: '+8801000000000', to_number: '',
        ...
        const db = makeDbMock({ all: jest.fn().mockImplementation(async (sql) => {
            if (sql.includes('multipart_part_index')) return segments;
            return [];
        }) });
        global.modemService = { isDeviceOnline: jest.fn().mockReturnValue(true), getDeviceStatus: jest.fn().mockReturnValue({}) };
        const app = buildRenderedApp(require('../routes/index'), '/', { id: 11, role: 'admin', username: 'admin', deviceId: 'device-11' }, db);
        const res = await request(app).get('/?deviceId=device-11&simSlot=1');
        expect(res.status).toBe(200);
        expect(res.body.locals.recentConversations).toEqual([expect.objectContaining({
            last_message_preview: 'বাংলা পরীক্ষা', message_count: 1, unread_count: 1
        })]);
        const call = db.all.mock.calls.find(([sql]) => sql.includes('multipart_part_index'));
        expect(call[0]).toContain('sim_slot = ?');
    ...
    test('renders the dashboard home page with device-scoped SMS and calls queries', async () => {
        ...
        const db = makeDbMock({
            ...
            get: jest.fn().mockImplementation(async (sql) => {
                if (String(sql).includes('FROM devices d') && String(sql).includes('device_profiles')) {
                    return {
                        capabilities: JSON.stringify({
                            bridge: 'firmware',
                            modem: true,
                ...
                return countResults[countIndex++] || { count: 0 };
            })
        });
...
describe('storage routes', () => {
    ...
    test('rejects unsupported storage mutations before publishing', async () => {
        global.mqttService = {
            connected: true,
        ...
        const format = await request(app).post('/api/storage/format').send({ deviceId: 'device-1', storageType: 'sd' });
        const upload = await request(app).post('/api/storage/upload').field('deviceId', 'device-1');
        expect(res.status).toBe(501);
        expect(format.status).toBe(501);
    ...
    test('returns enriched storage diagnostics from storage-info payloads', async () => {
        ...
        expect(res.body.data.mutations).toMatchObject({ delete: false, write: false, format: false });
    });

    test('denies delete until fresh status proves a mounted physical SD identity', async () => {
        global.mqttService = { connected: true, publishCommand: jest.fn() };
        const app = buildApp(require('../routes/storage'), '/api/storage');

        const res = await request(app)
            .post('/api/storage/delete')
            .send({ deviceId: 'device-1', items: ['logs/a.txt'] });

        expect(res.status).toBe(409);
        expect(res.body.message).toContain('physical SD identity');
        expect(global.mqttService.publishCommand).not.toHaveBeenCalled();
    });

    test('allows delete only when live status proves the SD-only contract', async () => {
        global.modemService = {
            getDeviceStatus: jest.fn().mockReturnValue({
                online: true,
                storage: { mounted: true, mediaAvailable: true, type: 'SD card' },
                sdCard: { mounted: true, detected: true }
            })
        };
        global.mqttService = {
            connected: true,
            publishCommand: jest.fn().mockResolvedValue({ result: 'completed', success: true })
        };
        const app = buildApp(require('../routes/storage'), '/api/storage');
        ...
        expect(res.status).toBe(200);
        expect(global.mqttService.publishCommand).toHaveBeenCalledWith(
            'device-1', 'storage-delete', { path: 'logs/a.txt' }, true, 30000
        );
    });
});
...
describe('POST /api/automation/flows/:id/run', () => {
    test('returns the manual test-run result from the automation engine', async () => {
        ...
        const router = require('../routes/automation');
        // SEC-03: run requires flow ownership — the mock DB returns the flow row.
        const db = makeDbMock();
        db.get.mockResolvedValue({ id: 7, user_id: 1, device_id: 'dev-1' });
        const app = buildApp(router, '/api/automation', { id: 1, role: 'admin', username: 'admin' }, db);
