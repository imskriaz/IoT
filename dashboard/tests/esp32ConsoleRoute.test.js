...
describe('ESP32 MQTT console route', () => {
    ...
    test('keeps every ESP32 console preset backed by a firmware handler', async () => {
        ...
        for (const preset of esp32Presets) {
            ...
            expect(actionModels).toContain(`return "${firmwareCommand}";`);
            expect(dispatchedEnums).toContain(actionEnum);
        }
    });

    test('keeps full newlib formatting enabled for 64-bit device capacity fields', () => {
        const repoRoot = path.join(__dirname, '..', '..');
        const firmwareRoot = path.join(repoRoot, 'firmware', 'espidf', 'esp32-s3-a7670e-idf6.1');
        const defaults = fs.readFileSync(path.join(firmwareRoot, 'sdkconfig.defaults'), 'utf8');
        const apiBridge = fs.readFileSync(
            path.join(firmwareRoot, 'components', 'api_bridge', 'src', 'api_bridge.c'),
            'utf8'
        );

        expect(defaults).toContain('# CONFIG_LIBC_NEWLIB_NANO_FORMAT is not set');
        expect(defaults).not.toMatch(/^CONFIG_LIBC_NEWLIB_NANO_FORMAT=y$/m);
        expect(apiBridge).toContain('PRIu64');
    });

    test('keeps get-status peak allocations off the bounded automation stack', () => {
        const repoRoot = path.join(__dirname, '..', '..');
        const firmwareRoot = path.join(repoRoot, 'firmware', 'espidf', 'esp32-s3-a7670e-idf6.1', 'components');
        const apiBridge = fs.readFileSync(
            path.join(firmwareRoot, 'api_bridge', 'src', 'api_bridge.c'),
        ...
        expect(apiBridge).toContain('device_status_snapshot_t *snapshot = NULL;');
        expect(apiBridge).toContain('snapshot = api_bridge_alloc_zeroed(sizeof(*snapshot));');
        expect(apiBridge).toContain('heap_caps_free(snapshot);');
        expect(apiBridge).not.toContain('device_status_snapshot_t snapshot;');
        expect(automationBridge).toContain('CONFIG_UNIFIED_TASK_STACK_XLARGE');
        expect(automationBridge).toContain('task_registry_heartbeat("automation_bridge_task")');
    });

    test('settles ESP MQTT action results only after the QoS 1 broker acknowledgement', () => {
        const repoRoot = path.join(__dirname, '..', '..');
        const mqttManager = fs.readFileSync(
            path.join(repoRoot, 'firmware', 'espidf', 'esp32-s3-a7670e-idf6.1', 'components', 'mqtt_mgr', 'src', 'mqtt_mgr.c'),
            'utf8'
        );
        expect(mqttManager).toContain('event_id != MQTT_EVENT_PUBLISHED');
        expect(mqttManager).toContain('case MQTT_EVENT_PUBLISHED:');
        expect(mqttManager).toContain('s_action_result_inflight_sequence > s_last_action_result_sequence');
        expect(mqttManager).toContain('mqtt_action_result_puback_timeout');
        expect(mqttManager).toContain('!mqtt_subscription_expired(s_action_result_ack_deadline_ms, now_ms)');
        expect(mqttManager).toContain('next connected transport replays the same action ID');
        expect(mqttManager).toContain('mqtt_recovery_request(&s_esp_recovery, now_ms);');
    });

    test('debounces status-watch shutdown across ordinary dashboard navigation', () => {
        const repoRoot = path.join(__dirname, '..', '..');
        const server = fs.readFileSync(path.join(repoRoot, 'dashboard', 'server.js'), 'utf8');
        expect(server).toContain('const statusWatchDisableTimers = new Map();');
        expect(server).toContain('STATUS_WATCH_DISCONNECT_GRACE_MS = 5000');
        expect(server).toContain('if (nextCount === 1 && !pendingDisable)');
        expect(server).toContain('!liveCommandAccepting || wasCommandAccepting');
        expect(server).toContain('disableTimer.unref?.();');
    });
    ...
    test('new firmware aliases are split between real file-gpio handlers and explicit unavailable actions', () => {
        const repoRoot = path.join(__dirname, '..', '..');
        const firmwareRoot = path.join(repoRoot, 'firmware', 'espidf', 'esp32-s3-a7670e-idf6.1', 'components');
        const apiBridge = fs.readFileSync(
            path.join(firmwareRoot, 'api_bridge', 'src', 'api_bridge.c'),
        ...
        expect(apiBridge).toContain('static unified_action_response_t api_bridge_execute_unavailable_action(');
        expect(apiBridge).toContain('return api_bridge_execute_unavailable_action(action, payload, payload_len);');
        expect(apiBridge).not.toContain('api_bridge_execute_placeholder_lane');
        expect(apiBridge).not.toContain('"implemented":false');
    });
    ...
    test.each([
        ...
        expect(publishCommand).toHaveBeenCalledWith(
            'device-1',
            'modem-at',
            expect.objectContaining({ raw_line: rawLine }),
            true,
            30000,
            expect.any(Object)
        );
