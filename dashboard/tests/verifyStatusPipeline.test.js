...
describe('verify_status_pipeline helper', () => {
    ...
    test('parse helpers normalize args and signal semantics', () => {
        ...
        expect(parseWifiSignalPercent(0)).toBeNull();
        expect(parseWifiSignalPercent(0)).toBeNull();
    });

    test('unwraps correlated action-result captures and verifies persisted cache fields', () => {
        const raw = {
            device_id: 'esp-a7670e-test',
            hardware_uid: 'board-test',
            active_path: 'wifi',
            uptime_ms: 1234,
            wifi_rssi: 0,
            sd_mounted: true,
            storage_total_bytes: 62519640064,
            low_stack_task_count: 0,
            health_degraded: false
        };
        const capture = [{ data: { action_id: 'status-1', result: 'completed', payload: raw } }];

        expect(resolvePayloadData(capture)).toBe(raw);
        expect(compareCachedPayload(capture, { ...raw })).toEqual(expect.objectContaining({
            ok: true,
            mismatches: []
        }));
        expect(compareCachedPayload(capture, { ...raw, storage_total_bytes: 1 })).toEqual(expect.objectContaining({
            ok: false,
            mismatches: expect.arrayContaining([expect.stringContaining('storage_total_bytes')])
        }));
    });
