...
describe('onboarding routes', () => {
    ...
    test('keeps board model implicit and scopes bridge onboarding fields', () => {
        ...
        expect(html).toContain('ESP codes start with esp-, Android codes start with android-, httpSMS codes start with httpsms-.');
        ...
        expect(html).toContain('Install the phone app, save this device, then scan the code in the app.');
        expect(html).toContain('In the httpSMS app, tap the key field or QR icon, then scan this code.');
        expect(html).not.toContain('API path used by app');
        expect(html).not.toContain('The QR contains only the API key');
        ...
        expect(html).toContain('<tr><td class="text-muted">Device code</td><td><code>${escHtml(summary.device_id || \'\')}</code></td></tr>');
    ...
    test('falls back to debug Android APK when release is missing', async () => {
        const debugApkDir = path.join(__dirname, '..', '..', 'firmware', 'android', 'app', 'build', 'outputs', 'apk', 'debug');
        const debugApkPath = path.join(debugApkDir, 'app-arm64-v8a-debug.apk');
        const dirExisted = fs.existsSync(debugApkDir);
        fs.mkdirSync(debugApkDir, { recursive: true });
        fs.writeFileSync(debugApkPath, 'stub-debug-apk');

        let existsSpy = null;
        try {
            // Simulate: release APK missing, debug APK present. A real stub file is
            // required so res.download()/sendFile can stream the artifact end to end.
            existsSpy = jest.spyOn(fs, 'existsSync').mockImplementation((candidatePath) => {
            ...
            const streamed = Buffer.isBuffer(res.body) ? res.body.toString('utf8') : String(res.text || '');
            expect(streamed).toBe('stub-debug-apk');
        } finally {
            if (existsSpy) existsSpy.mockRestore();
            fs.rmSync(debugApkPath, { force: true });
            if (!dirExisted) fs.rmSync(debugApkDir, { recursive: true, force: true });
        }
    });
