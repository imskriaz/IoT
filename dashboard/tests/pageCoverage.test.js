...
describe('dashboard screen coverage', () => {
    ...
    test('system status exposes truthful extended device metrics', () => {
        ...
        for (const id of ['panelTemperature', 'panelVoltage', 'panelActivePath', 'panelHealth', 'panelWifiIp', 'panelModemIp', 'panelTransport', 'panelTransportPaths', 'panelRamFree', 'panelPsramFree']) {
            expect(header).toContain(`id="${id}"`);
            expect(main).toContain(`'${id}'`);
        }
        expect(main).toContain("status.sim?.dataIp || status.sim?.pdpIp || (status.activePath === 'modem'");
        expect(main).toContain("status.wifi?.ipAddress");
        expect(main).toContain('formatIpTransport');
    });

    test('pins the compact device metrics strip to the sidebar footer', () => {
        const sidebar = fs.readFileSync(path.join(__dirname, '..', 'views', 'partials', 'sidebar.html'), 'utf8');
        const css = fs.readFileSync(path.join(__dirname, '..', 'public', 'css', 'app.css'), 'utf8');
        expect(sidebar).toContain('class="sidebar-metrics-footer app-hidden');
        expect(css).toContain('.sidebar-content > .nav-pills');
        expect(css).toContain('#sidebarMetricsStrip.sidebar-metrics-footer');
        expect(css).toContain('overflow-y: auto');
    });

    test('storage card renders firmware-reported memory totals', () => {
        const page = fs.readFileSync(path.join(__dirname, '..', 'views', 'pages', 'index.html'), 'utf8');
        for (const id of ['dashSdUsed', 'dashSdCapacity', 'dashRamUsage', 'dashPsramUsage', 'dashHeapUsage', 'dashFlashUsage']) {
            expect(page).toContain(`id="${id}"`);
        }
        expect(page).toContain('runtimeRamUsed');
        expect(page).toContain('psramUsed');
        expect(page).toContain('flashSizeBytes');
        expect((page.match(/<small class="dashboard-storage-compact-row">/g) || [])).toHaveLength(3);
        expect(page).toContain('System flash capacity; used bytes are not reported by firmware');
    });

    test('device about fails closed for missing live telemetry', () => {
        const page = fs.readFileSync(path.join(__dirname, '..', 'views', 'pages', 'device-about.html'), 'utf8');
        expect(page).toContain('const offline = live.online !== true;');
        expect(page).not.toContain("system.fuelGaugeIc || 'MAX17048G'");
        expect(page).not.toContain("system.chargingIc || 'ETA6098'");
        expect(page).not.toContain("system.solarChargingIc || 'CN3791'");
        expect(page).not.toContain('wifi.clients ?? wifiHotspot.connectedClients ?? 0');
        expect(page).toContain('formatCounterPair(live.storage.queueDepth, live.storage.dropped)');
    });
    ...
    test('Console command surface uses tabs with one dropdown and inline description', () => {
        ...
        expect(html).toContain('<option value="command" selected>Command</option>');
        expect(html).toContain('<option value="action">Action</option>');
        expect(html).toContain('<option value="function">Function</option>');
        expect(html).toContain('<option value="chain">Chain</option>');
        expect(html.indexOf('<option value="function">Function</option>')).toBeLessThan(html.indexOf('<option value="chain">Chain</option>'));
        expect(html).toContain('for="esp32CommandPresetSelect" id="esp32CommandPresetLabel">Command</label>');
        expect(html).toContain('for="esp32CommandLineInput">Command line</label>');
        expect(html).toContain('id="esp32CommandLineInput"');
        expect(html).toContain('class="command-description small" id="esp32OptionDetails"');
        expect(html).toContain('class="command-result" id="esp32CommandResultPanel"');
        expect(html).toContain('id="esp32CommandResultCopyBtn"');
        expect(html).toContain('id="esp32CommandResultSummary"');
        expect(html).toContain('id="esp32CommandResultJson"');
        expect(html).toContain('id="esp32EventDetailCopyBtn"');
        ...
        expect(js).toContain('const STORAGE_PREFIX = \'esp32Console.\'');
        expect(js).toContain("const CONSOLE_TABS = ['system', 'serial', 'mqtt'];");
        expect(js).toContain("const SYSTEM_LOG_SOURCES = ['app', 'mqtt', 'error', 'manual'];");
        expect(js).toContain("const LOG_LEVELS = ['', 'error', 'warn', 'info', 'debug'];");
        expect(js).toContain("const LOG_LIMITS = ['50', '100', '200', '500'];");
        expect(js).toContain("const MANUAL_EVENT_SOURCES = ['command', 'send', 'response', 'console', 'serial', 'serial-tx', 'serial-rx', 'error', 'scenario', 'wait', 'validation'];");
        expect(js).toContain('function restoreLogPreferences()');
        expect(js).toContain('function loadSystemLogCounts()');
        expect(js).toContain("readPreference(`command:${state.activeTab}:${state.mode}`)");
        expect(js).toContain("savePreference('activeTab', normalizedTab)");
        expect(js).toContain("savePreference('logSource', normalized)");
        expect(js).toMatch(/loadSystemLogs\(\);\s+loadSystemLogCounts\(\);/);
        expect(js).toContain("if (category === 'system') return 'Action';");
        ...
        expect(js).toContain('CONSOLE_TABS.includes(tab)');
        expect(js).toContain('SYSTEM_LOG_SOURCES.includes(logSource)');
        expect(js).toContain('MANUAL_EVENT_SOURCES.includes(source)');
        expect(js).toContain("state.mode === 'command' && category !== 'manual'");
        expect(js).toContain("state.mode === 'action' && category !== 'system'");
        expect(js).toContain("if (state.mode === 'function')");
        expect(js).toContain('state.presets.forEach((preset, index) => {');
        expect(js).toContain('state.vendorCommands.forEach((vendorCommand, commandIndex) => {');
        expect(js).toContain("Object.entries(state.tests || {}).forEach(([testId, test]) => {");
        expect(js).toContain('visibleManualLogRows');
        expect(js).toContain('function buildReadableDetail(entry, value)');
        expect(js).toContain('function effectiveEntryLevel(entry)');
        expect(js).toContain('function effectiveEntryStatus(entry)');
        ...
        expect(js).toContain('function commandResultStatus(payload, fallbackLevel = \'success\')');
        expect(js).toContain('const successFlags = [');
        expect(js).toContain("if (successFlags.includes(false) || /fail|timeout|denied|invalid/.test(statusText)) return 'danger';");
        expect(js).toContain('function renderCommandResult(update = null)');
        expect(js).toContain('function buildResultDetail(result)');
        expect(js).toContain('state.results[updateTransport]');
        expect(js).toContain('function updateLine(entry, updates = {}, options = {})');
        expect(js).toContain("appendLine('command'");
        expect(js).toContain('updateLine(commandEntry');
        expect(js).toContain('function normalizedLevelTag(level)');
        expect(js).toContain('function logLevelTag(level)');
        expect(js).toContain('function levelTagClass(level)');
        expect(js).toContain('function levelTagLabel(level)');
        expect(js).toContain('function entryTagClass(entry)');
        expect(js).toContain('function entryTagLabel(entry)');
        expect(js).toContain('String(entry?.level || \'info\').toUpperCase()');
        expect(js).toContain('const resultStatus = commandResultStatus(data, resultLevel);');
        expect(js).toContain('entry.status || entry.level');
        expect(js).toContain("<dt class=\"col-sm-3\">Status</dt>");
        expect(js).toContain('normalized.level = effectiveEntryLevel(normalized);');
        expect(js).toContain('normalized.status = effectiveEntryStatus(normalized);');
        expect(js).toContain("elements.resultCopy?.addEventListener('click'");
        expect(js).toContain("elements.detailCopy?.addEventListener('click'");
        expect(js).toContain('function copyTextBlock(text, successMessage)');
        expect(js).toContain('function resetLiveConsoleViews()');
        expect(js).toContain('state.clearedViews.serial = now;');
        expect(js).toContain('state.clearedViews.mqtt = now;');
        expect(js).toContain('const CHAIN_SCENARIOS = [');
        expect(js).toContain("key: 'custom'");
        expect(js).toContain("label: 'Custom'");
        expect(js).toContain('USSD Balance Menu');
        expect(js).toContain('wait 8000');
        expect(js).toContain('function parseChainPlan(value)');
        expect(js).toContain('function validateCommandPlan(plan, transport)');
        expect(js).toContain('function validateCommandList(commands, transport)');
        expect(js).toContain('function validateCommandText(command, lineNumber, transport)');
        expect(js).toContain('/^a(?:t)?(?:$|[+?=,])/i');
        expect(js).toContain('vendorCommand?.requiresVariant');
        expect(js).toContain('This command needs parameters or a target. Example:');
        expect(js).toContain('This command needs a non-bare command form. Example:');
        expect(js).toContain('function isValidCusdLine(command)');
        expect(js).toContain('function reportCommandValidationErrors(errors)');
        expect(js).toContain('const validationErrors = validateCommandPlan(plan, state.transport)');
        expect(js).toContain('if (validationErrors.length)');
        expect(js).toContain('AT+CUSD needs parameters');
        expect(js).toContain('Invalid CUSD format');
        expect(js).toContain("transport: 'mqtt'");
        expect(js).toContain("transport: 'serial'");
        expect(js).toContain("'console', 'serial'");
        expect(js).toContain("status: waitForResponse ? 'waiting' : 'published'");
        expect(js).toContain("status: resultStatus");
        expect(js).toContain('function commandHelp(option)');
        expect(js).toContain('function defaultCommandLine(option)');
        expect(js).toContain('function currentCommandLine(option)');
        expect(js).toContain('function vendorModule(option)');
        expect(js).toContain('function findVendorCommandByLine(command)');
        expect(js).toContain('function firstVendorExample(commandEntry)');
        ...
        expect(js).toContain('function normalizeMode(mode)');
        expect(js).toContain("case 'completed':");
        expect(js).toContain("case 'waiting':");
        expect(js).toContain("case 'debug':");
        ...
        expect(js).toContain("state.mode === 'action' && category !== 'system'");
        expect(js).toContain("state.mode === 'command' && category !== 'manual'");
        expect(js).toContain("'command', 'action', 'chain', 'function'");
        expect(js).toContain("state.mode === 'function'");
        expect(js).toContain("type: 'scenario'");
        expect(js).toContain("type: 'test'");
        expect(js).toContain('elements.commandLine.value = defaultCommandLine(option)');
        expect(js).toContain("parseChainPlan(elements.chain?.value || '')");
        expect(js).toContain('function sendConsolePlan(plan)');
        expect(js).toContain('function sendSerialPlan(plan)');
        expect(js).toContain('function delay(ms)');
        expect(js).toContain('<dt>Module</dt>');
        expect(js).toContain('<dt>Task</dt>');
        expect(js).toContain('<dt>Parameter</dt>');
        expect(js).toContain('How to use');
        expect(js).toContain('AT+CUSD=<n>,"<code>",<dcs>');
        expect(js).toContain('AT+CUSD=1,"*123#",15');
        expect(js).toContain('AT+CUSD=1,"1",15');
        expect(js).toContain('AT+CUSD=2');
        expect(js).toContain('No parameter is required for the selected base command.');
        expect(html).toContain('.esp32-console-page.is-logs-tab .console-events');
        expect(html).toContain('.esp32-console-page .event-level');
        expect(html).toContain('.esp32-console-page .event-level-pass');
        expect(html).toContain('.esp32-console-page .event-level-fail');
        expect(html).toContain('.esp32-console-page .event-level-processing');
        expect(html).toContain('.esp32-console-page .event-level-input');
        ...
        expect(js).not.toContain('<dt>Source</dt>');
        expect(js).not.toContain('MQTT Runtime');
        expect(js).not.toContain('MQTT Manual');
        expect(js).not.toContain('Serial Manual');
        expect(js).not.toContain('MQTT AT');
        expect(js).not.toContain('Serial AT');
    });
    ...
    test('dashboard home renders normalized flat firmware status in page locals', async () => {
        ...
        const realModemService = require('../services/modemService');
        realModemService.resetDevices();
        realModemService.updateDeviceStatus('device-1', {
                active_path: 'wifi',
                ...
                sd_detected: true,
                sd_mounted: true,
                sd_total_bytes: 62519640064,
                sd_used_bytes: 655360,
                sd_free_bytes: 62518984704,
                runtime_ram_total_bytes: 327680,
                runtime_ram_used_bytes: 98304,
                psram_total_bytes: 8388608,
                psram_used_bytes: 1048576,
                heap_total_bytes: 8716288,
                heap_used_bytes: 1130496,
                flash_size_bytes: 16777216
        });
        global.modemService = realModemService;
        ...
        const rendered = ejs.render(fs.readFileSync(templatePath, 'utf8'), {
            ...res.body.locals,
            moment: require('moment')
        }, { filename: templatePath });
        expect(rendered).toContain('id="dashSdUsed">640K</span>');
        expect(rendered).toContain('id="dashSdCapacity">58.23G</span>');
        expect(rendered).toContain('id="dashRamUsage">96K/320K</span>');
        expect(rendered).toContain('id="dashPsramUsage">1M/8M</span>');
        expect(rendered).toContain('id="dashFlashUsage">16M cap</span>');
        const missingSdCard = { ...res.body.locals.deviceStatus.sdCard, usedBytes: null, freeBytes: null };
        const missingUsageRendered = ejs.render(fs.readFileSync(templatePath, 'utf8'), {
            ...res.body.locals,
            sdCardMeter: require('../public/js/sd-status').meter(missingSdCard, true),
            deviceStatus: {
                ...res.body.locals.deviceStatus,
                sdCard: missingSdCard
            },
            moment: require('moment')
        }, { filename: templatePath });
        expect(missingUsageRendered).toContain('id="dashSdUsed">—</span>');
        const impossibleMemoryRendered = ejs.render(fs.readFileSync(templatePath, 'utf8'), {
            ...res.body.locals,
            deviceStatus: {
                ...res.body.locals.deviceStatus,
                systemRuntime: { runtimeRamUsed: 400000, runtimeRamTotal: 327680 },
                hardware: { flashSizeBytes: 'not-a-number' }
            },
            moment: require('moment')
        }, { filename: templatePath });
        expect(impossibleMemoryRendered).toContain('id="dashRamUsage">—</span>');
        expect(impossibleMemoryRendered).toContain('id="dashFlashUsage">—</span>');
...
describe('sidebar navigation coverage', () => {
    ...
    test('queue manager uses the shared appConfirm flow instead of an undefined legacy helper', () => {
        ...
        expect(html).toContain('Live calls and USSD stay runtime-only');
        expect(html).not.toContain('Clear Call Queue');
    });

    test('keeps notifications in the header modal instead of a separate sidebar page', () => {
        const sidebarHtml = renderSidebar();
        const headerHtml = fs.readFileSync(headerPath, 'utf8');

        expect(sidebarHtml).not.toContain('href="/notifications"');
        expect(headerHtml).toContain('id="dashboardNotificationsModal"');
        expect(headerHtml).toContain('id="notificationModalPageLabel"');
        expect(headerHtml).toContain('window.clearHeaderNotifications');
        expect(headerHtml).not.toContain('href="/notifications"');
    });
    ...
    test('SMS sync overlay has a watchdog and failure close event', () => {
        const smsJs = fs.readFileSync(smsJsPath, 'utf8');

        expect(smsJs).toContain('smsSyncWatchdogTimer');
        expect(smsJs).toContain('SMS sync is taking too long. Refreshing the dashboard now.');
        expect(smsJs).toContain("window.socket.on('sms:sync-failed', smsSyncCompletedHandler)");
        expect(smsJs).toContain("window.socket?.off?.('sms:sync-failed', smsSyncCompletedHandler)");
    });
    ...
    test('modem page script normalizes connection labels and uses plain dash fallbacks', () => {
        ...
        expect(js).toContain("elements.scanWiFiBtn.disabled = !isDeviceOnline || wifiCaps.scan === false;");
    ...
    test('modem page uses normalized Wi-Fi wording and active-source labels', () => {
        const js = fs.readFileSync(path.join(__dirname, '..', 'public', 'js', 'modem.js'), 'utf8');
        ...
        expect(js).toContain("'Dashboard Route Ready'");
        expect(js).toContain("'Dashboard Route Unavailable'");
        ...
        expect(modemHtml).toContain('Wi-Fi is primary. Mobile data is the automatic fallback.');
        expect(modemHtml).toContain('data-tab-url-param="panel"');
        expect(modemHtml).toContain('Connection properties');
        ...
        expect(modemHtml).toContain('Add network');
        ...
        expect(modemHtml).toContain('<i class="bi bi-wifi me-2"></i>Wi-Fi');
    ...
    test('device about page resolves the active device from the shared helper first', () => {
        ...
        expect(html).toContain('Not reported by SIM');
