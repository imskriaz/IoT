...
function buildApp(router, locals = {}) {
    ...
    // The application mounts this router after authentication. Happy-path
    // tests use an authenticated administrator; denial cases exercise real auth.
    app.use((req, _res, next) => {
        req.user = { id: 1, role: 'admin', username: 'status-test-admin' };
        next();
    });
...
describe('status route live refresh', () => {
    ...
    test('does not settle a requested refresh from an unrelated device status snapshot', async () => {
        const mqttService = new EventEmitter();
        mqttService.connected = true;
        mqttService.isDeviceOnline = jest.fn().mockReturnValue(true);
        mqttService.getDeviceQueueState = jest.fn().mockResolvedValue(null);
        let onPublished;
        const published = new Promise(resolve => { onPublished = resolve; });
        mqttService.publishCommand = jest.fn().mockImplementation(async (_deviceId, _command, _payload, _wait, _timeout, options) => {
            onPublished(options.messageId);
            return { success: true };
        });
        global.mqttService = mqttService;
        global.modemService = modemService;
        modemService.handleHeartbeat('device-correlated');
        const app = buildApp(require('../routes/status'));

        const responsePromise = request(app).get('/api/status?deviceId=device-correlated&refresh=1').then(response => response);
        const actionId = await published;
        expect(mqttService.listenerCount('status')).toBe(1);
        mqttService.emit('status', 'device-correlated', { type: 'device_status', messageId: 'another-action' });
        expect(mqttService.listenerCount('status')).toBe(1);
        mqttService.emit('status', 'device-correlated', { type: 'device_status', action_id: actionId });

        const response = await responsePromise;
        expect(response.status).toBe(200);
        expect(mqttService.listenerCount('status')).toBe(0);
    });
    ...
    test('does not overwrite newer live memory telemetry with an older database cache', async () => {
        global.mqttService = {
            connected: true,
            getDeviceQueueState: jest.fn().mockResolvedValue(null)
        };
        global.modemService = modemService;
        modemService.updateDeviceStatus('device-live-memory', {
            type: 'device_status',
            active_path: 'wifi',
            wifi_connected: true,
            wifi_ip_address: '192.168.0.10',
            ...
            runtime_ram_free_bytes: 229376,
            psram_total_bytes: 8388608,
            psram_used_bytes: 1048576
        });
        const db = {
            get: jest.fn(async (sql) => String(sql).includes('device_status_cache') ? {
                updated_at: new Date(Date.now() - 30000).toISOString(),
                payload_json: JSON.stringify({
                    runtime_ram_free_bytes: 100000,
                    psram_used_bytes: 4000000
                })
            } : null),
            all: jest.fn().mockResolvedValue([])
        };
        const app = buildApp(require('../routes/status'), { db });

        const res = await request(app).get('/api/status?deviceId=device-live-memory');

        expect(res.status).toBe(200);
        expect(res.body.data.systemRuntime).toEqual(expect.objectContaining({
            runtimeRamFree: 229376,
            psramUsed: 1048576
        }));
        expect(db.get.mock.calls.some(([sql]) => String(sql).includes('device_status_cache'))).toBe(false);
    });
    ...
    test('uses fresh durable status cache after dashboard restart before the next heartbeat', async () => {
        ...
        expect(db.get.mock.calls.filter(([sql]) => String(sql).includes('device_status_cache'))).toHaveLength(1);
        expect(res.body.caps).toEqual(expect.objectContaining({
            wifi: true,
            internet: true
        }));
    ...
    test('uses stored SIM number fallback when live modem status omits subscriber number', async () => {
        ...
        expect(res.body.data.simNumberSource).toBe('stored');
        expect(res.body.data.subscriberNumberSource).toBe('stored');
