...
describe('moduleHealth live snapshot preference', () => {
    ...
    test('does not let live snapshots override explicit unsupported capabilities', async () => {
        ...
        expect(wifi.supported).toBe(false);
        expect(wifi.state).toBe('unsupported');
        expect(storage.supported).toBe(false);
        expect(storage.state).toBe('unsupported');
    });

    test.each([true, false])('current capability declaration wins over stored health: %s', async (supported) => {
        const db = { all: jest.fn().mockResolvedValue(['camera', 'storage', 'wifi'].map(module_key => ({
            module_key, supported: supported ? 0 : 1,
            state: supported ? 'unsupported' : 'ok', last_message: 'Old firmware',
            details: '{"old":true}'
        }))) };
        const entries = await getDeviceModuleHealth(db, 'dev-1', {
            camera: supported, storage: supported, sd: true, wifi: supported
        }, { live: { online: true } });
        for (const key of ['camera', 'storage', 'wifi']) {
            expect(entries.find(entry => entry.moduleKey === key)).toMatchObject({
                supported, state: supported ? 'unknown' : 'unsupported', details: null
            });
        }
    });
