...
describe('smsDeliveryReports', () => {
    test.each([undefined, null, '', '   ', false])('does not invent zero from missing report values: %p', (value) => {
        expect(normalizeSmsDeliveryReport({ message_reference: value, status_report_status: value }))
            .toEqual(expect.objectContaining({ messageReference: null, statusReportStatus: null, status: 'pending' }));
    });

    test('preserves genuine zero reference and delivered status code', () => {
        expect(normalizeSmsDeliveryReport({ message_reference: 0, status_report_status: 0 }))
            .toEqual(expect.objectContaining({ messageReference: 0, statusReportStatus: 0, status: 'delivered' }));
    });
