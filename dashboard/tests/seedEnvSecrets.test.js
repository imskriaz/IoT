...
describe('seed_env_secrets utility', () => {
    ...
    test('parseArgs defaults to dry run unless write is requested', () => {
        expect(parseArgs([])).toEqual({ write: false, dryRun: true, rotate: false, keys: null });
        ...
        expect(parseArgs(['--rotate'])).toEqual({
            write: true,
            dryRun: false,
            rotate: true,
            keys: null
    ...
    test('rotate regenerates already-set supported secrets with a safety copy', () => {
        const tempDir = createTempDir();
        try {
            const envFile = path.join(tempDir, '.env');
            fs.writeFileSync(envFile, 'SESSION_SECRET=real-existing-value\nADMIN_PASSWORD=real-admin\nMQTT_HOST=broker.example.com\n', 'utf8');
            const plan = buildReplacementPlan(fs.readFileSync(envFile, 'utf8'), null, { rotate: true });
            expect(plan.replacements.map((entry) => entry.key).sort()).toEqual(['ADMIN_PASSWORD', 'SESSION_SECRET']);
            expect(plan.replacements.every((entry) => entry.nextValue !== 'real-existing-value' && entry.nextValue !== 'real-admin')).toBe(true);

            const result = seedSecrets({ envFilePath: envFile, write: true, rotate: true });
            expect(result.mode).toBe('write');
            expect(result.replacements.sort()).toEqual(['ADMIN_PASSWORD', 'SESSION_SECRET']);
            expect(result.safetyCopyPath).toBeTruthy();

            const nextContent = fs.readFileSync(envFile, 'utf8');
            expect(nextContent).toContain('MQTT_HOST=broker.example.com');
            expect(nextContent).not.toContain('real-existing-value');
            expect(nextContent).not.toContain('real-admin');
        } finally {
            removeTempDir(tempDir);
        }
    });
