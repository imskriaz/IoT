...
function buildApp(router) {
    ...
    app.use((req, _res, next) => {
        // SEC-01: routes now require device access; admin session for tests.
...
describe('modem route MQTT event flows', () => {
    test.each([
        ['missing heartbeat', false, true, true, true, true, false, false],
        ['stale status', true, false, true, true, true, false, false],
        ['dashboard broker down', true, true, false, true, true, false, false],
        ['device unsubscribed', true, true, true, true, false, false, false],
        ['subscribed and fresh', true, true, true, true, true, true, true],
        ['selected modem without modem bearer', true, true, true, true, true, false, false, 'modem']
    ])('Internet status command gate: %s', async (
        _name, online, statusFresh, brokerConnected, mqttConnected, mqttSubscribed,
        expectedInternet, expectedCommandReady, activePath = 'wifi'
    ) => {
        jest.resetModules();
        const deviceId = `gate-${_name.replace(/\s+/g, '-')}`;
        global.mqttService = { connected: brokerConnected };
        global.modemService = {
            getStatus: jest.fn().mockReturnValue({
                online, statusFresh, activePath,
                wifi: { started: true, connected: true, ssid: 'BenchNet', ipAddress: '192.168.1.50' },
                mqtt: { connected: mqttConnected, subscribed: mqttSubscribed }
            }),
            devices: new Map([[deviceId, {}]])
        };
        const app = buildApp(require('../routes/modem'));
        app.locals.db = { get: jest.fn().mockResolvedValue(null), all: jest.fn().mockResolvedValue([]) };

        const response = await request(app).get(`/api/modem/status?deviceId=${deviceId}`);

        expect(response.status).toBe(200);
        expect(response.body.data.deviceOnline).toBe(online);
        expect(response.body.data.internet.available).toBe(expectedInternet);
        expect(response.body.data.commandReady).toBe(expectedCommandReady);
        if (!statusFresh || !online) {
            expect(response.body.data.internet.activeSource).toBe('none');
            expect(response.body.data.internet.sources.wifi).toBe(false);
        } else {
            expect(response.body.data.internet.sources.wifi).toBe(true);
        }
    });

    test.each([
        ['capacity', 5, 3, { success: true, result: 'completed' }, 'device_profile_capacity_exceeded'],
        ['unknown revision', 1, 0, {}, 'device_profile_revision_unavailable'],
        ['accepted only', 1, 3, { success: true, result: 'accepted' }, 'device_profile_apply_not_confirmed'],
        ['wrong revision', 1, 3, { success: true, result: 'completed', payload: { applied: true, revision: 99, count: 1 } }, 'device_profile_apply_not_confirmed'],
        ['confirmed', 1, 3, { success: true, result: 'completed', payload: { applied: true, revision: 4, count: 1 } }, null]
    ])('profile sync: %s', async (_name, count, revision, result, reason) => {
        global.mqttService = {
            connected: true,
            deviceStatus: new Map([['device-1', { lastStatus: { wifi_profiles_revision: revision } }]]),
            publishRuntimeCommand: jest.fn().mockResolvedValue(result)
        };
        const app = buildApp(require('../routes/modem'));
        app.locals.db = {
            get: jest.fn().mockResolvedValue(null),
            run: jest.fn().mockResolvedValue({ changes: 1 }),
            all: jest.fn().mockResolvedValue(Array.from({ length: count }, (_, i) => ({
                ssid: `Network${i}`, password: 'test-secret', password_set: 1, priority: 50, enabled: 1, security: 'WPA2-PSK'
            })))
        };
        const response = await request(app).post('/api/modem/wifi/client/profiles').send({
            deviceId: 'device-1', ssid: 'Network0', security: 'WPA2-PSK', password: 'test-secret'
        });
        expect(response.status).toBe(200);
        expect(response.body.data.deviceSync).toMatchObject({ applied: reason === null, pending: reason !== null, reason });
        expect(JSON.stringify(response.body)).not.toContain('test-secret');
        expect(global.mqttService.publishRuntimeCommand).toHaveBeenCalledTimes(count > 4 || revision === 0 ? 0 : 1);
    });
    test('saves a Wi-Fi profile while MQTT is unavailable without returning its password', async () => {
        const app = buildApp(require('../routes/modem'));
        app.locals.db = {
            get: jest.fn().mockResolvedValue(null),
            run: jest.fn().mockResolvedValue({ changes: 1 }),
            all: jest.fn().mockResolvedValue([{ ssid: 'Saved network', password_set: 1, security: 'WPA2-PSK' }])
        };
        const response = await request(app).post('/api/modem/wifi/client/profiles').send({
            deviceId: 'device-1', ssid: 'Saved network', security: 'WPA2-PSK', password: 'test-secret'
        });
        expect(response.status).toBe(200);
        expect(response.body.success).toBe(true);
        expect(JSON.stringify(response.body)).not.toContain('test-secret');
        expect(app.locals.db.run).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO device_wifi_networks'), expect.arrayContaining(['device-1', 'Saved network']));
    });

    test('rejects overlong UTF-8 SSIDs and non-boolean profile enabled values', async () => {
        const app = buildApp(require('../routes/modem'));
        app.locals.db = { run: jest.fn() };
        const save = await request(app).post('/api/modem/wifi/client/profiles').send({ deviceId: 'device-1', ssid: 'é'.repeat(17), security: 'open' });
        expect(save.status).toBe(400);
        const update = await request(app).patch('/api/modem/wifi/client/profiles/test').send({ deviceId: 'device-1', enabled: 'false' });
        expect(update.status).toBe(400);
        expect(app.locals.db.run).not.toHaveBeenCalled();
    });

    test('rejects an invalid dashboard profile priority', async () => {
        const app = buildApp(require('../routes/modem'));
        app.locals.db = { run: jest.fn() };
        const response = await request(app).post('/api/modem/wifi/client/profiles').send({
            deviceId: 'device-1', ssid: 'RiazM', security: 'open', priority: 0
        });
    ...
    test('modem status does not claim connected when the new session contract is explicitly false', async () => {
        jest.resetModules();
        ...
        global.modemService = {
            getStatus: jest.fn().mockReturnValue({
                ...
                wifi: { connected: false, started: true }
            }),
            devices: new Map()
        };
        ...
        const res = await request(app).get('/api/modem/status?deviceId=device-contract-false');

        expect(res.status).toBe(200);
        expect(res.body.data.mobile.connected).toBe(false);
        expect(res.body.data.internet.sources.mobile).toBe(false);
    ...
    test('modem status hydration trusts explicit connection flags instead of guessing from SSID or signal', async () => {
        ...
        expect(res.body.data.networkTelemetryFresh).toBe(false);
        expect(res.body.data.internet.sources).toEqual(expect.objectContaining({
            mobile: false,
            wifi: false
        }));
    ...
    test('modem status hydration preserves Wi-Fi as the current path when both links report connected', async () => {
        ...
        expect(res.body.success).toBe(true);
        expect(res.body.data.routing.primarySource).toBe('wifi');
        expect(res.body.data.internet.activeSource).toBe('none');
        expect(res.body.data.internet.sources).toEqual(expect.objectContaining({
            mobile: false,
            wifi: false
        }));
    ...
    test('connection policy keeps the dual development mode explicit and publishes the versioned device field', async () => {
        jest.resetModules();
        ...
        mqttService.publishCommand = jest.fn().mockResolvedValue({
            success: true,
            payload: {
                connection_policy: 'dual_active_development',
                development_mode: true,
                activation_pending: true,
                wifi_connected: true,
                modem_bearer_ready: false,
                failover: true,
                load_balancing: false,
                nat: false,
                firewall: false,
        ...
        const res = await request(app)
            .post('/api/modem/routing/configure')
            .send({ deviceId: 'device-routing-dual', connectionPolicy: 'dual_active_development' });
        ...
        expect(mqttService.publishCommand).toHaveBeenCalledWith(
            ...
            true,
            15000,
            expect.objectContaining({ skipPersistentQueue: true })
        );
    });

    test('cellular-primary policy is rejected until secure modem MQTT is verified', async () => {
        jest.resetModules();
        ...
        const res = await request(app)
            .post('/api/modem/routing/configure')
            .send({ deviceId: 'device-routing-cellular', connectionPolicy: 'cellular_only' });

        expect(res.status).toBe(409);
        expect(res.body.data.code).toBe('SECURE_CELLULAR_MQTT_UNAVAILABLE');
        expect(mqttService.publishCommand).not.toHaveBeenCalled();
    });
    ...
    test('wifi connect treats timeout as success when status confirms the device moved to Wi-Fi', async () => {
        jest.resetModules();
        ...
        const res = await request(app)
            .post('/api/modem/wifi/client/connect')
            .send({
                deviceId: 'device-connect-timeout-success',
                ssid: 'BenchNet',
                password: 'bench-pass',
                security: 'WPA2-PSK'
            });
        ...
        expect(res.status).toBe(200);
        expect(res.body.success).toBe(true);
        expect(res.body.message).toContain('Connected to BenchNet.');
        expect(res.body.data).toEqual(expect.objectContaining({
            selectedSsid: 'BenchNet',
            observedWifiConnected: true,
            activePath: 'wifi',
        ...
        expect(mqttService.publishCommand).toHaveBeenCalledTimes(3);
        expect(mqttService.publishCommand).toHaveBeenNthCalledWith(
            2,
            'device-connect-timeout-success',
            'config-set',
    ...
    test('wifi connect treats timeout as success when cached device status confirms the Wi-Fi switch', async () => {
        jest.resetModules();

        const mqttService = new EventEmitter();
        mqttService.connected = true;
        mqttService.deviceStatus = new Map();
        mqttService.publishCommand = jest.fn().mockImplementation(async (deviceId, command) => {
            if (command === 'wifi-connect') {
                setTimeout(() => {
                    mqttService.deviceStatus.set(deviceId, {
        ...
        const res = await request(app)
            .post('/api/modem/wifi/client/connect')
            .send({
                deviceId: 'device-connect-timeout-cache',
                ssid: 'BenchNet',
                password: 'bench-pass',
                security: 'WPA2-PSK'
            });
    ...
    test('wifi connect returns immediately when the target Wi-Fi is already active and MQTT is healthy', async () => {
        jest.resetModules();
        ...
        const res = await request(app)
            .post('/api/modem/wifi/client/connect')
            .send({
                deviceId: 'device-connect-fast-path',
                ssid: 'BenchNet',
                password: 'bench-pass',
                security: 'WPA2-PSK'
            });
        ...
        expect(res.status).toBe(200);
        expect(res.body.success).toBe(true);
        expect(res.body.message).toBe('Connected to BenchNet.');
        expect(res.body.data).toEqual(expect.objectContaining({
            selectedSsid: 'BenchNet',
            observedWifiConnected: true,
            activePath: 'wifi',
