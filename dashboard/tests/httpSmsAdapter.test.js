...
describe('httpSmsAdapter routes', () => {
    ...
    test('GET /messages/outstanding returns one queued message in httpSMS format and marks it sending', async () => {
        ...
        // QUEUE-02: the conditional claim must report that THIS poll won the row.
        db.run.mockResolvedValue({ lastID: 0, changes: 1 });
        const router = require('../routes/httpSmsAdapter');
        const app = buildApp(router, db);
        ...
        expect(db.run).toHaveBeenCalledWith(
            expect.stringContaining("http_claimed_at = CURRENT_TIMESTAMP"),
            ['httpsms-01', 41]
        );
    });

    test('GET /messages/outstanding reports none when another poll holds the claim lease', async () => {
        const db = makeDbMock();
        db.get
            .mockResolvedValueOnce({ id: 'httpsms-01' })
            .mockResolvedValueOnce({
        ...
        // Claim UPDATE affects zero rows: a concurrent poll already owns it.
        db.run.mockResolvedValue({ lastID: 0, changes: 0 });
        const router = require('../routes/httpSmsAdapter');
        const app = buildApp(router, db);
        ...
        expect(res.body.message).toBe('no outstanding message');
    });
