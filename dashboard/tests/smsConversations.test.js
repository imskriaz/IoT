...
describe('sms conversation backfill', () => {
    ...
    test('persists one complete multipart preview and logical unread count, refreshing after read and delete', async () => {
        for (const [index, body] of ['বাংলা ', 'পরীক্ষা'].entries()) {
            await db.run(`INSERT INTO sms (device_id, from_number, to_number, message, timestamp, type, status,
                source, sim_slot, multipart_ref, multipart_part_index, multipart_part_count)
                VALUES (?, ?, '', ?, ?, 'incoming', 'received', 'esp32-mqtt', 0, '72', ?, 2)`,
            [deviceId, '+8801000000000', body, `2026-09-13T10:00:0${index}.000Z`, index + 1]);
        }
        await backfillSmsConversations(db);
        let conversation = await db.get('SELECT * FROM sms_conversations WHERE device_id = ?', [deviceId]);
        expect(conversation).toMatchObject({ message_count: 1, unread_count: 1, last_message_preview: 'বাংলা পরীক্ষা', last_message_at: '2026-09-13T10:00:01.000Z' });
        await db.run('UPDATE sms SET read = 1 WHERE conversation_id = ?', [conversation.id]);
        await refreshSmsConversation(db, conversation.id);
        expect(await db.get('SELECT unread_count FROM sms_conversations WHERE id = ?', [conversation.id])).toEqual({ unread_count: 0 });
        await db.run('DELETE FROM sms WHERE conversation_id = ? AND multipart_part_index = 1', [conversation.id]);
        await refreshSmsConversation(db, conversation.id);
        conversation = await db.get('SELECT * FROM sms_conversations WHERE device_id = ?', [deviceId]);
        expect(conversation).toMatchObject({ message_count: 1, last_message_preview: 'পরীক্ষা' });
    });
