...
public class MqttBridgeService extends Service {
    ...
    public void onCreate() {
        ...
        // Restore timeout ownership before any SMS callback can arrive after a
        // process restart. The tracker remains fail-closed if its journal is
        // corrupt or full.
        SmsSendTracker.restore(this);
    ...
    public void onDestroy() {
        ...
        // AND-02: the in-memory publish queue previously evaporated here.
        // Persist it to the durable outbox so the next connection delivers it.
        PendingPublish pending;
        while ((pending = pendingPublishes.poll()) != null) {
            BridgeOutbox.recordPublish(this, pending.topic, pending.payload);
        }
        // AND-06: shutdownNow() discards never-started queued tasks, so the
        // disconnect must run on its own short-lived thread and be awaited.
    ...
    static void publishSentResult(String actionId, String number, int partCount, int timeoutMs, boolean success, String detail) {
        publishSentResult(null, actionId, number, partCount, timeoutMs, success, detail);
    }

    static void publishSentResult(Context context, String actionId, String number, int partCount, int timeoutMs, boolean success, String detail) {
        MqttBridgeService service = activeService;
        if (service == null) {
            // AND-01: the service instance died (process restart or teardown).
            // Persist the terminal result so the dashboard row settles on the
            // next bridge connection instead of staying "accepted" forever.
            if (context != null && actionId != null && !actionId.trim().isEmpty()) {
                JSONObject json = new JSONObject();
                try {
                    json.put("action_id", actionId);
                    json.put("messageId", actionId);
                    json.put("command", "send_sms");
                    json.put("result", success ? "completed" : "failed");
                    json.put("success", success);
                    json.put("terminal", true);
                    json.put("result_code", success ? 0 : 1);
                    json.put("detail", detail == null ? "" : detail);
                    json.put("number", number == null ? "" : number);
                    BridgeOutbox.recordTerminalResult(context, actionId, json.toString());
    ...
    static void publishDelivered(String actionId, String number) {
        publishDelivered(null, actionId, number);
    }

    static void publishDelivered(Context context, String actionId, String number) {
        MqttBridgeService service = activeService;
        if (service == null) {
            if (context != null && actionId != null && !actionId.trim().isEmpty()) {
                JSONObject json = new JSONObject();
                try {
                    ...
                    json.put("messageId", actionId);
                    json.put("number", number == null ? "" : number);
                    BridgeOutbox.recordTerminalResult(context, "delivered_" + actionId, json.toString());
                } catch (JSONException ignored) {
                }
            }
            return;
        ...
        String journalActionId = actionId == null || actionId.trim().isEmpty()
                ? ""
                : "delivered_" + actionId.trim();
        if (!journalActionId.isEmpty()) {
            // Delivery is terminal work too. Persist before handing it to the
            // MQTT executor so a process death cannot erase the carrier event.
            BridgeOutbox.recordTerminalResult(service, journalActionId, json.toString());
            service.publishJson(service.currentConfig().topic("sms/delivered"), json,
                    () -> BridgeOutbox.acknowledgeTerminalByActionId(service, journalActionId));
        } else {
            service.publishJson(service.currentConfig().topic("sms/delivered"), json);
        }
    }
    ...
    private void connectInternal() {
        ...
        try {
            ...
            // Last Will: an abrupt crash (not just clean stops) reports the
            // device offline to the broker so the dashboard reacts instantly.
            try {
                JSONObject offline = new JSONObject();
                offline.put("type", "bridge_offline");
                offline.put("device_id", cfg.deviceId);
                offline.put("timestamp", System.currentTimeMillis());
                options.setWill(cfg.topic("status/lastwill"),
                        offline.toString().getBytes(StandardCharsets.UTF_8), 1, false);
            } catch (JSONException ignored) {
            }
            ...
            if (cfg.usesAutoTransport() && cfg.hasHttpBridgeConfig()) {
                ...
                if (result.success) {
                    ...
                    // AND-01: settle terminal results recorded while the
                    // process was down, through the HTTP lane as well.
                    List<BridgeOutbox.TerminalEntry> terminals = BridgeOutbox.takeTerminalResults(this);
                    for (BridgeOutbox.TerminalEntry entry : terminals) {
                        try {
                            JSONObject json = new JSONObject(entry.payloadJson);
                            String actionId = json.optString("action_id", entry.actionId);
                            JSONObject event = new JSONObject();
                            event.put("event", json.optString("result", "").equalsIgnoreCase("completed")
                                    ? "SENT" : "FAILED");
                            event.put("detail", json.optString("detail", ""));
                            BridgeHttpClient.Result sent = BridgeHttpClient.postMessageEvent(cfg, actionId, event);
                            if (sent.success) {
                                BridgeOutbox.acknowledgeTerminal(this, entry.id);
                            }
                        } catch (Exception invalidPayload) {
                            Log.w(TAG, "Outbox terminal result invalid: " + entry.actionId);
                        }
                    }
                    syncSmsAndCallsToDashboardOnce(consumePendingPermissionBulkSync(cfg));
                    return;
    ...
    private void subscribeAndAnnounce() {
        try {
            ...
            updateBridgeNotification(NOTIFICATION_STATUS_ONLINE);
            updateRuntimeState("online", true, "Connected to " + cfg.brokerUri());
            flushTerminalResultsFromOutbox();
    ...
    private void handleCommand(String topic, String body) {
        executor.execute(() -> {
            BridgeConfig cfg = currentConfig();
            if (!isCommandTopic(cfg, topic)) {
                logConsoleEvent("mqtt", "Ignored command from unauthorized topic");
                return;
            }
            if (body == null || body.getBytes(StandardCharsets.UTF_8).length > 65536) {
                logConsoleEvent("mqtt", "Rejected oversized command envelope");
                return;
            }
            JSONObject data = parseJson(body);
            if (data == null) {
                logConsoleEvent("mqtt", "Rejected malformed command envelope");
                return;
            }
            ...
            if (normalized.isEmpty() || actionId.isEmpty() || !actionId.matches("[A-Za-z0-9_.:-]{1,128}")) {
                logConsoleEvent("mqtt", "Rejected command envelope: invalid command/action ID");
                return;
            }
    ...
    private void handleSendSms(String actionId, JSONObject data) {
        // Dashboard commands may be wrapped in a payload object (for example,
        // HTTP-to-MQTT adapters). Flatten it before reading the SMS contract so
        // both {number,text} and {payload:{number,text}} are accepted.
        JSONObject commandData = flattenCommandPayload(data);
        String number = firstNonEmpty(commandData.optString("number", ""), commandData.optString("to", ""));
        String text = firstNonEmpty(commandData.optString("text", ""), commandData.optString("message", ""), commandData.optString("content", ""));
        int timeoutMs = commandData.optInt("timeout_ms", commandData.optInt("timeoutMs", commandData.optInt("timeout", 90000)));
        Integer preferredSimSlot = requestedSimSlot(commandData);
        Integer preferredSubscriptionId = firstInteger(
                jsonInteger(commandData, "subscription_id"),
                jsonInteger(commandData, "sim_subscription_id"),
                jsonInteger(commandData, "subscriptionId"),
                jsonInteger(commandData, "simSubscriptionId")
        );
        if (commandData.optBoolean("encrypted", false)) {
    ...
    private JSONObject flattenCommandPayload(JSONObject data) {
        if (data == null) {
            return new JSONObject();
        }
        JSONObject nested = data.optJSONObject("payload");
        if (nested == null) {
            nested = data.optJSONObject("data");
        }
        if (nested == null) {
            return data;
        }
        ...
        try {
            java.util.Iterator<String> nestedKeys = nested.keys();
            while (nestedKeys.hasNext()) {
                String key = nestedKeys.next();
                merged.put(key, nested.opt(key));
            }
            java.util.Iterator<String> outerKeys = data.keys();
            while (outerKeys.hasNext()) {
                String key = outerKeys.next();
                // Envelope metadata wins, while payload fields fill the SMS
                // contract when the dashboard wrapped them.
                if (!"payload".equals(key) && !"data".equals(key)) {
    ...
    private void syncExistingSmsToDashboardOnce(boolean allowInitialBackfill) {
        ...
        // AND-02: the watermark previously advanced BEFORE delivery, so a
        // crash skipped records permanently. It now advances only over records
        // that were successfully built and enqueued (with the durable outbox
        // those survive restarts, making this at-least-once with server-side
        // dedupe instead of at-most-once with data loss).
        long enqueuedThrough = watermark;
        ...
        for (java.util.Map<String, Object> message : messages) {
            if (publishSmsSyncRecord(message)) {
                synced += 1;
                long recordTs = objectLong(message.get("timestamp"), 0L);
                if (recordTs > enqueuedThrough) {
                    enqueuedThrough = recordTs;
                }
            }
        }
        ...
        prefs.edit()
                .putBoolean(doneKey, true)
                .putLong(watermarkKey, enqueuedThrough)
                .apply();
    ...
    private static void syncExistingSmsToDashboardOnce(Context context, BridgeConfig cfg, boolean allowInitialBackfill, JsonSender sender) {
        ...
        // AND-02: advance the watermark only over enqueued records.
        long enqueuedThrough = watermark;
        ...
        for (java.util.Map<String, Object> message : messages) {
            ...
            if (payload != null) {
                sender.send(payload);
                synced += 1;
                long recordTs = objectLong(message == null ? null : message.get("timestamp"), 0L);
                if (recordTs > enqueuedThrough) {
                    enqueuedThrough = recordTs;
                }
            }
    ...
    private JSONObject statusPayload(String state, boolean forceDetailed) {
        ...
        try {
            ...
            json.put("call_supported", snapshot.callPhonePermission);
            json.put("ussd_supported", snapshot.callPhonePermission && Build.VERSION.SDK_INT >= 26);
    ...
    private void publishActionResult(String actionId, String command, String result, int resultCode, String detail, JSONObject payload, int timeoutMs) {
        ...
        try {
            ...
            // `accepted` acknowledges enqueueing only. Consumers must wait for
            // a later completed/failed result before settling a durable action.
            json.put("terminal", "completed".equals(result) || "failed".equals(result));
        ...
        boolean terminal = "completed".equals(result) || "failed".equals(result);
        if (terminal && actionId != null && !actionId.trim().isEmpty()) {
            // Persist the terminal envelope before handing it to the MQTT
            // executor. A process death between callback and publish must not
            // leave the dashboard permanently in accepted/pending state.
            BridgeOutbox.recordTerminalResult(this, actionId, json.toString());
        }
        if (currentTransportUsesHttp()) {
            return;
        }
        if (terminal) {
            publishJson(currentConfig().topic("action/result"), json,
                    () -> BridgeOutbox.acknowledgeTerminalByActionId(this, actionId));
        } else {
            publishJson(currentConfig().topic("action/result"), json);
        }
    }
    ...
    private void publishJson(String topic, JSONObject json) {
        publishJson(topic, json, null);
    }

    private void publishJson(String topic, JSONObject json, Runnable onAccepted) {
        executor.execute(() -> {
            MqttClient current = client;
            if (current == null || !current.isConnected()) {
                // Terminal envelopes already live in the terminal journal.
                // Normal publishes use the general journal only; keeping the
                // same entry in RAM and on disk caused a duplicate first replay.
                boolean stored = onAccepted != null
                        || BridgeOutbox.recordPublish(this, topic, json.toString());
                if (stored) {
                    ...
                    logTelemetry("Device send rejected: durable outbox unavailable for "
                            + compactTopic(topic));
                    persistRuntimeTelemetry();
                }
                return;
    ...
    private void flushPendingPublishes() {
        ...
        // In-memory queue drained (or broker dropped): persist the current
        // memory queue so the durable outbox never loses track of it, then
        // drain persisted entries that predate this connection.
        List<BridgeOutbox.PublishEntry> durable = BridgeOutbox.loadPublishes(this);
        List<BridgeOutbox.PublishEntry> remaining = new ArrayList<>();
        boolean stalled = false;
        for (BridgeOutbox.PublishEntry entry : durable) {
            if (stalled) {
                remaining.add(entry);
                continue;
            }
            try {
                ...
                MqttMessage message = new MqttMessage(entry.payload.getBytes(StandardCharsets.UTF_8));
                message.setQos(1);
                current.publish(entry.topic, message);
                publishSuccessCount += 1;
                logTelemetry("Device sent outbox: " + compactTopic(entry.topic));
                recordPublishSuccess(entry.topic);
            } catch (MqttException error) {
                publishFailureCount += 1;
                stalled = true;
                remaining.add(entry);
                Log.e(TAG, "Outbox publish failed: " + entry.topic, error);
            }
        }
        // Also keep anything still queued in memory.
        for (PendingPublish memory : pendingPublishes) {
            remaining.add(new BridgeOutbox.PublishEntry(memory.topic, memory.payload));
        }
        BridgeOutbox.replacePublishes(this, remaining);
    }

    /** AND-01: publish terminal SMS results recorded while the service was
     * dead, then clear them from the durable outbox. */
    private void flushTerminalResultsFromOutbox() {
        List<BridgeOutbox.TerminalEntry> terminals = BridgeOutbox.takeTerminalResults(this);
        for (BridgeOutbox.TerminalEntry entry : terminals) {
            if (entry == null || entry.payloadJson == null || entry.payloadJson.trim().isEmpty()) {
                continue;
            }
            try {
                JSONObject json = new JSONObject(entry.payloadJson);
                String deliveredType = json.optString("type", "");
                if ("sms_delivered".equals(deliveredType)) {
                    publishJson(currentConfig().topic("sms/delivered"), json,
                            () -> BridgeOutbox.acknowledgeTerminal(this, entry.id));
                } else {
                    publishJson(currentConfig().topic("action/result"), json,
                            () -> BridgeOutbox.acknowledgeTerminal(this, entry.id));
                }
                logTelemetry("Outbox terminal result flushed for " + entry.actionId);
    ...
    private static boolean isCommandTopic(BridgeConfig cfg, String topic) {
        if (cfg == null || topic == null) return false;
        String command = cfg.topic("command/");
        String cmd = cfg.topic("cmd/");
        return topic.startsWith(command) || topic.startsWith(cmd);
    }
