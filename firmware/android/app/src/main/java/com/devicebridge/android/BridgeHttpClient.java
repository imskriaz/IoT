    ...
    private static Result requestJson(BridgeConfig config, String method, String path, JSONObject payload) {
        ...
        try {
            if (!config.hasHttpBridgeConfig()) {
                return Result.failure("HTTP bridge config is incomplete");
            }
            // SEC-08: reject cleartext dashboard URLs in release builds
            // (the OS-level network security config also blocks them; this
            // check yields a clear, actionable failure message).
            if (!config.transportSecurityOk()
                    && config.serverUrl.toLowerCase(java.util.Locale.ROOT).startsWith("http://")
                    && !com.devicebridge.android.BuildFlag.ALLOW_CLEARTEXT) {
                return Result.failure("Insecure HTTP blocked: use HTTPS or enable the insecure-transport option");
            }
