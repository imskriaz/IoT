...
$testBuild = Join-Path ([IO.Path]::GetTempPath()) ('iot-sms-cmgr-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    $testExe = Join-Path $testBuild 'sms_cmgr_test.exe'
    & $compiler /nologo /W4 /WX /TC (Join-Path $PSScriptRoot 'sms_cmgr_test.c') "/Fe:$testExe" "/Fo:$testBuild\sms_cmgr_test.obj"
