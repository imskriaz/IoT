...
/* Exact schema-2 layout for data-preserving OTA migration. */
typedef struct {
    uint32_t schema_version;
    bool wifi_primary;
    bool modem_fallback_enabled;
...
static void config_mgr_migrate_v2_config(
    ...
    memcpy(config->device_id_override, legacy->device_id_override, sizeof(legacy->device_id_override));
    memcpy(config->mqtt_topic_prefix, legacy->mqtt_topic_prefix, sizeof(legacy->mqtt_topic_prefix));
    ...
    memcpy(config->mqtt_uri, legacy->mqtt_uri, sizeof(legacy->mqtt_uri));
    ...
    memcpy(config->modem_apn, legacy->modem_apn, sizeof(legacy->modem_apn));

    if (legacy->wifi_primary && legacy->modem_fallback_enabled) {
        config->connection_policy = CONFIG_CONNECTION_DUAL_ACTIVE_DEVELOPMENT;
    } else if (legacy->wifi_primary) {
        config->connection_policy = CONFIG_CONNECTION_WIFI_ONLY;
    } else if (legacy->modem_fallback_enabled) {
    ...
    if (config_mgr_validate(config) != ESP_OK) {
        ESP_LOGW(TAG, "schema-2 config migration invalid, using defaults");
        reset_defaults(config);
    }
}
...
esp_err_t config_mgr_init(void) {
    ...
    if (stored_size == sizeof(v2)) {
        ...
        ESP_LOGW(TAG, "migrating schema-2 config blob (%u bytes -> %u bytes)",
                 (unsigned)stored_size, (unsigned)sizeof(s_config));
        config_mgr_migrate_v2_config(&v2, &s_config);
        s_ready = true;
