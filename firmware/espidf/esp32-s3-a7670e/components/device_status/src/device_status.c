...
esp_err_t device_status_build_json_from_snapshot(
    ...
    err = device_status_append_json(
        ...
        buffer_len,
        &used,
        ",\"sd_mounted\":%s,\"storage_media_available\":%s,\"storage_buffered_only\":%s"
        ",\"storage_queue_depth\":%" PRIu32 ",\"storage_total_bytes\":%s"
        ",\"storage_used_bytes\":%s,\"storage_free_bytes\":%s",
