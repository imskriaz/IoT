...
/* Workers register from their own entry points, so counting only existing
 * expected records could accept an image before a required task ever starts. */
static const char *const s_required_workers[] = {
...
static bool update_runtime_metrics(task_registry_record_t *record, bool force) {
    ...
    if (!record->health_stack_sampled || stack_bytes < record->health_min_stack_bytes) {
        record->health_min_stack_bytes = (uint32_t)stack_bytes;
    }
    record->health_stack_sampled = true;
    if (stack_bytes > 0U &&
...
esp_err_t task_registry_mark_running(const char *name, bool running) {
    ...
    if (running) {
        s_records[index].last_heartbeat_ms = unified_tick_now_ms();
        s_records[index].heartbeat_seen = true;
    }
...
static bool required_record_healthy(const task_registry_record_t *record,
                                    uint32_t now_ms, uint32_t max_age_ms) {
    return record->expected && record->initialized && record->running &&
        record->heartbeat_seen && record->health_stack_sampled &&
        record->health_min_stack_bytes > TASK_REGISTRY_LOW_STACK_THRESHOLD_BYTES &&
        (uint32_t)(now_ms - record->last_heartbeat_ms) <= max_age_ms;
}
...
bool task_registry_required_healthy(uint32_t max_age_ms) {
    ...
    if (!s_ready || !s_lock || max_age_ms == 0U || max_age_ms > INT32_MAX) {
        return false;
    }
    if (xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) != pdTRUE) {
        return false;
    ...
    now_ms = unified_tick_now_ms();
    for (index = 0U; index < sizeof(s_required_workers) / sizeof(s_required_workers[0]); ++index) {
        int record_index = find_index(s_required_workers[index]);
        if (record_index < 0 || !required_record_healthy(&s_records[record_index], now_ms, max_age_ms)) {
            healthy = false;
            break;
        }
    }
    for (index = 0U; healthy && index < TASK_REGISTRY_CAPACITY; ++index) {
        if (s_records[index].expected && !required_record_healthy(&s_records[index], now_ms, max_age_ms)) {
            healthy = false;
        }
    }
    xSemaphoreGive(s_lock);
