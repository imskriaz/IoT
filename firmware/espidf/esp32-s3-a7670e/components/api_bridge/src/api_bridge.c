 ...
 * passes. At most two transitions can be tracked at once; the serialized
 * hardware lane means more would not be meaningful. */
typedef enum {
...
static void api_bridge_begin_wifi_transition(
    const unified_action_envelope_t *action,
    api_bridge_wifi_transition_kind_t kind,
    const char *ssid
) {
    ...
    /* Replace a same-kind pending transition: the newer command supersedes. */
    entry = NULL;
    ...
    if (!entry) {
        /* No free slot: drop the oldest entry. */
        entry = &s_async_transitions.entries[0];
        uint32_t earliest = entry->deadline_ms;
        for (size_t index = 1U; index < API_BRIDGE_ASYNC_TRANSITION_MAX; ++index) {
            if ((int32_t)(s_async_transitions.entries[index].deadline_ms - earliest) < 0) {
                earliest = s_async_transitions.entries[index].deadline_ms;
                entry = &s_async_transitions.entries[index];
            }
        }
    }
...
static unified_action_response_t api_bridge_execute_wifi_reconnect(
    ...
    /* FW-02: the transition is asynchronous. Reply ACCEPTED now; the terminal
     * result is published when Wi-Fi reaches the connected state or the
     * transition deadline expires (api_bridge_poll_async_transitions). */
    api_bridge_begin_wifi_transition(action, API_BRIDGE_WIFI_TRANSITION_CONNECT, wifi.ssid);
...
static unified_action_response_t api_bridge_execute_wifi_connect(
    ...
    /* FW-02: association and DHCP take seconds; reply ACCEPTED and settle the
     * terminal result from the async transition poller. */
    api_bridge_begin_wifi_transition(action, API_BRIDGE_WIFI_TRANSITION_CONNECT, request->ssid);
...
static unified_action_response_t api_bridge_execute_wifi_disconnect(
    ...
    /* FW-02: the driver disconnect event lands asynchronously. */
    api_bridge_begin_wifi_transition(action, API_BRIDGE_WIFI_TRANSITION_DISCONNECT, wifi.ssid);
