...
try {
    ...
    $source = Get-Content -Raw (Join-Path (Split-Path $PSScriptRoot) 'components/mqtt_mgr/src/mqtt_mgr.c')
    $start = $source.IndexOf('static void mqtt_mgr_ota_reset_locked(void) {')
    $end = $source.IndexOf('static void mqtt_mgr_set_health_locked(void) {', $start)
    if ($start -lt 0 -or $end -le $start) { throw 'OTA runtime extraction seam missing' }
    $source.Substring($start, $end - $start) | Set-Content (Join-Path $testBuild 'ota_runtime_production.h')
    $runtimeExe = Join-Path $testBuild 'ota_runtime_test.exe'
    & $compiler /nologo /W4 /WX /TC /D_CRT_SECURE_NO_WARNINGS "/I$testBuild" (Join-Path $PSScriptRoot 'ota_runtime_test.c') "/Fe:$runtimeExe" "/Fo:$testBuild\ota_runtime_test.obj"
    if ($LASTEXITCODE -ne 0) { throw "OTA runtime compile failed: $LASTEXITCODE" }
    & $runtimeExe
    if ($LASTEXITCODE -ne 0) { throw "OTA runtime test failed: $LASTEXITCODE" }
} finally {
