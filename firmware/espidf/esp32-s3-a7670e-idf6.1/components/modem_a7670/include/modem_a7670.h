...
#define MODEM_A7670_MQTT_TLS_CA_CERT_NAME "bridge-ca-pinggy-20260921.pem"
...
typedef struct {
    ...
    char pdp_ip_address[UNIFIED_IPV4_ADDR_LEN];
    char ppp_ip_address[UNIFIED_IPV4_ADDR_LEN];
...
/* PPP is an exclusive UART byte-stream lease. Reserve before disconnecting
 * native modem MQTT so its recovery task cannot race the AT-to-PPP handoff. */
...
/* SMS-02 durability contract: consume_* reads WITHOUT deleting; after the
 * message is persisted and published call delete_consumed_sms() to remove it
 * from the SIM, or clear_consumed_sms() to retry delivery on the next poll. */
esp_err_t modem_a7670_delete_consumed_sms(uint32_t timeout_ms);
void modem_a7670_clear_consumed_sms(void);

/* FW-13: modem-derived UTC wall clock (epoch seconds). False until an SMS
 * service-center timestamp has been observed. */
