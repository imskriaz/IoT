...
#define MODEM_A7670_DUAL_BEARER_RETRY_MS       60000U
...
#define CONFIG_UNIFIED_API_BRIDGE_PAYLOAD_LEN  8192
#endif

#define MODEM_A7670_MQTT_QUEUE_DEPTH      12
#define MODEM_A7670_MQTT_TOPIC_LEN        160
/* Inbound commands do not need the larger outbound status-response budget. */
#define MODEM_A7670_MQTT_PAYLOAD_LEN      4096U
/* A CMQTTRX frame can contain both the topic and payload in one UART burst.
 * Keep the framing scratch large enough for both, plus the modem's headers and
 * CR/LF separators; otherwise a valid maximum-size frame would be truncated
 * before the length checks below can reject or queue it. */
#define MODEM_A7670_UART_PARSE_LINE_LEN \
    (MODEM_A7670_MQTT_PAYLOAD_LEN + MODEM_A7670_MQTT_TOPIC_LEN + 128U)
...
typedef struct {
    ...
    /* Certificate enumeration is multiline and cannot use the 48-byte status
     * detail buffer. Owned by the modem lock; allocated with the PSRAM scratch. */
    char tls_certificate_response[1024];
    ...
    char recv_topic[MODEM_A7670_MQTT_TOPIC_LEN];
    char recv_payload[MODEM_A7670_MQTT_PAYLOAD_LEN];
    char ussd_response_buffer[4096U];
...
static bool s_mqtt_rx_frame_started;
...
static uint32_t s_next_dual_bearer_retry_ms;
...
void modem_a7670_parse_response_locked(const char *response, size_t response_len);
...
static esp_err_t modem_a7670_mqtt_publish_locked(
    const char *topic,
    const char *payload,
    int qos,
    char *response,
...
static bool modem_a7670_clock_is_certificate_usable(const char *response) {
    const char *clock = response ? strstr(response, "+CCLK: \"") : NULL;
    unsigned year = 0U;

    return clock && sscanf(clock, "+CCLK: \"%2u/", &year) == 1 && year >= 24U && year < 70U;
}

static esp_err_t modem_a7670_configure_mqtt_tls_locked(
    char *response,
    size_t response_len,
    uint32_t timeout_ms
) {
    ...
    if (!s_runtime_scratch) {
        snprintf(response, response_len, "%s", "mqtt_tls_scratch_unavailable");
        return ESP_ERR_INVALID_STATE;
    }
    err = modem_a7670_send_command_locked(
        "AT+CCERTLIST",
        s_runtime_scratch->tls_certificate_response,
        sizeof(s_runtime_scratch->tls_certificate_response),
        timeout_ms,
        false
    );
    if (err != ESP_OK) {
        snprintf(response, response_len, "%s", "mqtt_tls_ca_query_failed");
        return err;
    }
    if (!modem_a7670_response_has_phrase(
            s_runtime_scratch->tls_certificate_response,
            "\"" MODEM_A7670_MQTT_TLS_CA_CERT_NAME "\"")) {
        snprintf(response, response_len, "%s", "mqtt_tls_ca_missing");
        return ESP_ERR_NOT_FOUND;
    }

    err = modem_a7670_send_command_locked("AT+CCLK?", response, response_len, timeout_ms, false);
    if (err != ESP_OK) {
        snprintf(response, response_len, "%s", "mqtt_tls_clock_query_failed");
        return err;
    }
    if (!modem_a7670_clock_is_certificate_usable(response)) {
        err = modem_a7670_send_command_locked(
        ...
        if (err == ESP_OK) {
            err = modem_a7670_send_command_locked("AT+CNTP", response, response_len, timeout_ms, false);
        }
        if (err == ESP_OK && !modem_a7670_response_has_phrase(response, "+CNTP: 0")) {
            err = modem_a7670_read_response_until_phrase_locked(response, response_len, timeout_ms, "+CNTP: 0");
        }
        if (err != ESP_OK || !modem_a7670_response_has_phrase(response, "+CNTP: 0")) {
            snprintf(response, response_len, "%s", "mqtt_tls_clock_sync_failed");
            return err == ESP_OK ? ESP_FAIL : err;
        }
        err = modem_a7670_send_command_locked("AT+CCLK?", response, response_len, timeout_ms, false);
        if (err != ESP_OK || !modem_a7670_clock_is_certificate_usable(response)) {
            snprintf(response, response_len, "%s", "mqtt_tls_clock_invalid");
    ...
    if (err != ESP_OK) {
        snprintf(response, response_len, "%s", "mqtt_tls_context_failed");
        return err;
    }
...
static bool modem_a7670_extract_subscriber_number_candidate(const char *line, char *out_value, size_t out_len) {
    ...
    out_value[0] = '\0';
    if (strncmp(line, "+CNUM:", 6) != 0 && strncmp(line, "+CPBR:", 6) != 0) {
        return false;
    }
    /* Both responses place the number in field TWO, not the first quoted
     * string. Alpha labels (e.g. "SIM1") and CPBR indexes are not numbers. */
    for (cursor = line + 6; *cursor && *cursor != '\r' && *cursor != '\n'; ++cursor) {
    ...
    if (*cursor++ != '"' || digits == 0U || (size_t)(end - start) >= out_len) return false;
    while (*cursor == ' ' || *cursor == '\t') ++cursor;
    if (*cursor != ',') return false;
    memcpy(out_value, start, (size_t)(end - start));
    out_value[end - start] = '\0';
    return true;
}
...
static void modem_a7670_reset_uart_parse_fragment_locked(void) {
    ...
    s_runtime_scratch->parse_fragment_used = 0U;
}
...
static size_t modem_a7670_parse_cmqttrx_length(const char *line) {
    ...
    if (end == marker || value >= MODEM_A7670_MQTT_PAYLOAD_LEN) {
        return 0U;
    }
...
static bool modem_a7670_parse_cmqttrx_start_lengths(const char *line, size_t *topic_len, size_t *payload_len) {
    ...
    /* The modem has one configured MQTT client. Reject another client index
     * or trailing garbage instead of entering a frame with untrusted lengths. */
    char trailing = '\0';
    if (sscanf(line, "+CMQTTRXSTART: %d,%u,%u %c", &client_index,
            &parsed_topic_len, &parsed_payload_len, &trailing) != 3 ||
        client_index != 0) {
        return false;
    }
...
static bool modem_a7670_mqtt_topic_is_command(const char *topic) {
    ...
    return strstr(topic, "/command/") != NULL;
}
...
static void modem_a7670_queue_completed_mqtt_rx_locked(void) {
    ...
    if (!s_mqtt_rx_frame_started ||
        s_mqtt_rx_topic_bytes != s_mqtt_rx_expected_topic_len ||
        s_mqtt_rx_payload_bytes != s_mqtt_rx_expected_payload_len ||
        !modem_a7670_mqtt_topic_is_command(s_mqtt_rx_topic)) {
        modem_a7670_reset_mqtt_rx_locked();
        return;
    }

    /* The command lane is text, but UART/MQTT lengths are byte counts. Never
     * let an embedded NUL turn a full wire message into a shorter C string. */
    if (memchr(s_mqtt_rx_topic, '\0', s_mqtt_rx_topic_bytes) != NULL ||
        !s_mqtt_rx_payload || memchr(s_mqtt_rx_payload, '\0', s_mqtt_rx_payload_bytes) != NULL) {
        snprintf(s_status.runtime.last_error_text, sizeof(s_status.runtime.last_error_text), "%s", "mqtt_rx_invalid_text");
        modem_a7670_reset_mqtt_rx_locked();
        return;
    }
...
static void modem_a7670_queue_sms_index_locked(int index) {
    ...
    if (s_sms_count < CONFIG_UNIFIED_MODEM_EVENT_QUEUE_DEPTH) {
        ...
        /* Never overwrite the retained in-flight head. Storage scanning can
         * recover the unqueued new item later. */
        s_status.timeout_count++;
        return;
    }
...
bool modem_a7670_queue_sms_delivery_locked(const unified_sms_delivery_payload_t *payload) {
    ...
    for (size_t i = 0U; i < s_sms_delivery_count; ++i) {
        const unified_sms_delivery_payload_t *queued =
            &s_sms_delivery_queue[(s_sms_delivery_head + i) % CONFIG_UNIFIED_MODEM_EVENT_QUEUE_DEPTH];
        if (payload->storage_index > 0 && queued->storage_index == payload->storage_index) {
            return true;
        }
    }
    ...
    if (s_sms_delivery_count < CONFIG_UNIFIED_MODEM_EVENT_QUEUE_DEPTH) {
        ...
        s_status.timeout_count++;
        /* A stored report remains readable in the modem for the next scan. */
        return false;
    }
...
static void modem_a7670_refresh_subscriber_number_locked(char *response, size_t response_len, uint32_t timeout_ms) {
    ...
    char command[40] = {0};
    char previous_storage[3] = {0};
    char number[sizeof(s_status.subscriber_number)] = {0};
    const char *marker;
    int used = 0, total = 0, first = 0, last = 0;
    bool changed_storage;
    ...
    /* ON is the own-number store. SM/ME are contacts, never subscriber truth.
     * Snapshot the selection first; do not alter it if it cannot be restored. */
    if (modem_a7670_command("AT+CPBS?", phonebook_response, sizeof(phonebook_response), timeout_ms) != ESP_OK) return;
    marker = strstr(phonebook_response, "+CPBS:");
    if (!marker || sscanf(marker, "+CPBS: \"%2[A-Z]\",%d,%d", previous_storage, &used, &total) != 3) return;
    changed_storage = strcmp(previous_storage, "ON") != 0;
    if (changed_storage && modem_a7670_command("AT+CPBS=\"ON\"", phonebook_response, sizeof(phonebook_response), timeout_ms) != ESP_OK) {
        goto restore_phonebook;
    }
    if (modem_a7670_command("AT+CPBS?", phonebook_response, sizeof(phonebook_response), timeout_ms) != ESP_OK) goto restore_phonebook;
    marker = strstr(phonebook_response, "+CPBS:");
    if (!marker || sscanf(marker, "+CPBS: \"ON\",%d,%d", &used, &total) != 2 || used <= 0 || total <= 0) goto restore_phonebook;
    if (modem_a7670_command("AT+CPBR=?", phonebook_response, sizeof(phonebook_response), timeout_ms) != ESP_OK) goto restore_phonebook;
    marker = strstr(phonebook_response, "+CPBR:");
    if (!marker || sscanf(marker, "+CPBR: (%d-%d)", &first, &last) != 2 || first < 1 || last < first) goto restore_phonebook;
    /* Bound response size without assuming ON has twenty entries (this SIM has
     * three). Never ask for indexes outside the modem's advertised range. */
    if (last - first > 19) last = first + 19;
    snprintf(command, sizeof(command), "AT+CPBR=%d,%d", first, last);
    if (modem_a7670_command(command, phonebook_response, sizeof(phonebook_response), timeout_ms) == ESP_OK &&
        modem_a7670_parse_phonebook_subscriber_response_locked(phonebook_response, number, sizeof(number))) {
        snprintf(s_status.subscriber_number, sizeof(s_status.subscriber_number), "%s", number);
    }

restore_phonebook:
    if (changed_storage) {
        snprintf(command, sizeof(command), "AT+CPBS=\"%s\"", previous_storage);
        if (modem_a7670_command(command, phonebook_response, sizeof(phonebook_response), timeout_ms) != ESP_OK) {
            ESP_LOGW(TAG, "Own-number lookup could not restore phonebook selection");
        }
    }
}
...
static void modem_a7670_parse_line_locked(const char *line) {
    ...
    if (strncmp(line, "+CMQTTRXSTART:", 13) == 0) {
        ...
        if (modem_a7670_parse_cmqttrx_start_lengths(line, &topic_len, &payload_len)) {
            s_mqtt_rx_frame_started = topic_len > 0U && payload_len > 0U;
    ...
    if (strncmp(line, "+CMQTTRXEND:", 11) == 0) {
        ...
        const bool topic_complete = s_mqtt_rx_frame_started &&
            s_mqtt_rx_topic_bytes == s_mqtt_rx_expected_topic_len;
        const bool payload_complete = s_mqtt_rx_frame_started &&
            s_mqtt_rx_payload_bytes == s_mqtt_rx_expected_payload_len;
    ...
    if (strncmp(line, "+CMQTTRECV:", 11) == 0) {
        /* The parser also runs on serial/control callers. Keep its 4 KiB
         * scratch off every caller's stack, under the existing modem lock. */
        char *recv_topic = s_runtime_scratch->recv_topic;
        char *recv_payload = s_runtime_scratch->recv_payload;
        recv_topic[0] = '\0';
        recv_payload[0] = '\0';

        ESP_LOGI(TAG, "CMQTTRECV bytes=%u", (unsigned)strlen(line));
        s_mqtt_service_started = true;
        s_mqtt_connected = true;
        if (modem_a7670_parse_cmqttrecv_line(line, recv_topic, sizeof(s_runtime_scratch->recv_topic),
                recv_payload, sizeof(s_runtime_scratch->recv_payload))) {
            modem_a7670_queue_mqtt_message_locked(recv_topic, recv_payload);
        } else {
            ESP_LOGW(TAG, "CMQTTRECV parse failed");
        }
        return;
    }
    ...
    } else if (strncmp(line, "+CMTI:", 6) == 0 || strncmp(line, "+CDSI:", 6) == 0) {
...
/* MODEM_IP_PARSER_TEST_BEGIN */
static bool modem_a7670_parse_ipv4_token(const char *value, char *out_ip, size_t out_len) {
    char candidate[UNIFIED_IPV4_ADDR_LEN] = {0};
    const char *end = NULL;
    size_t length = 0U;
    unsigned int octet0 = 0U;
    unsigned int octet1 = 0U;
    unsigned int octet2 = 0U;
    unsigned int octet3 = 0U;
    char trailing = '\0';

    if (!value || !out_ip || out_len == 0U) {
        return false;
    }
    ...
    length = end ? (size_t)(end - value) : strlen(value);
    while (length > 0U && value[length - 1U] == ' ') {
        --length;
    }
    if (length == 0U || length >= sizeof(candidate) || length >= out_len) {
        return false;
    }

    snprintf(candidate, sizeof(candidate), "%.*s", (int)length, value);
    if (sscanf(candidate, "%u.%u.%u.%u%c", &octet0, &octet1, &octet2, &octet3, &trailing) != 4 ||
        octet0 > 255U || octet1 > 255U || octet2 > 255U || octet3 > 255U ||
        (octet0 == 0U && octet1 == 0U && octet2 == 0U && octet3 == 0U)) {
        return false;
    }

    snprintf(out_ip, out_len, "%s", candidate);
    return true;
}
...
static bool modem_a7670_parse_ipaddr_response(const char *response, char *out_ip, size_t out_len) {
    ...
    return modem_a7670_parse_ipv4_token(value, out_ip, out_len);
}

static bool modem_a7670_parse_cgpaddr_response(const char *response, char *out_ip, size_t out_len) {
    const char *marker = NULL;
    const char *value = NULL;
    ...
    out_ip[0] = '\0';
    marker = strstr(response, "+CGPADDR:");
    if (!marker) {
        return false;
    }
    value = strchr(marker + strlen("+CGPADDR:"), ',');
    if (!value) {
        return false;
    }
    return modem_a7670_parse_ipv4_token(value + 1, out_ip, out_len);
}
/* MODEM_IP_PARSER_TEST_END */
...
static esp_err_t modem_a7670_start_task_locked(void) {
    ...
    task_ok = xTaskCreatePinnedToCore(
        modem_a7670_task,
        "modem_task",
        /* SMS/PDU parsing and modem response handling use bounded but sizeable
         * local frames. Keep this lane above the measured parse-line frame so
         * a multipart read cannot corrupt adjacent heap metadata. */
...
bool modem_a7670_uart_control_blocked_locked(void) {
    return !modem_a7670_ppp_at_allowed();
}
...
static esp_err_t modem_a7670_probe_uart_sideband(bool refresh_data_session) {
    ...
    if (!s_telephony_baseline_configured) {
        if (
            ...
            /* Both messages (+CMTI) and status reports (+CDSI) stay in storage until publication. */
            modem_a7670_command("AT+CNMI=2,1,0,2,0", response, sizeof(scratch->probe_response), CONFIG_UNIFIED_MODEM_AT_TIMEOUT_MS) == ESP_OK) {
...
static esp_err_t modem_a7670_refresh_data_session_locked(uint32_t timeout_ms) {
    ...
    if (modem_a7670_send_command_locked("AT+IPADDR", response, sizeof(scratch->data_session_response), timeout_ms, false) == ESP_OK &&
        ...
        unified_copy_cstr(s_status.pdp_ip_address, sizeof(s_status.pdp_ip_address), ip_address);
    ...
    memset(response, 0, sizeof(scratch->data_session_response));
    memset(ip_address, 0, sizeof(scratch->data_session_ip_address));
    if (modem_a7670_send_command_locked("AT+CGPADDR=1", response, sizeof(scratch->data_session_response), timeout_ms, false) == ESP_OK &&
        modem_a7670_parse_cgpaddr_response(response, ip_address, sizeof(scratch->data_session_ip_address))) {
        unified_copy_cstr(s_status.pdp_ip_address, sizeof(s_status.pdp_ip_address), ip_address);
    } else {
        s_status.pdp_ip_address[0] = '\0';
    }
...
void modem_a7670_parse_response_locked(const char *response, size_t response_len) {
    ...
    fragment_used = scratch->parse_fragment_used;
    ...
    while (cursor < (fragment_used + copy_len)) {
        ...
        /* Keep a split CRLF intact. A raw MQTT body can itself begin with a
         * newline; never strip arbitrary CR/LF bytes from that body. */
        if (has_line_ending && fragment[line_end] == '\r' &&
            line_end + 1U == fragment_used + copy_len) {
            break;
        }
        if (!has_line_ending) {
        ...
        if (line_len > 0U) {
            ...
            if (memchr(line, '\0', line_len) == NULL) {
                ...
                /* Corrupt AT/framing lines cannot be accepted by prefix. */
                modem_a7670_reset_mqtt_rx_locked();
            }
        }

        cursor = line_end + 1U;
        if (fragment[line_end] == '\r' && cursor < fragment_used + copy_len &&
            fragment[cursor] == '\n') {
            cursor++;
        }
    }

    scratch->parse_fragment_used = fragment_used + copy_len - cursor;
    if (cursor > 0U) {
    ...
    snprintf(s_status.last_response, sizeof(s_status.last_response), "%.*s", (int)response_len, response);
}
...
static esp_err_t modem_a7670_read_until_quiet_bounded_locked(
    ...
    while (esp_timer_get_time() < deadline_us) {
        ...
        read_buffer[bytes] = '\0';
        /* Parse every consumed UART byte once, independently of the caller's
         * bounded diagnostic response. A full response must not clip URCs. */
        modem_a7670_parse_response_locked(read_buffer, (size_t)bytes);
...
static esp_err_t modem_a7670_wait_mqtt_result_locked(
    char *response,
    size_t response_len,
    uint32_t timeout_ms,
    const char *prefix,
    ...
    size_t used = 0U;
    int64_t deadline_us = 0;
    modem_mqtt_result_t result;

    if (!response || response_len == 0U || !prefix || !out_code) return ESP_ERR_INVALID_ARG;
    *out_code = -1;
    used = strnlen(response, response_len);
    if (used >= response_len) return ESP_ERR_INVALID_SIZE;
    if (modem_a7670_uart_control_blocked_locked()) return ESP_ERR_INVALID_STATE;

    result = modem_mqtt_result_parse(response, response_len, prefix);
    if (result.state != MODEM_MQTT_RESULT_PENDING) {
        if (result.state == MODEM_MQTT_RESULT_COMPLETE) *out_code = result.code;
        return result.state == MODEM_MQTT_RESULT_COMPLETE ? ESP_OK : ESP_FAIL;
    }
    deadline_us = esp_timer_get_time() + ((int64_t)timeout_ms * 1000LL);
    while (esp_timer_get_time() < deadline_us && used < response_len - 1U) {
        ...
        read_buffer[bytes] = '\0';
        modem_a7670_parse_response_locked(read_buffer, (size_t)bytes);
        if (memchr(read_buffer, '\0', (size_t)bytes) != NULL) return ESP_FAIL;
        esp_err_t append_err = modem_a7670_append_response_chunk(response, response_len, &used, read_buffer);
        if (append_err != ESP_OK) return append_err;
        result = modem_mqtt_result_parse(response, response_len, prefix);
        if (result.state != MODEM_MQTT_RESULT_PENDING) {
            if (result.state == MODEM_MQTT_RESULT_COMPLETE) *out_code = result.code;
            return result.state == MODEM_MQTT_RESULT_COMPLETE ? ESP_OK : ESP_FAIL;
        }
        if (modem_a7670_response_has_error(response)) return ESP_FAIL;
    }
    return used >= response_len - 1U ? ESP_ERR_INVALID_SIZE : ESP_ERR_TIMEOUT;
}
...
static esp_err_t modem_a7670_open_network_stack_locked(char *response, size_t response_len, uint32_t timeout_ms) {
    ...
    if (err != ESP_OK || !modem_a7670_parse_ipaddr_response(ip_response, ip_address, sizeof(ip_address))) {
        memset(ip_response, 0, sizeof(ip_response));
        memset(ip_address, 0, sizeof(ip_address));
        if (modem_a7670_send_command_locked("AT+CGPADDR=1", ip_response, sizeof(ip_response), timeout_ms, false) == ESP_OK &&
            modem_a7670_parse_cgpaddr_response(ip_response, ip_address, sizeof(ip_address))) {
            unified_copy_cstr(s_status.pdp_ip_address, sizeof(s_status.pdp_ip_address), ip_address);
        } else {
            s_status.pdp_ip_address[0] = '\0';
        }
...
static void modem_a7670_task(void *arg) {
    ...
    while (true) {
        /* PPP owns every UART byte while active or transitioning.  Do not run
         * AT probes, URC parsing, SMS work, or native CMQTT reads against the
         * same serial stream.  The PPP adapter performs bounded binary drains
         * and forwards them to esp-netif/lwIP. */
        if (modem_a7670_ppp_uart_owned()) {
            if (s_uart_event_queue == NULL ||
                xQueueReceive(s_uart_event_queue, &uart_event, pdMS_TO_TICKS(100)) != pdTRUE) {
                /* CONNECT can consume the event but leave binary bytes in the
                 * UART ring. Poll bounded input even without a new event. */
                uart_event.type = UART_DATA;
            }
            modem_a7670_ppp_process_uart_event(&uart_event);
        ...
        /* Reservation can begin while the worker is blocked on its queue. */
        if (modem_a7670_ppp_uart_owned()) {
            if (probe_due) uart_event.type = UART_DATA;
            modem_a7670_ppp_process_uart_event(&uart_event);
            ESP_ERROR_CHECK(task_registry_heartbeat("modem_task"));
            continue;
        }
        if (probe_due) {
            ...
            if (config_mgr_connection_policy() == CONFIG_CONNECTION_DUAL_ACTIVE_DEVELOPMENT) {
                modem_a7670_status_t live = {0};
                uint32_t now_ms = unified_tick_now_ms();
                modem_a7670_get_status(&live);
                if (live.sim_ready && live.network_registered &&
                    (!live.data_session_open || !live.ip_bearer_ready || live.data_ip_address[0] == '\0') &&
                    (s_next_dual_bearer_retry_ms == 0U ||
                     (int32_t)(now_ms - s_next_dual_bearer_retry_ms) >= 0)) {
                    esp_err_t bearer_err = modem_a7670_open_data_session(
                        scratch->probe_response,
                        sizeof(scratch->probe_response),
                        CONFIG_UNIFIED_MODEM_AT_TIMEOUT_MS
                    );
                    s_next_dual_bearer_retry_ms = bearer_err == ESP_OK
                        ? 0U
                        : now_ms + MODEM_A7670_DUAL_BEARER_RETRY_MS;
                }
            } else {
                s_next_dual_bearer_retry_ms = 0U;
            }
...
esp_err_t modem_a7670_reset_modem(char *response, size_t response_len, uint32_t timeout_ms) {
    ...
    if (err == ESP_OK) {
        ...
        if (xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) == pdTRUE) {
            ...
            s_status.data_session_open = false;
            s_status.ip_bearer_ready = false;
            modem_a7670_ppp_note_modem_reset_locked();
            xSemaphoreGive(s_lock);
        }
    }
    return err;
...
static esp_err_t modem_a7670_mqtt_subscribe_locked(
    ...
    if (err == ESP_OK) {
        err = modem_a7670_wait_mqtt_result_locked(response, response_len, timeout_ms, "+CMQTTSUB:", &suback_code);
        if (err == ESP_OK && suback_code != 0) err = ESP_FAIL;
    }

    if (err != ESP_OK) {
        ESP_LOGW(
            TAG,
            "CMQTTSUBTOPIC flow topic=%s failed err=%s response_redacted=1; retrying with direct CMQTTSUB",
            topic,
            esp_err_to_name(err)
        );
        response[0] = '\0';
    ...
    if (err != ESP_OK) {
        ESP_LOGW(
            TAG,
            "CMQTTSUB topic=%s failed err=%s response_redacted=1",
            topic,
            esp_err_to_name(err)
        );
    } else {
...
esp_err_t modem_a7670_mqtt_connect(
    ...
    if (use_tls) {
        err = modem_a7670_configure_mqtt_tls_locked(response, response_len, timeout_ms);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "MQTTS context setup failed err=%s detail=%s", esp_err_to_name(err), response);
            xSemaphoreGive(s_lock);
            return err;
        }
    }
    ...
    for (int attempt = 0; attempt < 2; ++attempt) {
        ...
        if (err == ESP_OK || modem_a7670_response_has_mqtt_stop_done(response)) {
            err = modem_a7670_wait_mqtt_result_locked(response, response_len, timeout_ms, "+CMQTTSTART:", &start_code);
        }
        if (err == ESP_OK && start_code == 0) {
            break;
        }
        if (s_mqtt_connected) {
            snprintf(response, response_len, "%s", "mqtt_already_connected");
        ...
        if (attempt == 0) {
            ESP_LOGW(
                TAG,
                "CMQTTSTART attempt=%d failed err=%s response_redacted=1; retrying after forced shutdown",
                attempt + 1,
    ...
    if (err != ESP_OK || start_code != 0) {
        ESP_LOGW(
            TAG,
            "CMQTTSTART failed err=%s response_redacted=1",
            esp_err_to_name(err == ESP_OK ? ESP_FAIL : err)
        );
        xSemaphoreGive(s_lock);
        return err == ESP_OK ? ESP_FAIL : err;
    ...
    if (err != ESP_OK &&
        ...
        ESP_LOGW(
            TAG,
            "CMQTTACCQ failed err=%s response_redacted=1",
            esp_err_to_name(err),
    ...
    if (use_tls) {
        err = modem_a7670_send_command_locked("AT+CMQTTSSLCFG=0,0", response, response_len, timeout_ms, false);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "CMQTTSSLCFG failed err=%s response_redacted=1", esp_err_to_name(err));
            (void)modem_a7670_mqtt_shutdown_locked(response, response_len, timeout_ms);
            xSemaphoreGive(s_lock);
            return err;
        }
    ...
    err = modem_a7670_send_command_locked(command, response, response_len, timeout_ms, false);
    if (err == ESP_OK) {
        err = modem_a7670_wait_mqtt_result_locked(response, response_len, timeout_ms, "+CMQTTCONNECT:", &connect_code);
    }
    if (err != ESP_OK || connect_code != 0) {
        bool reopen_data_session = connect_code == 3;

        ESP_LOGW(
            TAG,
            "CMQTTCONNECT failed err=%s response_redacted=1",
            esp_err_to_name(err == ESP_OK ? ESP_FAIL : err)
        );
        (void)modem_a7670_mqtt_shutdown_locked(response, response_len, timeout_ms);
...
static esp_err_t modem_a7670_mqtt_publish_locked(
    ...
    err = modem_a7670_send_command_locked(command, response, response_len, timeout_ms, false);
    if (err == ESP_OK) {
        err = modem_a7670_wait_mqtt_result_locked(response, response_len, timeout_ms, "+CMQTTPUB:", &publish_code);
        if (err == ESP_OK && publish_code != 0) err = ESP_FAIL;
    }
...
esp_err_t modem_a7670_mqtt_publish(
    ...
    err = modem_a7670_mqtt_publish_locked(topic, payload, qos, response, response_len, timeout_ms);
...
bool modem_a7670_mqtt_is_connected(void) {
    ...
    if (!s_ready || !s_lock || modem_a7670_ppp_uart_owned()) {
        return false;
    }
    if (xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) != pdTRUE) {
...
bool modem_a7670_peek_sms_delivery(unified_sms_delivery_payload_t *out_payload) {
    bool has_value = false;
    if (!out_payload || !s_lock || xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) != pdTRUE) {
        return false;
    }
...
bool modem_a7670_peek_sms_index(int *out_index) {
    bool found = false;
    if (!out_index || !s_lock || xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) != pdTRUE) return false;
    if (s_sms_count > 0U) {
        *out_index = s_sms_index_queue[s_sms_head];
        found = true;
    }
...
void modem_a7670_defer_sms_index(int index) {
    if (!s_lock || xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) != pdTRUE) return;
    /* Remove only this notification after bounded retries; never delete the
     * modem message. Background scanning remains its recovery mechanism. */
    modem_a7670_drop_sms_indexes_locked(&index, 1U);
    xSemaphoreGive(s_lock);
}
