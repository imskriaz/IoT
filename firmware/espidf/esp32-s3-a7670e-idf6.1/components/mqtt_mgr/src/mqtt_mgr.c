...
extern const char mqtt_root_ca_start[] asm("_binary_mqtt_root_ca_pem_start");
#include "cJSON.h"
...
#define MQTT_MGR_MODEM_SUBSCRIBE_RETRY_MS 15000U
...
enum {
    ...
    MQTT_SUBSCRIBE_TOPIC_COUNT = 1
};
static int s_subscribe_ids[MQTT_SUBSCRIBE_TOPIC_COUNT] = {-1};
...
static uint32_t s_ota_validation_retry_ms;
static mqtt_mgr_transport_t s_ota_validation_transport;
...
static uint8_t s_modem_subscribe_failure_count;
static uint32_t s_next_modem_subscribe_retry_ms;
...
static void mqtt_mgr_ota_reset_locked(void);
static void mqtt_mgr_ota_tick_locked(void);
...
static esp_err_t mqtt_mgr_stop_esp_client_for_recovery_locked(void);
...
static esp_err_t mqtt_mgr_stop_transport_locked(void) {
    mqtt_mgr_ota_reset_locked();
    char response[UNIFIED_TEXT_MEDIUM_LEN] = {0};

    /* The ESP candidate can be running beside the selected modem path. Stop
     * the actual client before clearing its lifecycle flag. */
    ...
    if (s_transport == MQTT_MGR_TRANSPORT_MODEM) {
        ...
        if (disconnect_err != ESP_OK && modem_a7670_mqtt_is_connected()) {
            s_status.runtime.last_error = disconnect_err;
            snprintf(s_status.runtime.last_error_text,
                     sizeof(s_status.runtime.last_error_text),
                     "%s", "mqtt_modem_stop_failed");
    ...
    s_modem_subscribe_failure_count = 0U;
    s_next_modem_subscribe_retry_ms = 0U;
    s_next_modem_connect_retry_ms = 0U;
    s_next_esp_start_retry_ms = 0U;
...
/* ESP ingress/recovery owns only the ESP client, even when the modem is the
 * selected application transport. Full transport stop also tears down modem
 * MQTT, which is not an ESP ingress recovery operation. */
static esp_err_t mqtt_mgr_stop_esp_client_for_recovery_locked(void) {
    if (s_client_started && s_client) {
        esp_err_t stop_err = esp_mqtt_client_stop(s_client);
        if (stop_err != ESP_OK) {
...
static esp_err_t mqtt_mgr_subscribe_commands_locked(void) {
    ...
    memset(s_subscribe_acked, 0, sizeof(s_subscribe_acked));
    s_suback_deadline_ms = 0U;
    /* Native 6.1 uses the single current dashboard/device command contract. */
    if (mqtt_mgr_subscribe_topic_locked("command/#") != ESP_OK) {
        return ESP_FAIL;
    }

    s_suback_deadline_ms = mqtt_subscription_deadline(
        unified_tick_now_ms(), MQTT_MGR_SUBACK_TIMEOUT_MS);
...
static esp_err_t mqtt_mgr_subscribe_modem_command_topics_locked(void) {
    ...
    char topic[160] = {0};
    char response[UNIFIED_TEXT_MEDIUM_LEN] = {0};
    esp_err_t err = mqtt_mgr_build_topic_locked("command/#", topic, sizeof(topic));
    if (err == ESP_OK) {
        err = modem_a7670_mqtt_subscribe(topic, response, sizeof(response), 5000U);
    }
    if (err == ESP_OK) {
    ...
    s_status.subscribed = false;
    if (s_modem_subscribe_failure_count < UINT8_MAX) {
        s_modem_subscribe_failure_count++;
    }
    s_next_modem_subscribe_retry_ms = mqtt_failover_retry_deadline(
        unified_tick_now_ms(),
        mqtt_failover_retry_delay_ms(
            s_modem_subscribe_failure_count,
            MQTT_MGR_MODEM_SUBSCRIBE_RETRY_MS,
            MQTT_MGR_MODEM_RESUBSCRIBE_MS
        ));
    s_status.runtime.last_error = err;
...
static esp_err_t mqtt_mgr_parse_broker_uri(const char *uri, char *host, size_t host_len, uint16_t *port) {
    ...
    *port = 1883U;
    ...
    if (strncmp(start, "mqtts://", 8) == 0) {
        start += 8;
        *port = 8883U;
    } else if (strncmp(start, "ssl://", 6) == 0) {
        start += 6;
        *port = 8883U;
    } else if (strncmp(start, "mqtt://", 7) == 0) {
        start += 7;
...
static void mqtt_mgr_ota_reset_locked(void) {
    ...
    s_ota_validation_retry_ms = 0U;
    s_ota_validation_transport = MQTT_MGR_TRANSPORT_NONE;
    s_status.ota_validation_action_id[0] = '\0';
}

static void mqtt_mgr_ota_tick_locked(void) {
    state_mgr_snapshot_t state = {0};
    bool subscription_ready = s_transport == MQTT_MGR_TRANSPORT_ESP
        ? mqtt_mgr_primary_subscription_ready_locked()
        : s_status.subscribed;
    state_mgr_get_snapshot(&state);
    if (strcmp(state.ota_state, "valid") == 0) s_ota_confirmed = true;
    if (s_ota_confirmed || strcmp(state.ota_state, "pending_verify") != 0 ||
        !s_status.connected || !subscription_ready || s_transport == MQTT_MGR_TRANSPORT_NONE ||
        (s_transport == MQTT_MGR_TRANSPORT_MODEM && !modem_a7670_mqtt_is_connected())) {
        mqtt_mgr_ota_reset_locked();
        return;
    }
    uint32_t now_ms = unified_tick_now_ms();
    if (s_ota_validation_transport != s_transport || !ota_validation_live(&s_ota_validation, now_ms)) {
        char challenge[32];
        mqtt_mgr_ota_reset_locked();
        snprintf(challenge, sizeof(challenge), "ota_%08" PRIx32 "%08" PRIx32 "%08" PRIx32,
                 esp_random(), esp_random(), esp_random());
        (void)ota_validation_begin(&s_ota_validation, challenge, now_ms);
        s_ota_validation_transport = s_transport;
        memcpy(s_status.ota_validation_action_id, s_ota_validation.action_id,
               sizeof(s_status.ota_validation_action_id));
    }
    if (s_ota_validation.acknowledged &&
        (s_ota_validation_retry_ms == 0U || mqtt_subscription_expired(s_ota_validation_retry_ms, now_ms))) {
        /* Keep proof while internal-stack main_task checks worker health.
         * Transient lock/health failure must not be stranded by journal ACK. */
        (void)state_mgr_request_ota_validation();
        s_ota_validation_retry_ms = now_ms + 5000U;
    }
}

static void mqtt_mgr_ota_ack(const char *action_id, unified_action_command_t command) {
    if (command != UNIFIED_ACTION_CMD_GET_STATUS ||
        xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) != pdTRUE) return;
    mqtt_mgr_ota_tick_locked();
    if (ota_validation_ack(&s_ota_validation, action_id, unified_tick_now_ms(),
                           s_status.connected && s_status.subscribed)) {
        mqtt_mgr_ota_tick_locked();
    }
    xSemaphoreGive(s_lock);
}
...
static esp_err_t mqtt_mgr_refresh_config_locked(void) {
    ...
    broker_tls = strncmp(scratch->config.mqtt_uri, "mqtts://", 8) == 0 ||
    ...
    if (s_transport != MQTT_MGR_TRANSPORT_NONE || s_client_started) {
        esp_err_t stop_err = mqtt_mgr_stop_transport_locked();
        if (stop_err != ESP_OK) {
            return stop_err;
        }
    }
    ...
    /* Public deployment trust anchor, matching the modem CA; never a key or password. */
    if (s_broker_tls) {
        scratch->mqtt_config.broker.verification.certificate = mqtt_root_ca_start;
    }
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    if (event_id == MQTT_EVENT_SUBSCRIBED) {
        ...
        copy->invalid = event->data_len <= 0 || !event->data ||
            ...
            (event->topic_len > 0 && (!event->topic ||
                memchr(event->topic, '\0', (size_t)event->topic_len) != NULL)) ||
...
static void mqtt_mgr_process_esp_event_locked(const mqtt_mgr_ingress_event_t *event) {
    ...
    switch ((esp_mqtt_event_id_t)event->event_id) {
        ...
        case MQTT_EVENT_SUBSCRIBED:
            /* A SUBACK is valid only for the connected session that issued
             * the current subscribe request.  Late ACKs after disconnect or
             * recovery must not resurrect readiness. */
            if (!mqtt_suback_current_session(
                    s_status.connected,
            ...
            for (size_t index = 0U; index < MQTT_SUBSCRIBE_TOPIC_COUNT; ++index) {
                ...
                if (event->invalid) {
                    s_subscribe_ids[index] = -1;
                    s_subscribe_acked[index] = false;
                    if (mqtt_subscription_rejection_requires_recovery(true, true)) {
                        s_status.runtime.last_error = ESP_FAIL;
        ...
        case MQTT_EVENT_DATA:
            {
                ...
                s_status.command_messages++;
                xSemaphoreGive(s_lock);
                /* Validate the original byte count before either C-string
                 * parser: an ACK must not reclaim a result by NUL prefix. */
                if (!unified_json_text_is_cstring_safe(s_ingress->payload, s_ingress->received)) {
                    submit_err = ESP_ERR_INVALID_ARG;
                } else if (mqtt_mgr_is_result_ack_topic(s_ingress->topic)) {
...
static void mqtt_mgr_task(void *arg) {
    ...
    bool publish_ussd_results = false;
    ...
    while (true) {
        ...
        if (xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) == pdTRUE) {
            ...
            if (s_status.configured && s_client) {
                ...
                if (recovery_action != MQTT_RECOVERY_NONE) {
                    esp_err_t recovery_err = recovery_action == MQTT_RECOVERY_STOP
                        ? mqtt_mgr_stop_esp_client_for_recovery_locked()
                ...
                if (mqtt_mgr_policy_allows_wifi(&scratch->config) &&
                    ...
                    if (esp_connect_stalled && modem_fallback_ready) {
                        ...
                        if (mqtt_mgr_stop_transport_locked() == ESP_OK &&
                            mqtt_mgr_start_modem_client_locked() == ESP_OK) {
                            modem_mqtt_connected = modem_a7670_mqtt_is_connected();
                        }
                    ...
                    if (s_transport == MQTT_MGR_TRANSPORT_ESP &&
                        ...
                        if (mqtt_mgr_subscribe_commands_locked() == ESP_OK) {
                           ...
                           mqtt_mgr_modem_fallback_ready(&scratch->config, &scratch->modem)) {
                    uint32_t now_ms = unified_tick_now_ms();
                    bool ready_to_select_modem = true;
                    if (s_transport == MQTT_MGR_TRANSPORT_ESP) {
                        ready_to_select_modem = mqtt_mgr_stop_transport_locked() == ESP_OK;
                    }
                    if (ready_to_select_modem &&
                        s_transport == MQTT_MGR_TRANSPORT_MODEM && modem_mqtt_connected) {
                        s_status.connected = true;
                        s_status.runtime.running = true;
                    } else if (ready_to_select_modem) {
                        if (s_transport == MQTT_MGR_TRANSPORT_MODEM) {
                            s_status.connected = false;
                            s_status.subscribed = false;
                            s_status.runtime.running = false;
                        ...
                        if (mqtt_failover_retry_pending(s_next_modem_connect_retry_ms, now_ms)) {
                            /* Keep the actual connection failure visible during
                             * backoff; waiting is not a new timeout/failure. */
                            if (!s_status.runtime.last_error_text[0]) {
                                snprintf(
                                    s_status.runtime.last_error_text,
                                    sizeof(s_status.runtime.last_error_text),
                                    "%s",
            ...
            publish_ussd_results = loop_connected;
        ...
        /* USSD publication also needs a current locked transport snapshot;
         * never reuse the prior loop's connection bit after lock contention. */
        publish_ussd_results = false;
        if (modem_mqtt_connected) {
            /* A lock timeout must not drain modem commands while the ESP
             * client may already own ingress. Retry on the next loop. */
            bool process_modem_messages = false;
            bool resubscribe_modem = false;
            if (xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) == pdTRUE) {
                if (!s_modem_connection_seen) {
                    s_modem_connection_seen = true;
                }
                process_modem_messages = mqtt_modem_ingress_allowed(
                    s_transport == MQTT_MGR_TRANSPORT_ESP,
                    s_status.connected,
                    s_status.subscribed,
                    s_suback_deadline_ms,
                ...
                if (process_modem_messages) {
                    resubscribe_modem = !s_status.subscribed &&
                        mqtt_failover_retry_due(
                            s_next_modem_subscribe_retry_ms,
                            unified_tick_now_ms()
                        );
                    s_transport = MQTT_MGR_TRANSPORT_MODEM;
                    s_status.connected = true;
                ...
                publish_ussd_results = s_status.connected;
                xSemaphoreGive(s_lock);
            }
            if (process_modem_messages) {
...
void mqtt_mgr_get_status(mqtt_mgr_status_t *out_status) {
    ...
    if (!s_lock || xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) != pdTRUE) {
        /* No unlocked struct copy: concurrent updates can tear counters and
         * the validation challenge. Missing observation is safer than fiction. */
        return;
    }
...
static esp_err_t mqtt_mgr_process_result_ack(const char *topic, const char *payload) {
    ...
    if (!unified_json_text_is_cstring_safe(payload, strlen(payload))) return err;
    ...
    cJSON_ArrayForEach(field, root) {
        unsigned bit = 0U;
        if (!field->string) goto done;
        if (strcmp(field->string, "schema_version") == 0) bit = 1U;
        else if (strcmp(field->string, "device_id") == 0) bit = 2U;
        else if (strcmp(field->string, "action_id") == 0) bit = 4U;
        else if (strcmp(field->string, "command") == 0) bit = 8U;
        else goto done;
        if (seen & bit) goto done;
        seen |= bit;
    }
    if (seen != 15U) goto done;
    ...
    /* Rejected unknown commands are journaled as NONE. They still require an
     * exact device/action/command DB ACK, otherwise they permanently block the
     * oldest-first durable result queue. NONE here never admits execution. */
    bool command_found = false;
    for (int candidate = UNIFIED_ACTION_CMD_NONE;
         candidate <= UNIFIED_ACTION_CMD_WIFI_PROFILES_APPLY; ++candidate) {
...
static void mqtt_mgr_publish_durable_result(void) {
    ...
    if (!s_ready || !s_lock || !s_action_records ||
        xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) != pdTRUE) {
        return;
    }
    retry_due = s_status.connected &&
        (s_next_durable_result_retry_ms == 0U ||
         mqtt_subscription_expired(s_next_durable_result_retry_ms, now_ms));
    if (retry_due) {
        s_next_durable_result_retry_ms = mqtt_subscription_deadline(now_ms, 10000U);
    }
    xSemaphoreGive(s_lock);
    if (!retry_due) {
        return;
    ...
    /* Count successful durable send attempts too (including modem transport).
     * Delivery/DB acknowledgement remains owned by the journal, not this metric. */
    ...
    if (err == ESP_OK &&
        xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) == pdTRUE) {
        mqtt_mgr_ota_tick_locked();
        if (!s_ota_confirmed && s_status.connected && s_status.subscribed &&
            ...
            /* Committed success must belong to this boot and the fresh
             * challenge. This arms proof; only the later exact DB ACK settles
             * it, on either transport. No retain metadata is invented. */
            ...
            (void)ota_validation_published(&s_ota_validation,
...
static void mqtt_mgr_publish_recent_action_results(void) {
    ...
    for (index = 0; index < count; ++index) {
        ...
        if (!s_action_records[index].replay &&
            s_action_records[index].response.action.correlation.correlation_id[0] &&
