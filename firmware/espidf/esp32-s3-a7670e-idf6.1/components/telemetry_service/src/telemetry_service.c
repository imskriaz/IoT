...
#define TELEMETRY_STATUS_BUFFER_LEN              DEVICE_STATUS_JSON_MAX_LEN
...
static uint32_t telemetry_snapshot_fingerprint(const device_status_snapshot_t *snapshot) {
    ...
    hash = telemetry_hash_text(hash, snapshot->ota_state);
    hash = telemetry_hash_text(hash, snapshot->ota_validation_action_id);
    ...
    hash = telemetry_hash_u32(hash, snapshot->wifi_profiles_revision);
    hash = telemetry_hash_u32(hash, snapshot->wifi_profiles_count);
    ...
    hash = telemetry_hash_bool(hash, snapshot->sd_detected);
    hash = telemetry_hash_text(hash, snapshot->sd_error);
    hash = telemetry_hash_u32(hash, (uint32_t)(snapshot->sd_capacity_bytes >> 32));
    hash = telemetry_hash_u32(hash, (uint32_t)snapshot->sd_capacity_bytes);
...
static void telemetry_service_task(void *arg) {
    ...
    while (true) {
        ...
        /* A 60s idle sample can repeatedly publish an almost-expired 60s
         * challenge. Sample pending proof promptly without flooding status:
         * the fingerprint below publishes only challenge/state changes. */
        if (mqtt.ota_validation_action_id[0] && sample_interval_ms > 5000U) sample_interval_ms = 5000U;
