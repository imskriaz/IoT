...
#define CONFIG_POLICY_KEY "conn_policy"
#define CONFIG_VERSION   CONFIG_MGR_RUNTIME_SCHEMA_VERSION
#define CONFIG_APPLY_QUEUE_DEPTH 4U
#define CONFIG_APPLY_ENQUEUE_MS 250U
#define CONFIG_APPLY_WAIT_MS 10000U
...
static void config_mgr_apply_job_release(config_mgr_apply_job_t *job) {
    bool destroy = false;
    if (!job || !job->state_lock) return;
    if (xSemaphoreTake(job->state_lock, portMAX_DELAY) == pdTRUE) {
        if (job->references > 0U) job->references--;
        destroy = job->references == 0U;
        xSemaphoreGive(job->state_lock);
    }
    if (destroy) {
        vSemaphoreDelete(job->done);
        vSemaphoreDelete(job->state_lock);
        heap_caps_free(job);
    }
}
...
const char *config_mgr_connection_policy_name(config_connection_policy_t policy) {
    switch (policy) {
        case CONFIG_CONNECTION_WIFI_ONLY: return "wifi_only";
        case CONFIG_CONNECTION_WIFI_CELLULAR_FALLBACK: return "wifi_cellular_fallback";
        case CONFIG_CONNECTION_CELLULAR_ONLY: return "cellular_only";
        case CONFIG_CONNECTION_CELLULAR_WIFI_FALLBACK: return "cellular_wifi_fallback";
        case CONFIG_CONNECTION_DUAL_ACTIVE_DEVELOPMENT: return "dual_active_development";
        default: return "unknown";
    }
}

bool config_mgr_parse_connection_policy(const char *value, config_connection_policy_t *out_policy) {
    if (!value || !out_policy) {
        return false;
    }
    for (uint32_t candidate = 0U; candidate < (uint32_t)CONFIG_CONNECTION_POLICY_COUNT; ++candidate) {
        config_connection_policy_t policy = (config_connection_policy_t)candidate;
        if (strcmp(value, config_mgr_connection_policy_name(policy)) == 0) {
            *out_policy = policy;
            return true;
        }
    }
...
static config_connection_policy_t config_mgr_derive_connection_policy(
    bool wifi_primary,
    bool modem_fallback_enabled
) {
    if (wifi_primary && modem_fallback_enabled) {
        return CONFIG_CONNECTION_DUAL_ACTIVE_DEVELOPMENT;
    }
    if (wifi_primary) {
        return CONFIG_CONNECTION_WIFI_ONLY;
    }
    if (modem_fallback_enabled) {
        return CONFIG_CONNECTION_CELLULAR_WIFI_FALLBACK;
    }
    return CONFIG_CONNECTION_CELLULAR_ONLY;
}

static esp_err_t config_mgr_migrate_v2_config(
    const config_mgr_v2_data_t *legacy,
    ...
    config_mgr_expand_rollback_v2(legacy, policy, config);
    err = config_mgr_validate(config);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "schema-2 config invalid; preserving NVS record and using RAM defaults");
        reset_defaults(config);
    }
    return err;
}

static esp_err_t config_mgr_migrate_legacy_config(
    ...
    err = config_mgr_validate(config);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "legacy config invalid; preserving NVS record and using RAM defaults");
        reset_defaults(config);
    }
    return err;
}

static esp_err_t config_mgr_migrate_minimal_v1_config(
    ...
    err = config_mgr_validate(config);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "minimal v1 config invalid; preserving NVS record and using RAM defaults");
        reset_defaults(config);
    }
    return err;
}
...
esp_err_t config_mgr_validate(const config_mgr_data_t *config) {
    ...
    if (config->connection_policy >= (uint32_t)CONFIG_CONNECTION_POLICY_COUNT) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t config_mgr_init(void) {
    ...
    config_mgr_v2_data_t v2 = {0};
    uint32_t stored_policy = (uint32_t)CONFIG_CONNECTION_POLICY_COUNT;
    bool policy_present = false;
    esp_err_t migration_err = ESP_OK;
    ...
    err = nvs_get_u32(handle, CONFIG_POLICY_KEY, &stored_policy);
    if (err == ESP_OK && stored_policy < (uint32_t)CONFIG_CONNECTION_POLICY_COUNT) {
        policy_present = true;
    } else if (err != ESP_ERR_NVS_NOT_FOUND && err != ESP_OK) {
        ESP_LOGW(TAG, "connection policy read failed (%s); deriving from rollback config",
                 esp_err_to_name(err));
    } else if (err == ESP_OK) {
        ESP_LOGW(TAG, "stored connection policy invalid (%" PRIu32 "); deriving from rollback config",
                 stored_policy);
    }
    ...
    if (stored_size == sizeof(loaded)) {
        ...
        s_config = loaded;
        s_ready = true;
        /* One-time migration away from the schema-3 monolithic record. */
        err = config_mgr_store(&s_config);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "failed to rewrite rollback-compatible config (%s)",
                     esp_err_to_name(err));
        }
        return ESP_OK;
    }

    if (stored_size == sizeof(v2)) {
        size_t actual_size = stored_size;
        err = nvs_get_blob(handle, CONFIG_KEY, &v2, &actual_size);
        if (err != ESP_OK) {
            nvs_close(handle);
            return err;
        }

        if (v2.schema_version != CONFIG_MGR_ROLLBACK_SCHEMA_VERSION) {
            nvs_close(handle);
            ESP_LOGE(TAG, "536-byte config has unexpected schema %" PRIu32
                     "; preserving NVS record and using RAM defaults", v2.schema_version);
            s_ready = true;
            return ESP_OK;
        }

        config_connection_policy_t policy = policy_present
            ? (config_connection_policy_t)stored_policy
            : config_mgr_derive_connection_policy(v2.wifi_primary, v2.modem_fallback_enabled);
        nvs_close(handle);
        migration_err = config_mgr_migrate_v2_config(&v2, policy, &s_config);
        s_ready = true;
        if (migration_err == ESP_OK && !policy_present) {
            err = config_mgr_store(&s_config);
            if (err != ESP_OK) {
                ESP_LOGW(TAG, "failed to persist derived connection policy (%s)",
                         esp_err_to_name(err));
            }
        }
        return ESP_OK;
    ...
    if (stored_size == sizeof(legacy)) {
        ...
        migration_err = config_mgr_migrate_legacy_config(&legacy, &s_config);
        s_ready = true;
        if (migration_err == ESP_OK) {
            err = config_mgr_store(&s_config);
            if (err != ESP_OK) {
                ESP_LOGW(TAG, "legacy config migration could not be persisted (%s)",
                         esp_err_to_name(err));
            }
        }
        return ESP_OK;
    ...
    if (stored_size == sizeof(minimal_v1)) {
        ...
        ESP_LOGW(TAG, "migrating minimal v1 config blob (%u bytes -> %u bytes)",
                 (unsigned)stored_size, (unsigned)sizeof(s_config));
        migration_err = config_mgr_migrate_minimal_v1_config(&minimal_v1, &s_config);
        s_ready = true;
        if (migration_err == ESP_OK) {
            err = config_mgr_store(&s_config);
            if (err != ESP_OK) {
                ESP_LOGW(TAG, "minimal-v1 config migration could not be persisted (%s)",
                         esp_err_to_name(err));
            }
        }
        return ESP_OK;
    ...
    nvs_close(handle);
    ESP_LOGE(TAG, "stored config size %u unsupported; preserving NVS record and using RAM defaults",
...
config_connection_policy_t config_mgr_connection_policy(void) {
    config_connection_policy_t value = CONFIG_CONNECTION_DUAL_ACTIVE_DEVELOPMENT;
    ...
    if (s_config.connection_policy < (uint32_t)CONFIG_CONNECTION_POLICY_COUNT) {
        value = (config_connection_policy_t)s_config.connection_policy;
    }
    xSemaphoreGive(s_lock);
    return value;
}
...
esp_err_t config_mgr_store(const config_mgr_data_t *config) {
    ...
    config_mgr_project_rollback_v2(&next, &persisted);
    err = nvs_open(CONFIG_NAMESPACE, NVS_READWRITE, &handle);
    if (err == ESP_OK) {
        err = nvs_set_blob(handle, CONFIG_KEY, &persisted, sizeof(persisted));
        if (err == ESP_OK) {
            err = nvs_set_u32(handle, CONFIG_POLICY_KEY, next.connection_policy);
        }
        if (err == ESP_OK) {
...
esp_err_t config_mgr_apply_key_value(
    ...
    if (strcmp(key, "storage_enabled") == 0) {
        ...
        if (bool_value) {
            next.connection_policy = next.modem_fallback_enabled
                ? CONFIG_CONNECTION_WIFI_CELLULAR_FALLBACK
                : CONFIG_CONNECTION_WIFI_ONLY;
        } else {
            next.connection_policy = next.modem_fallback_enabled
                ? CONFIG_CONNECTION_CELLULAR_WIFI_FALLBACK
                : CONFIG_CONNECTION_CELLULAR_ONLY;
        }
        ...
        if (next.wifi_primary) {
            next.connection_policy = bool_value
                ? CONFIG_CONNECTION_WIFI_CELLULAR_FALLBACK
                : CONFIG_CONNECTION_WIFI_ONLY;
        } else {
            next.connection_policy = bool_value
    ...
    } else if (strcmp(key, "connection_policy") == 0) {
        config_connection_policy_t policy = CONFIG_CONNECTION_DUAL_ACTIVE_DEVELOPMENT;
        if (!config_mgr_parse_connection_policy(value, &policy)) {
            return ESP_ERR_INVALID_ARG;
        }
        next.connection_policy = (uint32_t)policy;
        next.wifi_primary = policy == CONFIG_CONNECTION_WIFI_ONLY ||
            policy == CONFIG_CONNECTION_WIFI_CELLULAR_FALLBACK ||
            policy == CONFIG_CONNECTION_DUAL_ACTIVE_DEVELOPMENT;
        next.modem_fallback_enabled = policy == CONFIG_CONNECTION_WIFI_CELLULAR_FALLBACK ||
            policy == CONFIG_CONNECTION_CELLULAR_WIFI_FALLBACK ||
            policy == CONFIG_CONNECTION_DUAL_ACTIVE_DEVELOPMENT;
    ...
    if (memcmp(&current, &next, sizeof(next)) == 0) {
        ...
        if (out_sensitive) *out_sensitive = sensitive;
        return ESP_OK;
    }
...
static void config_mgr_apply_worker(void *arg) {
    (void)arg;
    config_mgr_apply_job_t *job = NULL;
    while (true) {
        if (xQueueReceive(s_apply_queue, &job, portMAX_DELAY) != pdTRUE || !job) continue;
        esp_err_t result = ESP_ERR_TIMEOUT;
        bool restart_required = false;
        bool sensitive = false;
        if ((int32_t)(xTaskGetTickCount() - job->deadline) < 0) {
            result = config_mgr_apply_key_value(job->key, job->value,
                                                &restart_required, &sensitive);
        }
        if (xSemaphoreTake(job->state_lock, portMAX_DELAY) == pdTRUE) {
            job->err = result;
        ...
        config_mgr_apply_job_release(job);
        job = NULL;
    }
}

static esp_err_t config_mgr_ensure_apply_worker(void) {
    if (!s_lock) return ESP_ERR_INVALID_STATE;
    if (xSemaphoreTake(s_lock, pdMS_TO_TICKS(250)) != pdTRUE) return ESP_ERR_TIMEOUT;
    if (!s_apply_queue) {
        s_apply_queue = xQueueCreate(CONFIG_APPLY_QUEUE_DEPTH, sizeof(config_mgr_apply_job_t *));
        if (s_apply_queue && xTaskCreatePinnedToCore(
                config_mgr_apply_worker, "cfg_apply", CONFIG_UNIFIED_TASK_STACK_SMALL,
                NULL, tskIDLE_PRIORITY + 2, &s_apply_task, 1) != pdPASS) {
            vQueueDelete(s_apply_queue);
            s_apply_queue = NULL;
            s_apply_task = NULL;
        }
    }
    const esp_err_t result = s_apply_queue && s_apply_task ? ESP_OK : ESP_ERR_NO_MEM;
    xSemaphoreGive(s_lock);
    return result;
}
...
esp_err_t config_mgr_apply_key_value_safe(
    ...
    err = config_mgr_ensure_apply_worker();
    ...
    job->done = done;
    job->state_lock = xSemaphoreCreateMutex();
    if (!job->state_lock) {
    ...
    job->references = 2U;
    job->deadline = xTaskGetTickCount() + pdMS_TO_TICKS(CONFIG_APPLY_WAIT_MS);
    if (xQueueSend(s_apply_queue, &job, pdMS_TO_TICKS(CONFIG_APPLY_ENQUEUE_MS)) != pdTRUE) {
        config_mgr_apply_job_release(job);
        config_mgr_apply_job_release(job);
        return ESP_ERR_NO_MEM;
    }

    if (xSemaphoreTake(done, pdMS_TO_TICKS(CONFIG_APPLY_WAIT_MS)) != pdTRUE) {
        config_mgr_apply_job_release(job);
        return ESP_ERR_TIMEOUT;
    }
    if (xSemaphoreTake(job->state_lock, portMAX_DELAY) == pdTRUE) {
        err = job->err;
        if (out_restart_required) *out_restart_required = job->restart_required;
        if (out_sensitive) *out_sensitive = job->sensitive;
        xSemaphoreGive(job->state_lock);
    } else {
        err = ESP_ERR_TIMEOUT;
    }
