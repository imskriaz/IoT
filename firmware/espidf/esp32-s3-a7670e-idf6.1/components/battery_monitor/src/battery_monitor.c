...
static i2c_master_dev_handle_t s_gauge_device;
...
esp_err_t battery_monitor_init(void) {
    const i2c_master_bus_config_t bus_config = {
        .i2c_port = BATTERY_MONITOR_I2C_PORT,
    ...
    const i2c_device_config_t device_config = {
        ...
        .scl_speed_hz = BATTERY_MONITOR_I2C_FREQ_HZ,
    };
    ...
    if (err == ESP_OK) {
        err = i2c_master_bus_add_device(s_i2c_bus, &device_config, &s_gauge_device);
    }
    if (err != ESP_OK) {
        if (s_i2c_bus) {
            const esp_err_t cleanup_err = i2c_del_master_bus(s_i2c_bus);
            if (cleanup_err != ESP_OK) {
                ESP_LOGW(BATTERY_MONITOR_TAG, "i2c cleanup failed: %s", esp_err_to_name(cleanup_err));
            }
            s_i2c_bus = NULL;
        }
        s_gauge_device = NULL;
    ...
    if (task_ok != pdPASS) {
        if (s_gauge_device) {
            (void)i2c_master_bus_rm_device(s_gauge_device);
            s_gauge_device = NULL;
        }
        if (s_i2c_bus) {
