...
#define RESULT_JOURNAL_VERSION_V1 1U
#define RESULT_JOURNAL_VERSION_V2 2U
/* Disk sizes are a format contract, not a build-time buffer setting. Changing
 * CONFIG_UNIFIED_API_BRIDGE_PAYLOAD_LEN must never invalidate existing files.
 * Keep fitting records (including ACK tombstones) readable by old firmware.
 * Drain v2 results before rollback: old firmware can only recover the previous
 * v1 reservation as uncertain, not replay the larger terminal payload. */
#define RESULT_JOURNAL_PAYLOAD_V1 4096U
#define RESULT_JOURNAL_PAYLOAD_V2 8192U
typedef char result_payload_config_must_fit[
    (CONFIG_UNIFIED_API_BRIDGE_PAYLOAD_LEN >= RESULT_JOURNAL_PAYLOAD_V1 &&
     CONFIG_UNIFIED_API_BRIDGE_PAYLOAD_LEN <= RESULT_JOURNAL_PAYLOAD_V2) ? 1 : -1];
...
typedef struct {
    ...
    char payload[RESULT_JOURNAL_PAYLOAD_V2];
} result_disk_record_t;

/* V1 used the native 32-bit action ABI. Shared-model drift requires an explicit
 * decoder/migration, never silently changing the persistent layout. */
typedef char result_disk_layout_must_remain_compatible[
    (sizeof(unified_action_response_t) == 140U &&
     offsetof(unified_action_response_t, result) == 80U &&
     offsetof(unified_action_response_t, detail) == 92U &&
     offsetof(unified_action_envelope_t, command) == 68U &&
     offsetof(unified_correlation_t, device_id) == 32U &&
     offsetof(result_disk_record_t, checksum) == 12U &&
     offsetof(result_disk_record_t, state) == 16U &&
     offsetof(result_disk_record_t, response) == 20U &&
     offsetof(result_disk_record_t, payload) == 160U &&
     sizeof(result_disk_record_t) == 8352U) ? 1 : -1];
...
static size_t result_disk_size(uint32_t version) {
    if (version == RESULT_JOURNAL_VERSION_V1) {
        return offsetof(result_disk_record_t, payload) + RESULT_JOURNAL_PAYLOAD_V1;
    }
    if (version == RESULT_JOURNAL_VERSION_V2) {
        return sizeof(result_disk_record_t);
    }
    return 0U;
}
...
static uint32_t result_checksum(const result_disk_record_t *record) {
    ...
    size_t length = result_disk_size(record->version) - offsetof(result_disk_record_t, state);
...
static bool result_read(unsigned slot, unsigned bank, result_disk_record_t *record) {
    ...
    size_t count = 0U;
    size_t disk_size = 0U;
    const size_t header_size = offsetof(result_disk_record_t, state);
    int extra = 0;
    ...
    count = fread(record, 1U, header_size, file);
    if (count != header_size || record->magic != RESULT_JOURNAL_MAGIC ||
        !(disk_size = result_disk_size(record->version))) {
        (void)fclose(file);
        return false;
    }
    count += fread((unsigned char *)record + header_size, 1U, disk_size - header_size, file);
    extra = fgetc(file);
    (void)fclose(file);
    return count == disk_size && extra == EOF &&
        record->generation != 0U &&
        record->state >= RESULT_RESERVED && record->state <= RESULT_ACKED &&
        record->checksum == result_checksum(record) &&
        memchr(record->payload, '\0', disk_size - offsetof(result_disk_record_t, payload)) &&
        memchr(record->response.action.correlation.device_id, '\0',
               sizeof(record->response.action.correlation.device_id)) &&
        memchr(record->response.action.correlation.correlation_id, '\0',
               sizeof(record->response.action.correlation.correlation_id)) &&
        memchr(record->response.detail, '\0', sizeof(record->response.detail));
}
...
static esp_err_t result_write(unsigned slot, unsigned bank, result_disk_record_t *record) {
    ...
    record->version = strlen(record->payload) < RESULT_JOURNAL_PAYLOAD_V1
        ? RESULT_JOURNAL_VERSION_V1 : RESULT_JOURNAL_VERSION_V2;
    disk_size = result_disk_size(record->version);
    record->checksum = result_checksum(record);
    file = fopen(path, "wb");
    if (!file) {
    ...
    okay = fwrite(record, 1U, disk_size, file) == disk_size &&
...
esp_err_t storage_mgr_result_reserve(const unified_action_envelope_t *action) {
    ...
    for (unsigned slot = 0U; slot < RESULT_JOURNAL_SLOTS; ++slot) {
        ...
        if (found && result_same_action(s_result_current, action)) {
            err = s_result_current->state == RESULT_TERMINAL || s_result_current->state == RESULT_ACKED
    ...
    for (unsigned offset = 0U; offset < RESULT_JOURNAL_SLOTS; ++offset) {
        ...
        if (result_load_slot(slot, s_result_current, &bank) && s_result_current->state == RESULT_TERMINAL) {
            *out_response = s_result_current->response;
            if (strlen(s_result_current->payload) >= payload_len) {
                err = ESP_ERR_INVALID_ARG;
                break;
            }
            snprintf(out_payload, payload_len, "%s", s_result_current->payload);
...
esp_err_t storage_mgr_result_get(const unified_action_envelope_t *action,
    ...
    for (unsigned slot = 0U; slot < RESULT_JOURNAL_SLOTS; ++slot) {
        unsigned bank = 0U;
        if (result_load_slot(slot, s_result_current, &bank) &&
            (s_result_current->state == RESULT_TERMINAL || s_result_current->state == RESULT_ACKED) &&
...
esp_err_t storage_mgr_result_ack(const char *device_id, const char *action_id,
    ...
    for (unsigned slot = 0U; slot < RESULT_JOURNAL_SLOTS; ++slot) {
        ...
        if (s_result_current->generation == 0U) s_result_current->generation = 1U;
        s_result_current->state = RESULT_ACKED;
        /* ACK releases capacity, not the original outcome. Keep the body until
         * this bounded slot is reused so redelivery can replay without execution. */
        err = result_write(slot, 1U - bank, s_result_current);
        break;
    }
done:
