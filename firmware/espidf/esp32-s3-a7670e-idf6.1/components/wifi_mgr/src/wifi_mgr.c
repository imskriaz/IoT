...
static bool s_time_sync_initialized;
#include "esp_wifi.h"
...
static const uint32_t WIFI_MGR_RUNTIME_OVERRIDE_TRIAL_MS = 60000U;
...
static uint32_t s_runtime_override_started_ms;
...
static void wifi_mgr_clear_runtime_override_locked(void) {
    if (!s_runtime_override_active) return;
    s_runtime_override_active = false;
    s_runtime_override_ssid[0] = '\0';
    memset(s_runtime_override_password, 0, sizeof(s_runtime_override_password));
    s_runtime_override_revision++;
}

bool wifi_mgr_runtime_override_active_locked(void) {
    return s_runtime_override_active;
}

uint32_t wifi_mgr_runtime_override_revision_locked(void) {
    return s_runtime_override_revision;
}
...
bool wifi_mgr_refresh_config_locked(config_mgr_data_t *out_config) {
    ...
    uint32_t profiles_revision = 0U;
    bool auto_available = false;
    if (wifi_mgr_profiles_auto_state(&auto_available, &profiles_revision) != ESP_OK) return false;
    bool configured = s_runtime_override_active ? s_runtime_override_ssid[0] != '\0' : auto_available;

    if (!out_config &&
        config_revision != 0U &&
        s_config_revision == config_revision &&
        s_status.configured == configured &&
        s_applied_profiles_revision == profiles_revision &&
    ...
    s_config_revision = config_revision;
    bool profiles_changed = s_applied_profiles_revision != 0U && s_applied_profiles_revision != profiles_revision;
    s_applied_profiles_revision = profiles_revision;
    s_applied_runtime_override_revision = runtime_override_revision;
    ...
    if (s_status.configured != configured) {
        changed = true;
    } else if (profiles_changed && !s_runtime_override_active && !s_status.connected) {
        changed = true;
    } else if (s_runtime_override_active && !s_status.connected && strncmp(s_status.ssid, config.wifi_ssid, sizeof(s_status.ssid)) != 0) {
        changed = true;
    }

    s_status.configured = configured;
    if (!s_status.connected && s_runtime_override_active) {
        snprintf(s_status.ssid, sizeof(s_status.ssid), "%s", config.wifi_ssid);
    }

    if (!s_status.configured && !s_status.connected) {
        s_status.ssid[0] = '\0';
...
static void wifi_mgr_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data) {
    ...
    if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP) {
        ...
        /* TLS certificate dates need wall time. Initialize once on usable Wi-Fi. */
        if (!s_time_sync_initialized) {
            ...
            s_time_sync_initialized = time_err == ESP_OK || time_err == ESP_ERR_INVALID_STATE;
...
static void wifi_mgr_task(void *arg) {
    ...
    uint32_t attempt_override_revision = 0U;
    uint32_t attempt_profiles_revision = 0U;
    ...
    while (true) {
        ...
        if (xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) == pdTRUE) {
            bool was_configured = s_status.configured;
            bool config_changed = wifi_mgr_refresh_config_locked(NULL);
            if ((config_changed || !was_configured) && s_status.configured) wifi_mgr_request_connect_locked();
            if (s_status.connected && esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
                s_status.rssi = ap_info.rssi;
                snprintf(s_status.ssid, sizeof(s_status.ssid), "%s", (const char *)ap_info.ssid);
                snprintf(
                    s_status.security,
            ...
            /* A one-shot manual target must not indefinitely exclude saved
             * profiles after failed association or DHCP. A successful link
             * keeps the target until it is lost, then the expired trial is
             * released on the next worker pass. */
            if (s_runtime_override_active && !s_status.connected) {
                uint32_t elapsed_ms = now_ms - s_runtime_override_started_ms;
                if (elapsed_ms >= WIFI_MGR_RUNTIME_OVERRIDE_TRIAL_MS) {
                    wifi_mgr_clear_runtime_override_locked();
                    (void)wifi_mgr_refresh_config_locked(NULL);
                    s_next_connect_attempt_ms = 0U;
                    wifi_mgr_reset_absent_retry_backoff();
                    if (s_status.configured && s_status.started &&
                        !s_runtime_connect_suppressed && !s_startup_connect_suppressed) {
                        wifi_mgr_request_connect_locked();
                    }
                } else if (WIFI_MGR_RUNTIME_OVERRIDE_TRIAL_MS - elapsed_ms < delay_ms) {
                    delay_ms = WIFI_MGR_RUNTIME_OVERRIDE_TRIAL_MS - elapsed_ms;
                }
            }
            ...
            if (s_connect_requested
                ...
                if (s_next_connect_attempt_ms != 0U && (int32_t)(s_next_connect_attempt_ms - now_ms) > 0) {
                    connect_due_in_ms = s_next_connect_attempt_ms - now_ms;
                } else {
                    s_connect_requested = false;
                    s_next_connect_attempt_ms = 0U;
                    attempt_override_revision = s_runtime_override_revision;
                    attempt_profiles_revision = s_applied_profiles_revision;
                    should_connect = true;
                }
        ...
        /* Retry backoff limits scans, not worker liveness or config refresh.
         * Keep checking state during a long AP-absent cooldown, while waking
         * sooner for a near retry deadline. Notifications still wake us early. */
        if (connect_due_in_ms > 0U && connect_due_in_ms < delay_ms) {
        ...
        if (should_connect) {
            ...
            /* A scan with no eligible saved AP produces no disconnect event.
             * Keep discovery alive with bounded backoff rather than requiring
             * a dashboard reconnect command. Respect newer manual suppression. */
            if (connect_err != ESP_OK && xSemaphoreTake(s_lock, portMAX_DELAY) == pdTRUE) {
                (void)wifi_mgr_refresh_config_locked(NULL);
                if (!s_status.connected && s_status.started && !s_runtime_connect_suppressed &&
                    !s_startup_connect_suppressed) {
                    s_connect_requested = true;
                    if (attempt_override_revision != s_runtime_override_revision ||
                        attempt_profiles_revision != s_applied_profiles_revision) {
                        s_next_connect_attempt_ms = 0U;
                        wifi_mgr_reset_absent_retry_backoff();
                    } else {
                        s_next_connect_attempt_ms = unified_tick_now_ms() + wifi_mgr_next_absent_retry_delay_ms();
                    }
                }
                xSemaphoreGive(s_lock);
            }
        ...
        if (delay_ticks == 0) delay_ticks = 1; /* Sub-tick deadline: never spin. */
...
esp_err_t wifi_mgr_init(void) {
    ...
    config_mgr_snapshot(&config);
    err = wifi_mgr_profiles_auto_state(&s_status.configured, &s_applied_profiles_revision);
    if (err != ESP_OK) return err;
    s_startup_connect_suppressed =
    ...
    if (s_runtime_override_active && s_status.configured) {
...
esp_err_t wifi_mgr_request_connect(void) {
    ...
    if (config_changed && s_runtime_override_active && s_status.configured) {
        wifi_mgr_build_sta_config(&config, &wifi_config, NULL);
        ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));
    }
    configured = s_status.configured;
...
esp_err_t wifi_mgr_request_runtime_connect(const char *ssid, const char *password) {
    ...
    s_runtime_override_started_ms = unified_tick_now_ms();
    s_runtime_override_revision++;
    ...
    /* The scan worker owns the Wi-Fi driver until its generation check and
     * cleanup complete. It will reject its stale candidate; the queued
     * request below will then apply this newer target. */
    if (!scan_in_progress) {
    ...
    if (already_on_target) {
        ...
        apply_disconnect = !scan_in_progress;
    }
...
void wifi_mgr_get_status(wifi_mgr_status_t *out_status) {
    ...
    *out_status = s_status;
    xSemaphoreGive(s_lock);
    (void)wifi_mgr_profiles_metadata(&out_status->profiles_revision, &out_status->profiles_count);
}
