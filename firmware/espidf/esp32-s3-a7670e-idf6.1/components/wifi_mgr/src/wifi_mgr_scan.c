...
typedef struct {
    ...
    wifi_mgr_profile_set_t profiles;
    bool manual_target;
...
static uint16_t wifi_mgr_find_target_record(
    ...
    const wifi_mgr_profile_set_t *profiles
) {
    if (!records) {
        return UINT16_MAX;
    }
    ...
    for (uint16_t index = 0; index < record_count; ++index) {
        ...
        if (profiles) {
            for (size_t profile_index = 0; profile_index < profiles->count; ++profile_index) {
                const wifi_mgr_profile_t *profile = &profiles->profiles[profile_index];
                if (profile->enabled && strcmp((const char *)records[index].ssid, profile->ssid) == 0) {
                    priority = profile->priority;
                    break;
                }
            }
        } else if (target_ssid && target_ssid[0] != '\0' && strcmp((const char *)records[index].ssid, target_ssid) == 0) {
            priority = 1U;
        }
        if (priority != UINT16_MAX && (best_index == UINT16_MAX || priority < best_priority ||
            (priority == best_priority && (records[index].rssi > best_rssi ||
             (records[index].rssi == best_rssi && strcmp((const char *)records[index].ssid,
                 (const char *)records[best_index].ssid) < 0))))) {
            best_index = index;
            best_priority = priority;
            best_rssi = records[index].rssi;
        }
    }
    return best_index;
}
...
static esp_err_t wifi_mgr_prepare_connect_config(char *ssid, size_t ssid_size) {
    ...
    uint32_t current_profiles_revision = 0U;
    uint8_t current_profiles_count = 0U;
    ...
    /* Never touch the shared preconnect buffer before checking ownership. */
    memset(scratch, 0, sizeof(*scratch));
    (void)wifi_mgr_refresh_config_locked(&scratch->config);
    scratch->manual_target = wifi_mgr_runtime_override_active_locked();
    scratch->override_revision = wifi_mgr_runtime_override_revision_locked();
    err = wifi_mgr_profiles_get(&scratch->profiles);
    if (err != ESP_OK) {
        xSemaphoreGive(s_lock);
        return err;
    }
    if (scratch->config.wifi_ssid[0] == '\0' && scratch->profiles.count == 0U) {
        xSemaphoreGive(s_lock);
        return ESP_ERR_INVALID_STATE;
    }
    ...
    scratch->active_scan_config.ssid = scratch->manual_target ? (uint8_t *)scratch->config.wifi_ssid : NULL;
    ...
    err = wifi_mgr_run_preconnect_scan(&scratch->config, scratch->manual_target ? NULL : &scratch->profiles, &scratch->active_scan_config, &scratch->active_result);
    if (err != ESP_OK) {
        goto done;
    }
    ...
    if (scratch->active_result.selected_index != UINT16_MAX) {
        ...
        err = wifi_mgr_run_preconnect_scan(&scratch->config, scratch->manual_target ? NULL : &scratch->profiles, &scratch->active_ext_scan_config, &scratch->active_ext_result);
        if (err != ESP_OK) {
            goto done;
        }
        ...
        err = wifi_mgr_run_preconnect_scan(&scratch->config, scratch->manual_target ? NULL : &scratch->profiles, &scratch->passive_scan_config, &scratch->passive_result);
        if (err != ESP_OK) {
            goto done;
        }
    ...
    if (!selected_ap && !scratch->manual_target) {
        err = ESP_ERR_NOT_FOUND;
        goto done;
    }
    if (selected_ap && !scratch->manual_target) {
        for (size_t profile_index = 0; profile_index < scratch->profiles.count; ++profile_index) {
            const wifi_mgr_profile_t *profile = &scratch->profiles.profiles[profile_index];
            if (profile->enabled && strcmp((const char *)selected_ap->ssid, profile->ssid) == 0) {
                unified_copy_cstr(scratch->config.wifi_ssid, sizeof(scratch->config.wifi_ssid), profile->ssid);
                unified_copy_cstr(scratch->config.wifi_password, sizeof(scratch->config.wifi_password), profile->password);
                break;
            }
        }
    }
    ...
    if (scratch->override_revision != wifi_mgr_runtime_override_revision_locked()) {
        xSemaphoreGive(s_lock);
        err = ESP_ERR_INVALID_STATE;
        goto done;
    }
    if (!scratch->manual_target) {
        err = wifi_mgr_profiles_metadata(&current_profiles_revision, &current_profiles_count);
        if (err != ESP_OK || current_profiles_revision != scratch->profiles.revision) {
            xSemaphoreGive(s_lock);
    ...
    if (err == ESP_OK) {
        unified_copy_cstr(s_status.ssid, sizeof(s_status.ssid), scratch->config.wifi_ssid);
    ...
    if (err != ESP_OK && xSemaphoreTake(s_lock, pdMS_TO_TICKS(100)) == pdTRUE) {
        wifi_mgr_set_last_scan_locked(
            ...
            err == ESP_ERR_NOT_FOUND ? "no_eligible_saved_network" : "scan_error"
        );
        if (selected_ap == NULL) {
    ...
    /* Cleanup must not abandon the ownership flag on temporary contention. */
    bool notify_after_scan = false;
    if (claimed_scan_slot && xSemaphoreTake(s_lock, portMAX_DELAY) == pdTRUE) {
        s_scan_in_progress = false;
        notify_after_scan = s_connect_requested && s_status.started && !s_status.connected;
        xSemaphoreGive(s_lock);
    }
    if (notify_after_scan) wifi_mgr_notify_task();
    return err;
}
