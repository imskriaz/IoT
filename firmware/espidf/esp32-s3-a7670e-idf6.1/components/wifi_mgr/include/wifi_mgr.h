...
#define WIFI_MGR_PROFILE_SSID_LEN 33U
#define WIFI_MGR_PROFILE_PASSWORD_LEN 65U

typedef struct {
    bool enabled;
    uint16_t priority;
    char ssid[WIFI_MGR_PROFILE_SSID_LEN];
    char password[WIFI_MGR_PROFILE_PASSWORD_LEN];
} wifi_mgr_profile_t;

typedef struct {
    uint32_t revision;
    uint8_t count;
    wifi_mgr_profile_t profiles[WIFI_MGR_PROFILE_MAX];
} wifi_mgr_profile_set_t;
...
/* Call during startup before Wi-Fi workers. May migrate and commit NVS. */
